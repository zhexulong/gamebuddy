...
1. **Coverage check.** Count the memory ids in the supplied category. Your manifest must contain exactly one `<entry>` or `<merge>` for EVERY id — no id may be missing. If your draft skipped any id, add it before emitting.
2. **Polarity check.** Re-read every cue you wrote. Any cue whose source memory forbids, excludes, rejects, or disables something MUST carry the `⊘` marker directly on the forbidden term, followed by its `(mechanism)` parenthetical. A prohibition written as plain text is a defect: "masons must not use translation APIs" is WRONG; "⊘external translation APIs (`translate-shell`/`trans`) — translate directly" is right.
