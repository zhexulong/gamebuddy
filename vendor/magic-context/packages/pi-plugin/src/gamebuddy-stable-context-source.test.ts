...
describe("GameBuddyStableContextSource", () => {
	...
	it("fails closed for missing adapters, binding mismatch, hash mismatch, and unknown kinds", () => {
		...
		expect(() => materializeGameBuddyStableContextSnapshot(snapshot(), { ...binding, surface: "invalid" as "tavern" })).toThrow("active binding surface is unsupported");
		expect(() => validateGameBuddyStableContextSnapshot(snapshot({ sessionId: "other" }), binding)).toThrow("does not match active binding");
		expect(() => validateGameBuddyStableContextSnapshot(snapshot({ canonicalHash: "0".repeat(64) }), binding)).toThrow("does not match canonical snapshot content");
		const invalid = snapshot();
		(invalid.sources[0] as { kind: string }).kind = "untrusted";
		expect(() => validateGameBuddyStableContextSnapshot(invalid, binding)).toThrow("unsupported source kind");
	});
	...
	it("creates Host-delegated inferred semantic memories in the bound continuity only", async () => {
		const cwd = `${process.env.TMPDIR ?? "/tmp"}/gamebuddy-memory-facade-${crypto.randomUUID()}`;
		const facade = createGameBuddyMemoryFacade({ continuityId: binding.continuityId, runtimeCwd: cwd });
		await expect(facade.createDelegatedInferredSemanticMemory({
			continuityId: "other-continuity",
			operationId: "delegated-memory-test",
			content: "Must fail before storage access.",
		})).rejects.toThrow("gamebuddy_memory_continuity_mismatch");

		const first = await facade.createDelegatedInferredSemanticMemory({
			continuityId: binding.continuityId,
			operationId: "delegated-memory-once",
			content: "The player and companion share a tea ritual.",
		});
		const replay = await facade.createDelegatedInferredSemanticMemory({
			continuityId: binding.continuityId,
			operationId: "delegated-memory-once",
			content: "Provider retry changed its prose but must not create another entry.",
		});
		expect(replay).toEqual(first);
		expect((await facade.listMemories({ continuityId: binding.continuityId })).length).toBe(1);
		await expect(facade.createDelegatedInferredSemanticMemory({
			continuityId: binding.continuityId,
			operationId: "invalid operation id with spaces",
			content: "Must reject untrusted operation identity.",
		})).rejects.toThrow("gamebuddy_memory_delegation_operation_invalid");
	});
