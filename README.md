...
**当前验证状态：** 正式 `HostFarmhandProvisioner`、App `StardewAttachmentFlow` 与 AI-client `FarmhandProvisioner` 已在本机目标版本完成无 UI Host-first 回归：signed attachment、原生 `Saving/Saved`、AI-client `readyToPlay`、AI 退出后 Host 保存、同 Host 重连、Host 重启 nonce 轮换、旧 manifest 拒绝和新 manifest 恢复均通过。正式 AI-client named-pipe bridge 已通过真实 `move_to_tile` 的 `target_reached`、replacement/supersede、local cancel、bridge disconnect invalidation、同 request reconnect replay、stale revision 与 idempotency conflict；`equip_tool` 也有 `before/expected/after` 权威 evidence。Wave 1 的 `travel` 与 Wave 2 的 `till_soil` 也已完成独立真实 gate：目标版本 `PathFindController` 绕过 FarmHouse 家具到达 Warp 邻接 tile，native `Warped` 后置确认到达 `Farm:25,33`；随后从 live `soilTiles` 选择 `(24,34)`，native receipt `succeeded/soil_tilled` 确认真实 `HoeDirt`。`travel` 与 `till_soil` 已进入 published registry，其他未通过 live gate 的动作仍保持隐藏。当前 player-facing Dialogue Director 默认使用 `cpa-oai/deepseek-v4-flash`、`high` thinking；显式启用的私有 gameplay task subagent 独立固定为 `cpa-oai/gpt-5.6-luna`、`medium`，不会继承 DeepSeek 或取得玩家表达工具。两者的真实受限 tool trace 已分别验证 snapshot/knowledge/action/receipt 与权威 completion 边界；同 identity Context re-entry 已验证 JSONL session 恢复和新 live snapshot。尚未宣称完整 Phase 3/4 或“可玩语音 Farmhand Demo”：Game Action 已采用已发布能力默认同意、玩家 deny-by-exception 的权限语义，但完整世界变化/多步成功/人格边界场景仍未逐项验收；SenseVoiceSmall/FSMN-VAD 的真实 CPU asset/runtime 与音频设备 manifest 未配置，因此真实 `@voice` ASR/设备硬门仍 blocked。`tools/check-stardew-phase1-prerequisites.ps1 -GamePath <path>` 继续只报告环境前置，不把 diagnostic probe 或单客户端 smoke 当作通过。
