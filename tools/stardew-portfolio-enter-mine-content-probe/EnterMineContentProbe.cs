...
var expectedAssemblies = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
{
...
var assemblyPath = VerifiedChild(root, "Stardew Valley.dll", expectedAssemblies["Stardew Valley.dll"]);
var xTilePath = VerifiedChild(root, "xTile.dll", expectedAssemblies["xTile.dll"]);
var monoGamePath = VerifiedChild(root, "MonoGame.Framework.dll", expectedAssemblies["MonoGame.Framework.dll"]);
var mapsPath = VerifiedChild(root, Path.Combine("Content", "Maps", "Mine.xnb"), ExpectedMapDigest);
if (assemblyPath is null || xTilePath is null || monoGamePath is null || mapsPath is null || expectedAssemblies.Any(entry => VerifiedChild(root, entry.Key, entry.Value) is null)) { Console.Error.WriteLine("snapshot_missing_reparse_or_hash_drift"); return 3; }
var before = Snapshot(root, expectedAssemblies, mapsPath);
...
try
{
    ...
    AssertSnapshot(root, expectedAssemblies, mapsPath, before);
    ...
    foreach (var layer in layers)
    {
        ...
        for (var y = 0; y < height; y++) for (var x = 0; x < width; x++)
        {
            ...
            if (text is not ("Mine" or "Mine 77377")) continue;
            if (layerName != "Buildings" || text.Length > 128 || records.Count >= 2) throw new InvalidOperationException("mine_action_records_exceed_bound");
            records.Add(new { layer = layerName, x, y, action = text });
        }
    }
    AssertSnapshot(root, expectedAssemblies, mapsPath, before);
    Console.WriteLine(JsonSerializer.Serialize(new { state = "probed", gameAssemblyVersion = gameAssembly.GetName().Version?.ToString(), mapAsset = ExpectedAsset, mapFile = "Content/Maps/Mine.xnb", mapXnbSha256 = ExpectedMapDigest, layerCount = layers.Length, layerNames = layers.Select(layer => Text(Get(layer, "Id")) ?? Text(Get(layer, "Name")) ?? "<unnamed>").ToArray(), actionCount = records.Count, actions = records, note = "Locked private snapshot map Action producers only; Mine 77377 is reported for explicit exclusion, never as an enter_mine option." }, new JsonSerializerOptions { WriteIndented = true }));
    return 0;
}
...
static Dictionary<string, string> Snapshot(string root, Dictionary<string, string> assemblies, string map) => assemblies.Keys.Append(Path.Combine("Content", "Maps", "Mine.xnb")).ToDictionary(relative => relative, relative => Identity(RegularChild(root, relative)!), StringComparer.OrdinalIgnoreCase);
static void AssertSnapshot(string root, Dictionary<string, string> assemblies, string map, Dictionary<string, string> before)
{
    foreach (var entry in before) if (!string.Equals(Identity(RegularChild(root, entry.Key)!), entry.Value, StringComparison.Ordinal)) throw new InvalidOperationException("snapshot_identity_or_hash_drift");
    if (VerifiedChild(root, Path.Combine("Content", "Maps", "Mine.xnb"), ExpectedMapDigest) is null) throw new InvalidOperationException("map_snapshot_hash_drift");
}
