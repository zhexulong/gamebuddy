...
try {
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_npc_relationship_target", trace, snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "npc_relationship_inspected" : receipt.reasonCode, target, receipt: summarizeReceipt(receipt), evidence, afterTarget, trace, before: summarize(before), after: summarize(after), durationMs: Date.now() - startedAt }));
...
function chooseTarget(snapshot) {
  return snapshot.npcRelationshipTargets?.find((entry) => Number.isInteger(entry.x) && Number.isInteger(entry.y) && typeof entry.targetId === "string" && typeof entry.npcName === "string") ?? null;
}
...
function summarize(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, npcRelationshipTargets: snapshot.npcRelationshipTargets?.length ?? 0, activeExecution: snapshot.activeExecution ?? null }; }
function targetLocation(target, snapshot) { return snapshot.location; }
...
function findReceipt(executionId) { return receipts.find((receipt) => receipt.executionId === executionId) ?? (client.state.latestReceipt?.executionId === executionId ? client.state.latestReceipt : null); }
function farmhouseApproachCandidates(snapshot) {
  ...
  candidates.sort((left, right) => left.chebyshevDistance - right.chebyshevDistance || left.manhattanDistance - right.manhattanDistance || left.y - right.y || left.x - right.x);
...
async function travelToFarm(snapshot) {
  ...
  const accepted = await client.execute({ requestId: `npc_fixture_travel_${Date.now()}`, idempotencyKey: `npc_fixture_travel_idem_${Date.now()}`, action: "travel", args: { x: warp.sourceX, y: warp.sourceY }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
  ...
  return await waitFor((latest) => latest.location === "Farm" && latest.activeExecution == null, accepted.executionId, 15_000);
}
async function move(snapshot, target, phase) {
  ...
  return await waitFor((latest) => latest.activeExecution == null && adjacent(latest.tile, target), accepted.executionId, 55_000);
}
