...
try {
  ...
  console.log(JSON.stringify({ state: passed ? "passed" : "blocked", topology: "native_local_player_fixture", reasonCode: passed ? "animal_product_collected" : "collect_animal_product_postcondition_mismatch", target, receipt: summaryReceipt(receipt), evidence, inventory: { qualifiedItemId: target.qualifiedProduceItemId, before: inventoryBefore, after: inventoryAfter, expectedDelta: target.produceStack, freshDeltaSatisfied: inventoryGainedFresh }, before: summary(before), after: summary(after), durationMs: Date.now() - startedAt }));
  if (!passed) process.exitCode = 2;
} catch (error) {
  console.error(JSON.stringify({ state: "blocked", topology: "native_local_player_fixture", reasonCode: String(error instanceof Error ? error.message : error).slice(0, 256), latestReceipt: summaryReceipt(client.state.latestReceipt), durationMs: Date.now() - startedAt }));
  process.exitCode = 2;
} finally { unsubscribe(); client.close(); }

function validateConfig(value) {
  const fixture = value?.NativeLocalPlayerFixture;
  if (fixture?.Enable !== true || fixture.Bootstrap?.Enable === true || fixture.FixtureScenario !== "native_collect_animal_product_v1" || value.ActionPolicyVersion !== 0 || !same(value.EnabledActions, ["collect_animal_product"])) throw new Error("native_local_collect_animal_product_fixture_config_invalid");
  if (value.Portfolio?.Enable === true || value.HostAutomation?.Enable === true || value.HostFarmhandProvisioning?.Enable === true || value.FarmhandProvisioner?.Enable === true) throw new Error("native_local_collect_animal_product_topology_invalid");
}
function requireCapabilities(snapshot) { if (!same([...(snapshot.capabilities ?? [])].sort(), ["cancel_active_execution", "collect_animal_product", "inspect_self"].sort())) throw new Error("native_local_collect_animal_product_capability_not_isolated"); }
async function actionableSnapshot() { const snapshot = await client.observe(); if (!snapshot.actionable || snapshot.activeExecution != null || !Number.isInteger(snapshot.revision)) throw new Error("native_local_collect_animal_product_snapshot_not_actionable"); return snapshot; }
function countQualifiedInventory(snapshot, qualifiedItemId) {
  ...
  return facts.filter((entry) => entry?.qualifiedItemId === qualifiedItemId && Number.isInteger(entry.stack) && entry.stack > 0)
    .reduce((total, entry) => total + entry.stack, 0);
}
function chooseTarget(snapshot) {
  const targets = (snapshot.animalProductTargets ?? []).filter((entry) => validId(entry?.targetId) && Number.isInteger(entry.slot) && validTile(entry.x) && validTile(entry.y) && typeof entry.qualifiedProduceItemId === "string" && (entry.toolKind === "milk_pail" || entry.toolKind === "shears") && (entry.produceStack === 1 || entry.produceStack === 2));
...
async function waitForTerminal(accepted, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const receipt = receipts.find((entry) => entry.executionId === accepted.executionId && entry.requestId === accepted.requestId && terminal(entry.state));
...
function terminal(state) { return ["succeeded", "failed", "invalidated", "cancelled", "expired", "uncertain", "rejected"].includes(state); }
...
function summary(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, activeExecution: snapshot.activeExecution ?? null, animalProductTargets: snapshot.animalProductTargets?.length ?? 0, inventoryItemFacts: snapshot.inventoryItemFacts?.length ?? 0 }; }
