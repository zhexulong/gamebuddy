...
function identity(overrides = {}) {
  return { requestId: "req", traceId: "trace", executionId: "exec", ...overrides };
}
  ...
  const base = {
    ...
    evidence: {
      ...
      phaseTrace: [
        ...
        phase("transition_started", 8, "mine_elevator_transition_started"),
        phase("postcondition", 9, "postcondition_observed"),
        phase("terminal", 9, "mine_elevator_floor_selected"),
      ],
      entryObserved: true,
      currentFloorBefore: 20,
      lowestMineLevelBefore: 25,
      opaqueElevatorTarget: "target",
      nativeElevatorTransitionObserved: true,
      currentFloorAfter: 25,
      lowestMineLevelAfter: 25,
      lowestMineLevelObserved: true,
    },
    postcondition: {
      selectedCheckpoint: 25,
      actualCurrentFloor: 25,
      observedLowestMineLevel: 25,
      opaqueElevatorTarget: "target",
...
function nativeGiven(overrides = {}) {
  return {
    source: "target_version_native_probe",
    readOnly: true,
...
function freshFloor(overrides = {}) {
  return {
    ...identity(),
    source: "target_version_native_floor_reader",
    fresh: true,
    readOnly: true,
    saveMutationObserved: false,
    gameplayMutationObserved: false,
...
test("Given uses only a dynamic native read-only probe and rejects terminal/static fixture facts", async () => {
  const observed = [];
  const ready = await readM8ElevatorGiven({
    expectedScope: scope,
    observeNative: async () => {
      observed.push("read");
      return nativeGiven();
  ...
  assert.equal(
    (
      await readM8ElevatorGiven({
        expectedScope: scope,
        observeNative: async () => nativeGiven({ terminalOutcomePresent: true }),
      })
    ).state,
    "BLOCKED",
  );
  assert.equal(
    (
      await readM8ElevatorGiven({
        expectedScope: scope,
        observeNative: async () => nativeGiven({ selectedCheckpoint: 0 }),
      })
    ).state,
    "BLOCKED",
  );
  assert.equal(
    (
      await readM8ElevatorGiven({
        expectedScope: scope,
        observeNative: async () => nativeGiven({ scope: { ...scope, forged: true } }),
      })
    ).state,
    "BLOCKED",
  );
...
test("Then requires exact receipt semantics and execution-correlated native fresh floor facts", async () => {
  const reads = [];
  const result = await replayM8ElevatorResult({
    request,
    acceptedPhase: accepted,
    receipt: receipt(),
    expectedScope: scope,
    readFreshFloor: async (value) => {
      reads.push(value);
      return freshFloor();
    },
  });
  ...
  for (const invalid of cases)
    assert.equal(
      (
        await replayM8ElevatorResult({
          request,
          acceptedPhase: accepted,
          receipt: invalid,
          expectedScope: scope,
          readFreshFloor: async () => freshFloor(),
        })
      ).state,
      "BLOCKED",
    );
  assert.equal(
    (
      await replayM8ElevatorResult({
        ...
        receipt: receipt(),
        expectedScope: scope,
        readFreshFloor: async () => freshFloor({ traceId: "other" }),
      })
    ).state,
    "BLOCKED",
  );
...
test("composed preflight binds Given to request without an unrelated persistence reader", async () => {
  const args = {
    expectedScope: scope,
    observeNative: async () => nativeGiven(),
    request,
    acceptedPhase: accepted,
    receipt: receipt(),
    readFreshFloor: async () => freshFloor(),
  ...
  const mismatch = await runM8ElevatorPreflight({
    ...args,
    observeNative: async () => nativeGiven({ selectedCheckpoint: 30, lowestMineLevel: 30 }),
  });
...
test("PREFLIGHT_READY is only a complete callback-backed BDD pipeline, never live closure", async () => {
  const result = await runM8ElevatorPreflight({
    expectedScope: scope,
    observeNative: async () => nativeGiven(),
    request,
    acceptedPhase: accepted,
