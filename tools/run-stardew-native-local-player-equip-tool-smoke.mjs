...
try {
  ...
  if (!before.actionable || before.activeExecution != null) throw new Error("native_local_equip_tool_player_not_actionable");
  if (!before.capabilities.includes("equip_tool")) throw new Error("native_local_equip_tool_capability_missing");
  const selected = before.toolSlots?.find((entry) => Number.isInteger(entry.slot) && typeof entry.label === "string" && entry.label.length > 0 && entry.label !== before.currentTool)
  ...
  const passed = receipt.state === "succeeded" && receipt.reasonCode === "tool_selected"
    && typeof expectedTool === "string" && expectedTool.length > 0
    && expectedTool === selectedLabel && evidence.after === selectedLabel
  ...
  console.error(JSON.stringify({ state: "blocked", reasonCode: String(error instanceof Error ? error.message : error).slice(0, 256), latestReceipt: summarizeReceipt(client.state.latestReceipt) }));
  process.exitCode = 2;
} finally {
  client.close();
}
...
function summarize(snapshot) { return { revision: snapshot.revision, currentTool: snapshot.currentTool ?? null, toolSlots: snapshot.toolSlots ?? [] }; }
