...
try {
  ...
  const target = Array.isArray(snapshot.petTargets)
    ? snapshot.petTargets.find((entry) => entry.pettedToday === false && typeof entry.targetId === "string" && Number.isInteger(entry.x) && Number.isInteger(entry.y)) ?? null
    : null;
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_unpetted_pet_target", snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
...
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, petTargets: snapshot.petTargets?.length ?? 0 };
}
function summarizeTarget(target) {
  return { targetId: target.targetId, x: target.x, y: target.y, petType: target.petType, friendship: target.friendship, pettedToday: target.pettedToday };
}
