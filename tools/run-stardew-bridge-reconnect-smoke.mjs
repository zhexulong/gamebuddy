...
if (!Number.isSafeInteger(expectedRevision) || !Number.isInteger(targetX) || !Number.isInteger(targetY))
...
try {
  ...
  const passed =
    replay.executionId === expectedExecutionId &&
    replay.state === "invalidated" &&
    replay.reasonCode === "bridge_disconnected";
  console.log(
    JSON.stringify({
      state: passed ? "passed" : "blocked",
      reconnectRevision: snapshot.revision,
      replay: {
