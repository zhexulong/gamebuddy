...
test("bait content contract accepts only the pinned target-version object datum", () => {
  const probe = {
    objectsContent: {
      state: "loaded",
      digest: BAIT_CONTRACT.expectedObjectsDigest,
      entries: [{ itemId: "685", name: "Bait", category: -21, type: "Basic", unknownFields: [] }],
    },
  };
...
test("bait content contract fails closed for hash-derived digest or semantic drift", () => {
  ...
  assert.equal(
    validateBaitProbe({ objectsContent: { ...valid, entries: [{ ...valid.entries[0], category: -20 }] } }),
    "bait_content_contract_mismatch",
  );
});
