...
test("P5 publishes only the bounded body categories through the existing semantic publication path", async () => {
  ...
  assert.match(
    manager,
    /ExecutionState\.Failed or ExecutionState\.Expired or ExecutionState\.Uncertain\s*=>\s*"execution_settled_failed"/,
  );
  ...
  assert.doesNotMatch(
    manager,
    /if \(category is "execution_settled_succeeded" or "execution_settled_cancelled" or "execution_settled_failed" or "execution_invalidated"\)\s*this\.AddPublicTrace\("body_idle", executionId, requestId\)/s,
  );
  assert.match(
    manager,
    /this\.active = null;\s*LocalExecutionReceipt receipt = new\(specification\.ExecutionId,[\s\S]*?this\.AddTrace\(receipt\);\s*if \(terminal\)\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/,
  );
  assert.match(
    manager,
    /if \(animationComplete\)\s*\{\s*this\.activeItemUse = null;\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/s,
  );
  assert.match(
    manager,
    /if \(animationComplete\)\s*\{\s*Game1\.player\.CurrentToolIndex = specification\.PreviousSlot;\s*this\.activeAnimalProduct = null;\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/s,
  );
...
test("P5 release-settlement matrix traces every released manager owner before one idle", async () => {
  ...
  assert.match(
    manager,
    /activeTravel\.DeadlineMs[\s\S]*?this\.activeTravel = null;[\s\S]*?this\.Remember\(receipt\);\s*this\.AddTrace\(receipt\);\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/,
  );
  assert.match(
    manager,
    /CompleteTravelAfterWarp\(\)[\s\S]*?this\.activeTravel = null;\s*this\.Remember\(receipt\);\s*this\.AddTrace\(receipt\);\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/,
  );
  ...
  assert.match(
    manager,
    /if \(this\.activeItemPickup is \{ \} pickup[\s\S]*?this\.active = null;[\s\S]*?this\.activeItemPickup = null;[\s\S]*?this\.Remember\(pickupReceipt\);\s*this\.AddTrace\(pickupReceipt\);\s*this\.PublishIdleAfterRelease\(pickup\.ExecutionId, pickup\.RequestId\);/,
  );
  assert.match(
    manager,
    /else if \(\(!sameLocation \|\| nowMs > specification\.DeadlineMs\) && approachSettled\)[\s\S]*?this\.activeItemPickup = null;[\s\S]*?this\.AddTrace\(receipt\);\s*this\.PublishIdleAfterRelease\(specification\.ExecutionId, specification\.RequestId\);/,
  );
  ...
  assert.match(
    manager,
    /if \(this\.idlePublishedExecutionIds\.Contains\(executionId\)\)\s*return;[\s\S]*?this\.pendingIdleByExecutionId\[executionId\] = requestId;/,
  );
  assert.match(
    manager,
    /!this\.idlePublishedExecutionIds\.Add\(executionId\)[\s\S]*?this\.pendingIdleByExecutionId\.Remove\(executionId\);\s*this\.AddPublicTrace\("body_idle", executionId, requestId\);/,
  );
  assert.match(
    manager,
    /Farmer\? player = Game1\.player;\s*ExecutionTrace entry = new\(category, executionId, requestId, this\.tick, this\.revision,\s*player\?\.currentLocation\?\.NameOrUniqueName, player\?\.Tile\);/,
  );
});
...
test("P5 native tick and terminal paths settle controller ownership without an ambient pilot", async () => {
  ...
  assert.match(
    controller,
    /if \(ReferenceEquals\(Game1\.player\.controller, this\.pathController\)\)\s*Game1\.player\.controller = null;[\s\S]*?Game1\.player\.Halt\(\);[\s\S]*?this\.active = null;/,
  );
  ...
  assert.match(
    manager,
    /Farmer\? player = Game1\.player;[\s\S]*?if \(player is null\)\s*return CreateWorldNotReadyBridgeSnapshot\(advertisedCapabilities\);/,
  );
  assert.match(
    manager,
    /private BridgeSnapshot CreateWorldNotReadyBridgeSnapshot\(IReadOnlyList<string> advertisedCapabilities\)/,
  );
});
