...
if (["SaveId", "WorldId", "PlayerId", "CompanionId", "PipeName", "BridgeToken"].some((key) => typeof config[key] !== "string" || config[key].length === 0)) {
...
try {
  ...
  if (!client.state.capabilities.includes("move_to_tile") || !initial.capabilities.includes("move_to_tile")) throw new Error("move_to_tile_capability_missing");
  ...
  client.close();
  console.log(JSON.stringify({ state: "accepted_then_disconnected", requestId: request.requestId, idempotencyKey: request.idempotencyKey, action: request.action, args: request.args, expectedRevision: request.expectedRevision, executionId: accepted.executionId, target, revision: accepted.revision }));
} catch (error) {
  client.close();
