...
import { createFormalActionGate, FormalActionGateError, isTerminalExecutionState } from "./lib/stardew-formal-action-gate.mjs";

const scopeSnapshot = (overrides = {}) => ({ revision: 1, actionable: true, activeExecution: null, ...overrides });
const accepted = { executionId: "execution_1", requestId: "request_1", state: "accepted", reasonCode: "accepted", revision: 1, evidence: null };
const succeeded = { ...accepted, state: "succeeded", reasonCode: "item_used", revision: 2 };

for (const state of ["blocked", "invalidated", "succeeded", "partially_succeeded", "failed", "cancelled", "expired", "rejected", "uncertain"]) {
...
test("nonterminal states are not terminal", () => {
  for (const state of ["accepted", "running", "meaningful_progress", "unknown"]) assert.equal(isTerminalExecutionState(state), false);
});
...
test("waits for same-execution terminal event and then fresh rereads", async () => {
  const client = fakeClient({
    ...
    snapshots: [scopeSnapshot({ revision: 1, activeExecution: { executionId: "execution_1" } }), scopeSnapshot({ revision: 2 })],
  });
  const gate = createFormalActionGate(client, { pollMs: 25 });
  try {
    const result = await gate.executeAndAwaitTerminal({ action: "use_item" }, { terminalTimeoutMs: 1_000, settleTimeoutMs: 1_000 });
...
test("fails closed when bridge disconnects while waiting", async () => {
  ...
  try {
    await assert.rejects(
      ...
      (error) => error instanceof FormalActionGateError && error.reasonCode === "bridge_disconnected" && error.details.reasonCode === "pipe_closed",
    );
  } finally { gate.close(); }
});
...
function fakeClient({ execute, snapshots }) {
  ...
  const client = {
    ...
    observeCount: 0,
    onFact(listener) { facts.add(listener); return () => facts.delete(listener); },
    onConnectionFact(listener) { connections.add(listener); return () => connections.delete(listener); },
    async observe() {
      client.observeCount++;
