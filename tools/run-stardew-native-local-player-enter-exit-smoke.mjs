...
async function observeActionable() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null)
    throw new Error("native_local_enter_exit_player_not_actionable");
  if (
...
function requireExactCapabilities(snapshot) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected))
    throw new Error("native_local_enter_exit_capability_not_isolated");
}
...
function summary(snapshot) {
  return {
    ...
    doorTargets: snapshot.doorTargets?.map((door) => doorSummary(snapshot.location, door)) ?? [],
  };
}
