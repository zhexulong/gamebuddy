...
try {
  ...
  if (door === null && process.argv.includes("--prepare-with-move")) {
    ...
    if (door !== null) {
      ...
      trace.push({
        phase: "prepare_move",
        target: { x: door.sourceX, y: door.sourceY },
        receipt: summarizeReceipt(move),
        door,
      });
      ...
      trace.push({
        ...
        snapshot: summarizeSnapshot(moved.snapshot),
      });
      if (moved.receipt?.state !== "succeeded" || moved.receipt.reasonCode !== "target_reached")
        throw new Error(`prepare_move_failed:${moved.receipt?.state}:${moved.receipt?.reasonCode}`);
  ...
  if (door === null) {
    console.log(
      JSON.stringify({
        state: "blocked",
        reasonCode: "no_adjacent_native_door",
        snapshot: summarizeSnapshot(snapshot),
        doorCount: snapshot.doorTargets?.length ?? 0,
    ...
    if (accepted.state !== "accepted")
      throw new Error(`enter_exit_not_accepted:${accepted.state}:${accepted.reasonCode}`);
  ...
  const recovered =
    latest?.executionId === acceptedExecutionId &&
  ...
  if (recovered) {
    ...
    console.log(
      JSON.stringify({
        ...
        target,
        trace,
        recoveredReceipt: summarizeReceipt(latest),
        durationMs: Date.now() - startedAt,
      }),
    );
    if (!passed) process.exitCode = 2;
...
function chooseAdjacentDoor(snapshot) {
  ...
  return (
    snapshot.doorTargets.find(
      (door) => Math.abs(door.sourceX - snapshot.tile.x) <= 1 && Math.abs(door.sourceY - snapshot.tile.y) <= 1,
    ) ?? null
  );
}
function chooseNearestDoor(snapshot) {
  ...
  return (
    [...snapshot.doorTargets].sort(
      (a, b) =>
        Math.abs(a.sourceX - snapshot.tile.x) +
        Math.abs(a.sourceY - snapshot.tile.y) -
        Math.abs(b.sourceX - snapshot.tile.x) -
        Math.abs(b.sourceY - snapshot.tile.y),
    )[0] ?? null
  );
}
...
async function waitForMoveTerminal(client, executionId, door, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    const receipt =
      client.state.latestReceipt?.executionId === executionId ? client.state.latestReceipt : findReceipt(executionId);
  ...
  if (finalReceipt !== null && isTerminalState(finalReceipt.state))
    return { receipt: summarizeReceipt(finalReceipt), snapshot: latest };
...
async function waitForTransitionTerminal(client, executionId, door, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (latest?.location === door.targetLocation && latest.tile.x === door.targetX && latest.tile.y === door.targetY) {
      if (refreshedReceipt?.state === "succeeded")
        return { receipt: summarizeReceipt(refreshedReceipt), snapshot: latest };
    }
    await delay(250);
  }
...
function snapshotFromReceipt(receipt) {
  ...
  return actual
    ? {
        location: actual[0],
        tile: { x: Number(actual[1]), y: Number(actual[2]) },
        actionable: true,
        activeExecution: null,
        doorTargets: null,
      }
    : { location: "unknown", tile: { x: -1, y: -1 } };
}
...
function isDoorArrival(snapshot, door) {
  return Math.abs(snapshot.tile.x - door.sourceX) <= 1 && Math.abs(snapshot.tile.y - door.sourceY) <= 1;
}
...
function summarizeSnapshot(snapshot) {
  return {
    ...
    activeExecution: snapshot.activeExecution?.executionId ?? null,
  };
}
