...
try {
  ...
  const request = await execute("harvest", "harvest_crop", {
    x: target.x,
    y: target.y,
  ...
  if (terminal.state !== "succeeded" || terminal.reasonCode !== "crop_harvested") throw new Error(`harvest_failed:${terminal.reasonCode}`);
...
async function waitForFreshActionablePostcondition(timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (String(error instanceof Error ? error.message : error) !== "native_local_harvest_player_not_actionable") throw error;
      await delay(100);
    }
  }
...
function requireActionableSnapshot(snapshot) {
  if (!snapshot.actionable || snapshot.activeExecution != null) throw new Error("native_local_harvest_player_not_actionable");
  if (!Number.isInteger(snapshot.revision) || !Number.isInteger(snapshot.tile?.x) || !Number.isInteger(snapshot.tile?.y)) throw new Error("native_local_harvest_snapshot_invalid");
  if (!Array.isArray(snapshot.capabilities) || !Array.isArray(snapshot.warps) || !Array.isArray(snapshot.harvestTargets)) throw new Error("native_local_harvest_snapshot_facts_missing");
  return snapshot;
}
...
function requireExactCapabilities(snapshot) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error("native_local_harvest_capability_not_isolated");
}
...
function hasOneFreshHarvestTarget(snapshot) { return validHarvestTargets(snapshot).length === 1; }
function chooseOnlyFreshHarvestTarget(snapshot) {
  const targets = validHarvestTargets(snapshot);
  if (targets.length !== 1) throw new Error(targets.length === 0 ? "no_fresh_live_harvest_target" : "ambiguous_live_harvest_targets");
  return targets[0];
}
function validHarvestTargets(snapshot) {
  if (!Array.isArray(snapshot.harvestTargets)) return [];
  return snapshot.harvestTargets.filter((target) => Number.isInteger(target?.x) && Number.isInteger(target?.y)
    && target.x >= 0 && target.y >= 0 && typeof target.targetId === "string" && target.targetId.length > 0
    && typeof target.qualifiedHarvestItemId === "string" && target.qualifiedHarvestItemId.length > 0
    && typeof target.regrowsAfterHarvest === "boolean" && adjacent(snapshot.tile, target));
}
...
function parseEvidence(evidence) {
  ...
  for (const field of detail.split(";")) {
    ...
    if (!/^[a-z][a-z0-9_]{0,63}$/.test(key) || value.length > 512 || Object.hasOwn(result, key)) throw new Error("invalid_harvest_evidence");
    result[key] = value;
  }
  return result;
}
...
function targetSummary(target) { return { targetId: target.targetId, x: target.x, y: target.y, cropId: target.cropId, qualifiedHarvestItemId: target.qualifiedHarvestItemId, regrowsAfterHarvest: target.regrowsAfterHarvest }; }
function snapshotSummary(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, currentTool: snapshot.currentTool ?? null, harvestTargets: snapshot.harvestTargets?.map(targetSummary) ?? [], activeExecution: snapshot.activeExecution ?? null }; }
