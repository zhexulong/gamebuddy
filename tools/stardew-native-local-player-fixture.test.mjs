...
test("native-local bootstrap launcher does not restore an external action template", async () => {
  const launcher = await readFile(new URL("./run-stardew-native-local-player-move-fixture.ps1", import.meta.url), "utf8");
...
test("native-local machine-load fixture supplies only an idle Keg and exact Coffee Bean stack", async (t) => {
  ...
  const setupStart = entry.indexOf('if (fixture.FixtureScenario is "native_machine_inspect_v1" or "native_machine_coffee_load_v1" or "native_machine_coffee_collect_v1")');
  const setup = entry.slice(setupStart, entry.indexOf('if (fixture.FixtureScenario == "native_dig_artifact_spot_v1")', setupStart));
  ...
  assert.doesNotMatch(setup, /\.checkAction\(|PlaceInMachine|performObjectDropInAction|RequestLocalLoadCoffeeIntoKeg|PublishReceipt/);
  ...
  const action = manager.slice(actionStart, manager.indexOf("public LocalExecutionReceipt RequestLocalInspectMachine", actionStart));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-machine-load-smoke.mjs", import.meta.url), "utf8");
...
test("native-local machine-collect fixture starts from loading only; production owns ready-time and collection", async (t) => {
  ...
  assert.doesNotMatch(action, /PlaceInMachine|performObjectDropInAction/);
  const runner = await readFile(new URL("./run-stardew-native-local-player-machine-collect-output-smoke.mjs", import.meta.url), "utf8");
...
test("native-local dig-artifact-spot fixture selects an intact artifact spot and Basic Hoe without consuming sources", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf('if (fixture.FixtureScenario == "native_clear_hoedirt_v1")', setupStart));
  ...
  assert.doesNotMatch(setup, /farm\.dropObject\(artifact, artifactTile \* 64f, Game1\.viewport, initialPlacement: true\)/);
  assert.match(setup, /farm\.objects\.Pairs\.Count\(pair => pair\.Value\.QualifiedItemId == "\(O\)590"\)/);
  assert.match(setup, /artifactSourceCount < 1/);
  assert.doesNotMatch(setup, /DoFunction|digUpArtifactSpot|checkAction|RequestLocalDigArtifactSpot|PublishReceipt|\b(?:request|receipt|reward|debris|pickup|outcome)\b|Items\[/i);
  ...
  assert.match(entry, /!farm\.IsTileOccupiedBy\(artifactPending\.StandingTile, ~CollisionMask\.Farmers, CollisionMask\.None, useFarmerTile: false\)/);
  const failureMarker = 'error=fixture_native_local_artifact_spot_approach_unreachable';
  const failureBranchStart = entry.indexOf('this.nativeLocalDigArtifactSpotFixturePending = null;', entry.indexOf('if (this.nativeLocalDigArtifactSpotFixturePending'));
  const failureBranch = entry.slice(failureBranchStart, entry.indexOf('if (this.nativeLocalClearHoeDirtFixturePending', failureBranchStart));
  ...
  for (const fact of [
    'player_is_game1_player', 'new_location_is_farm', 'farm_name_equal', 'source_count',
    'source_present_at_expected_tile', 'source_qid_is_590', 'source_on_map', 'source_terrain_absent',
    'source_hoedirt_absent', 'source_indoor_pot', 'standing_on_map', 'standing_passable',
    'standing_occupied_use_farmer_tile_false', 'hoe_count', 'basic_hoe', 'player_tile',
  ...
  const helper = entry.slice(helperStart, entry.indexOf("private static Vector2? FindNativeLocalFarmFixtureTile", helperStart));
  ...
  assert.match(helper, /!farm\.IsTileOccupiedBy\(candidate, ~CollisionMask\.Farmers, CollisionMask\.None, useFarmerTile: false\)/);

  const artifactWarpStart = entry.indexOf("if (this.nativeLocalDigArtifactSpotFixturePending");
  const artifactWarp = entry.slice(artifactWarpStart, entry.indexOf("if (this.nativeLocalClearHoeDirtFixturePending", artifactWarpStart));
  ...
  assert.match(artifactWarp, /!farm\.IsTileOccupiedBy\(artifactPending\.StandingTile, ~CollisionMask\.Farmers, CollisionMask\.None, useFarmerTile: false\)/);
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-dig-artifact-spot-smoke.mjs", import.meta.url), "utf8");
  assert.match(runner, /same\(value\.EnabledActions, \["move_to_tile", "travel", "equip_tool", "dig_artifact_spot"\]\)/);
  assert.match(runner, /\["cancel_active_execution", "dig_artifact_spot", "equip_tool", "inspect_self", "move_to_tile", "travel"\]/);
  ...
  assert.match(runner, /Math\.abs\(\(-parseFiniteDecimal\(evidence\.stamina_delta\)\) - parseFiniteDecimal\(evidence\.expected_stamina_cost\)\) <= DIG_ARTIFACT_STAMINA_EVIDENCE_EPSILON/);
  ...
  const executionManager = await readFile(new URL("../integrations/stardew/ExecutionManager.cs", import.meta.url), "utf8");
  ...
  const artifactDiscoveryStart = executionManager.indexOf("private static IReadOnlyList<BridgeArtifactSpotTarget> DiscoverArtifactSpotTargets");
  const artifactDiscovery = executionManager.slice(artifactDiscoveryStart, executionManager.indexOf("private IReadOnlyList<BridgeArtifactSpotResultTarget>", artifactDiscoveryStart));
  ...
  assert.match(execution, /if \(hoeDirtPresentBefore\)\s+return this\.RememberTerminal\(requestId, executionId, ExecutionState\.Rejected, "artifact_spot_hoedirt_present_before"/);
  assert.match(execution, /float staminaBefore = Game1\.player\.Stamina;[\s\S]*hoe\.DoFunction\(location, targetX \* 64 \+ 32, targetY \* 64 \+ 32, 1, Game1\.player\);[\s\S]*Game1\.player\.lastClick = Vector2\.Zero;[\s\S]*Game1\.player\.checkForExhaustion\(staminaBefore\);[\s\S]*float staminaAfter = Game1\.player\.Stamina;/);
...
test("native-local place-crab-pot fixture discovers one exact native target and never performs placement", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf('if (fixture.FixtureScenario == "native_plant_seed_v1")', setupStart));
  ...
  assert.doesNotMatch(setup, /player\.Items\.Remove|new CrabPot\(|placementAction|reduceActiveItemByOne|dropObject|createItemDebris|PublishReceipt|receipt/i);
  const helperStart = entry.indexOf("private static (Vector2 TargetTile, Vector2 StandingTile)? FindNativeLocalCrabPotFixtureTarget");
  const helper = entry.slice(helperStart, entry.indexOf("private static Vector2? FindNativeLocalFarmFixtureTile", helperStart));
  ...
  assert.doesNotMatch(warp, /new CrabPot\(|placementAction|reduceActiveItemByOne|PublishReceipt|dropObject|createItemDebris/i);
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-place-crab-pot-fixture-smoke.mjs", import.meta.url), "utf8");
  ...
  const bootstrap = config.slice(bootstrapStart, config.indexOf("private static bool IsObservedFixtureSlot", bootstrapStart));
  ...
  const published = config.slice(publishedStart, config.indexOf("private static readonly IReadOnlySet<string> PublishedFamilies", publishedStart));
  assert.match(published, /place_crab_pot/);
  const productionRunner = await readFile(new URL("./run-stardew-native-local-player-place-crab-pot-smoke.mjs", import.meta.url), "utf8");
...
test("native-local break-rock-source fixture is isolated and cannot perform the hit", async (t) => {
  ...
  const entry = await readFile(new URL("../integrations/stardew/ModEntry.cs", import.meta.url), "utf8");
  const setup = entry.slice(entry.indexOf('fixture.FixtureScenario == "native_break_rock_source_v1"'), entry.indexOf('fixture.FixtureScenario is "native_tree_first_hit_v1" or "native_chop_tree_source_v1"'));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-break-rock-source-smoke.mjs", import.meta.url), "utf8");
...
test("native-local clear-hoedirt fixture establishes only the intact empty ground precondition", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf('fixture.FixtureScenario is "native_tree_first_hit_v1" or "native_chop_tree_source_v1"', setupStart));
  ...
  const protocol = await readFile(new URL("../integrations/stardew/BridgeProtocol.cs", import.meta.url), "utf8");
  assert.match(protocol, /BridgeClearHoeDirtTarget\(string TargetId, string Location, int X, int Y, bool Crop, bool Ground\)/);
  ...
  const validation = session.slice(validationStart, session.indexOf('else if (request.Action == "equip_tool")', validationStart));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-clear-hoedirt-smoke.mjs", import.meta.url), "utf8");
  ...
  assert.match(runner, /else if \(snapshot\.location !== "Farm"\) throw new Error\("clear_hoedirt_route_must_start_at_farmhouse_or_fixture_farm"\);/);
...
test("native-local clear-debris fixture establishes one intact native resource clump only", async (t) => {
  ...
  assert.match(setup, /farm\.addResourceClumpAndRemoveUnderlyingTerrain\(debrisParentSheetIndex, debrisWidth, debrisHeight, debrisTile\)/);
  assert.match(setup, /new Pickaxe\(\)/);
  assert.doesNotMatch(setup, /resourceClumps\.Add|DoFunction|RequestLocalClearDebris|performToolAction|health\.Value\s*=|PublishReceipt/);

  const executionManager = await readFile(new URL("../integrations/stardew/ExecutionManager.cs", import.meta.url), "utf8");
  const protocol = await readFile(new URL("../integrations/stardew/BridgeProtocol.cs", import.meta.url), "utf8");
  assert.match(protocol, /BridgeDebrisTarget\(string TargetId, int Slot, int X, int Y, int ParentSheetIndex, string ToolKind, int RequiredUpgradeLevel, int Health\)/);

  const requestStart = executionManager.indexOf("public LocalExecutionReceipt RequestLocalClearDebris");
  const request = executionManager.slice(requestStart, executionManager.indexOf("private static StardewValley.TerrainFeatures.ResourceClump? FindDebrisTarget", requestStart));
  ...
  const findTarget = executionManager.slice(executionManager.indexOf("private static StardewValley.TerrainFeatures.ResourceClump? FindDebrisTarget"), executionManager.indexOf("private static string BuildDebrisTargetId"));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-clear-debris-smoke.mjs", import.meta.url), "utf8");
...
test("native-local pet-animal fixture establishes only an unpetted native Pet precondition", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf("private static Vector2? FindNativeLocalFarmFixtureTile", setupStart));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-pet-animal-smoke.mjs", import.meta.url), "utf8");
...
test("native-local npc-relationship fixture establishes an unchanged persisted fact and native villager target", async (t) => {
  ...
  assert.match(setup, /actual\.Points != 250 \|\| actual\.TalkedToToday \|\| actual\.GiftsToday != 0 \|\| actual\.GiftsThisWeek != 0/);
  assert.doesNotMatch(setup, /RequestLocalInspectNpcRelationship|PublishReceipt|friendshipData\.Remove/);
  const runner = await readFile(new URL("./run-stardew-native-local-player-npc-relationship-smoke.mjs", import.meta.url), "utf8");
  ...
  assert.match(runner, /accepted\.state !== "accepted" && !\(accepted\.state === "succeeded" && accepted\.reasonCode === "npc_relationship_inspected"\)/);
...
test("native-local tree-first-hit fixture configures its isolated profile and restores managed files", async (t) => {
  ...
  const entry = await readFile(new URL("../integrations/stardew/ModEntry.cs", import.meta.url), "utf8");
  const setup = entry.slice(entry.indexOf('fixture.FixtureScenario is "native_tree_first_hit_v1" or "native_chop_tree_source_v1"'), entry.indexOf('fixture.FixtureScenario == "native_use_item_v1"'));
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-tree-first-hit-smoke.mjs", import.meta.url), "utf8");
  ...
  for (const name of BUNDLE_FILES) assert.equal(await readFile(join(options.modRoot, name), "utf8"), `original-${name}`);
  await assert.rejects(readFile(prepared.backup), { code: "ENOENT" });
  await assert.rejects(readFile(join(options.root, ".stardew-native-local-player-fixture.lock", "transaction.json")), { code: "ENOENT" });
});
...
test("native-local chop-tree-source fixture establishes only the terminal-tree precondition", async (t) => {
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-chop-tree-source-smoke.mjs", import.meta.url), "utf8");
...
test("native-local refill-watering-can fixture remains isolated and pre-action only", async (t) => {
  ...
  const entry = await readFile(new URL("../integrations/stardew/ModEntry.cs", import.meta.url), "utf8");
  const setup = entry.slice(entry.indexOf('fixture.FixtureScenario == "native_refill_watering_can_v1"'), entry.indexOf('fixture.FixtureScenario != "native_water_crop_v1"'));
  ...
  assert.doesNotMatch(setup, /DoFunction|RequestLocalRefillWateringCan|PublishReceipt|WaterLeft = suppliedCan.waterCanMax/);
  const runner = await readFile(new URL("./run-stardew-native-local-player-refill-watering-can-smoke.mjs", import.meta.url), "utf8");
  ...
  assert.match(runner, /receipt\.executionId === accepted\.executionId && receipt\.requestId === accepted\.requestId/);
  ...
  const executionManager = await readFile(new URL("../integrations/stardew/ExecutionManager.cs", import.meta.url), "utf8");
  const discoveryStart = executionManager.indexOf("private static IReadOnlyList<BridgeRefillWateringCanTarget> DiscoverRefillWateringCanTargets");
  const discovery = executionManager.slice(discoveryStart, executionManager.indexOf("BuildRefillWateringCanTargetId", discoveryStart));
...
test("native-local collect-animal-product fixture establishes only the approved pre-attachment state", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf('if (fixture.FixtureScenario == "native_refill_watering_can_v1")', setupStart));
  ...
  assert.doesNotMatch(setup, /BeginUsingTool|DoFunction|RequestLocalCollectAnimalProduct|PublishReceipt|currentProduce\.Value =|addItemToInventory\(produce/);
  const warpedStart = entry.indexOf("private void OnWarped");
  const warped = entry.slice(entry.indexOf('if (this.nativeLocalCollectAnimalProductFixturePending', warpedStart), entry.indexOf('if (this.nativeLocalFeedFixturePending', warpedStart));
  assert.match(warped, /animal\.isAdult\(\) && animal\.currentProduce\.Value == productPending\.ProduceId/);
  assert.doesNotMatch(warped, /BeginUsingTool|DoFunction|RequestLocalCollectAnimalProduct|PublishReceipt|currentProduce\.Value\s*=(?!=)|addItemToInventory/);

  const runner = await readFile(new URL("./run-stardew-native-local-player-collect-animal-product-smoke.mjs", import.meta.url), "utf8");
  assert.match(runner, /\["collect_animal_product"\]/);
  assert.match(runner, /entry\.executionId === accepted\.executionId && entry\.requestId === accepted\.requestId/);
  assert.match(runner, /targets\.sort\(\(left, right\) => left\.y - right\.y \|\| left\.x - right\.x \|\| left\.targetId\.localeCompare\(right\.targetId\)\)/);
...
test("native-local feed-animal fixture is isolated, pre-bridge only, and requires typed navigation evidence", async (t) => {
  ...
  const setup = entry.slice(setupStart, entry.indexOf('fixture.FixtureScenario == "native_refill_watering_can_v1"', setupStart));
  assert.match(setup, /parseDebugInput\("SetupBigFarm", null\)/);
  ...
  const runner = await readFile(new URL("./run-stardew-native-local-player-feed-animal-smoke.mjs", import.meta.url), "utf8");
...
test("native-local fixture topology refusal returns before scenario setup while valid topology remains allowed", async () => {
  ...
  assert.match(method, /if \(Context\.IsMultiplayer[\s\S]*?this\.nativeLocalPlayerFixtureTerminal = true;[\s\S]*?LogLevel\.Error\);\s*return;\s*}\s*this\.TryInitializeNativeLocalPlayerFixtureScenario\(fixture\);/);
  assert.match(method, /}\s*this\.TryInitializeNativeLocalPlayerFixtureScenario\(fixture\);[\s\S]*?\/\/ The shared embodiment initializes only after the native load/);
});
...
test("native-local fixture restore fails closed when a registered backup is tampered", async (t) => {
  ...
  assert.ok(await readFile(join(options.root, ".stardew-native-local-player-fixture.lock", "transaction.json"), "utf8"));
});
