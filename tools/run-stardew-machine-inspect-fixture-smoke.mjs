...
try {
  ...
  const accepted = await execute("inspect_machine", "machine_inspect", {
    x: target.x,
    y: target.y,
    expectedTargetId: target.targetId,
  }, snapshot);
  const terminal = await terminalForRequest(accepted, 5_000);
  if (terminal.state !== "succeeded" || terminal.reasonCode !== "machine_inspected") throw new Error(`machine_inspect_failed:${terminal.reasonCode}`);
...
async function observeActionable() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null) throw new Error("native_local_machine_player_not_actionable");
  if (!Number.isInteger(snapshot.revision) || !Number.isInteger(snapshot.tile?.x) || !Number.isInteger(snapshot.tile?.y)) throw new Error("native_local_machine_snapshot_invalid");
  if (!Array.isArray(snapshot.capabilities) || !Array.isArray(snapshot.warps) || !Array.isArray(snapshot.machineTargets)) throw new Error("native_local_machine_snapshot_facts_missing");
  return snapshot;
}
function requireExactCapabilities(snapshot) {
  if (JSON.stringify([...snapshot.capabilities].sort()) !== JSON.stringify([...EXPECTED_CAPABILITIES].sort())) throw new Error("native_local_machine_capability_not_isolated");
}
function chooseOnlyFreshMachineTarget(snapshot) {
  const targets = validMachineTargets(snapshot);
  if (targets.length !== 1) throw new Error(targets.length === 0 ? "no_fresh_live_machine_target" : "ambiguous_live_machine_targets");
  return targets[0];
}
...
function validMachineTargets(snapshot) {
  return snapshot.machineTargets.filter((target) => Number.isInteger(target?.x) && Number.isInteger(target?.y) && target.x >= 0 && target.y >= 0
    && typeof target.targetId === "string" && target.targetId.length > 0 && typeof target.qualifiedItemId === "string" && target.qualifiedItemId.length > 0
    && typeof target.readyForHarvest === "boolean" && Number.isInteger(target.minutesUntilReady)
    && (target.heldObjectQualifiedItemId == null || typeof target.heldObjectQualifiedItemId === "string")
    && (target.lastInputQualifiedItemId == null || typeof target.lastInputQualifiedItemId === "string") && adjacent(snapshot.tile, target));
}
...
function requireReceiptIdentity(receipt, executionId, requestId) { if (receipt?.executionId !== executionId || receipt?.requestId !== requestId) throw new Error("receipt_identity_mismatch"); return receipt; }
function evidenceMatchesTarget(evidence, target, location) { return evidence.location === location && evidence.target === target.targetId && evidence.tile === `${target.x},${target.y}` && evidence.machine === target.qualifiedItemId && evidence.ready_for_harvest === String(target.readyForHarvest) && evidence.minutes_until_ready === String(target.minutesUntilReady) && evidence.held === (target.heldObjectQualifiedItemId ?? "none") && evidence.last_input === (target.lastInputQualifiedItemId ?? "none"); }
function sameMachine(left, right) { return left.targetId === right.targetId && left.x === right.x && left.y === right.y && left.qualifiedItemId === right.qualifiedItemId && left.readyForHarvest === right.readyForHarvest && left.minutesUntilReady === right.minutesUntilReady && (left.heldObjectQualifiedItemId ?? null) === (right.heldObjectQualifiedItemId ?? null) && (left.lastInputQualifiedItemId ?? null) === (right.lastInputQualifiedItemId ?? null); }
function parseEvidence(evidence) {
  ...
  for (const field of detail.split(";")) {
    ...
    const key = field.slice(0, separator); const value = field.slice(separator + 1);
    if (!/^[a-z][a-z0-9_]{0,63}$/.test(key) || value.length > 512 || Object.hasOwn(result, key)) throw new Error("invalid_machine_inspect_evidence");
    result[key] = value;
  }
  const expected = ["held", "last_input", "location", "machine", "minutes_until_ready", "ready_for_harvest", "target", "tile"];
  if (JSON.stringify(Object.keys(result).sort()) !== JSON.stringify(expected)) throw new Error("invalid_machine_inspect_evidence");
  return result;
}
function validateNativeLocalConfig(value) {
  ...
  if (fixture?.Enable !== true || fixture.Bootstrap?.Enable === true || fixture.FixtureScenario !== SCENARIO
    || typeof fixture.LogicalSaveName !== "string" || !/^GameBuddyFixture[A-Za-z0-9]{0,64}$/.test(fixture.LogicalSaveName)
    || typeof fixture.ObservedSaveSlot !== "string" || !new RegExp(`^${fixture.LogicalSaveName}_[0-9]{1,32}$`).test(fixture.ObservedSaveSlot)) throw new Error("native_local_fixture_config_invalid");
  ...
  if (value.ActionPolicyVersion !== 0 || JSON.stringify(value.EnabledActions) !== JSON.stringify(["move_to_tile", "machine_inspect"])) throw new Error("native_local_machine_action_policy_invalid");
...
function targetSummary(target) { return target ? { targetId: target.targetId, x: target.x, y: target.y, qualifiedItemId: target.qualifiedItemId, readyForHarvest: target.readyForHarvest, minutesUntilReady: target.minutesUntilReady, heldObjectQualifiedItemId: target.heldObjectQualifiedItemId ?? null, lastInputQualifiedItemId: target.lastInputQualifiedItemId ?? null } : null; }
...
function snapshotSummary(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, machineTargets: snapshot.machineTargets?.map(targetSummary) ?? [], activeExecution: snapshot.activeExecution ?? null }; }
