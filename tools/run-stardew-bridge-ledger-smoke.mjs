...
try {
  ...
  if (!client.state.capabilities.includes("equip_tool") || !initial.capabilities.includes("equip_tool"))
  ...
  try {
    ...
    if (!String(error).includes("bridge_rejected:stale_snapshot"))
      failures.push(`unexpected_stale_error:${String(error)}`);
  }
  ...
  try {
    ...
    if (!String(error).includes("bridge_rejected:idempotency_key_conflict"))
      failures.push(`unexpected_conflict_error:${String(error)}`);
  }
