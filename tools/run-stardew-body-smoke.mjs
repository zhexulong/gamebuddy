...
try {
  ...
  const target = process.argv.includes("--target-x") || process.argv.includes("--target-y")
  ...
  if (accepted === undefined || accepted === null || typeof accepted !== "object") throw new Error(`move_response_missing:${JSON.stringify(client.state.latestReceipt)}`);
  ...
  const replacementTarget = { x: clampTile(progress.tile.x <= 9 ? progress.tile.x + 1 : progress.tile.x - 1), y: clampTile(progress.tile.y) };
  ...
  if (replacement === undefined || replacement === null || typeof replacement !== "object") throw new Error(`replacement_response_missing:${JSON.stringify(client.state.latestReceipt)}`);
  trace.push({ phase: "replacement", receipt: summarizeReceipt(replacement), snapshot: summarizeSnapshot(progress), target: replacementTarget });
  ...
  const oldExecutionTerminal = await waitForReceiptEvent(client, accepted.executionId, "superseded_by_new_directive", 8_000);
  ...
  if (execution != null) {
    const cancelled = await client.cancel(execution.requestId, execution.executionId, "body_smoke_cancel");
      if (cancelled === undefined || cancelled === null || typeof cancelled !== "object") throw new Error(`cancel_response_missing:${JSON.stringify(client.state.latestReceipt)}`);
    trace.push({ phase: "cancelled", receipt: summarizeReceipt(cancelled), snapshot: summarizeSnapshot(active) });
    if (cancelled.state !== "cancelled" && cancelled.state !== "invalidated") failures.push(`cancel_not_terminal:${cancelled.state}:${cancelled.reasonCode}`);
...
function clampTile(value) { return Math.max(0, Math.min(1000, Math.round(value))); }
function summarizeSnapshot(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, activeExecution: snapshot.activeExecution == null ? null : { executionId: snapshot.activeExecution.executionId, requestId: snapshot.activeExecution.requestId, state: snapshot.activeExecution.state, reasonCode: snapshot.activeExecution.reasonCode } };
}
function summarizeReceipt(receipt) {
  return { executionId: receipt.executionId, requestId: receipt.requestId, state: receipt.state, reasonCode: receipt.reasonCode, revision: receipt.revision, evidence: receipt.evidence };
}
async function waitForActiveOrProgress(client, revision, timeoutMs) {
  return waitUntil(client, (snapshot) => snapshot.revision > revision && snapshot.activeExecution !== null, timeoutMs, "move_progress_timeout");
}
async function waitForReceiptEvent(_client, executionId, reasonCode, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const receipt = receiptFacts.find((candidate) => candidate.executionId === executionId && candidate.reasonCode === reasonCode);
...
async function waitForTerminalSnapshot(client, revision, timeoutMs) {
  return waitUntil(client, (snapshot) => snapshot.activeExecution == null && snapshot.revision >= revision, timeoutMs, "cancel_terminal_timeout");
}
