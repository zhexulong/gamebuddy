...
const SCOPE = Object.freeze({
  ...
  bindingGeneration: 1,
  bindingHash: computePortfolioBindingHash({ saveId: SAVE_NAME, worldId: "world_01", localPlayerId: "player_01", companionId: "companion_01", bindingGeneration: 1 }),
});
...
function dsm(overrides = {}) {
  const variants = [
    { variantId: MOVE_VARIANT.variantId, actionClass: MOVE_VARIANT.actionClass, domainHash: MOVE_VARIANT.domain.valueHash, provenanceHash: HASH_E, ccmRowId: "ccm_move_01", closureAttestationHash: HASH_A },
    { variantId: SLEEP_VARIANT.variantId, actionClass: SLEEP_VARIANT.actionClass, domainHash: SLEEP_VARIANT.domain.valueHash, provenanceHash: HASH_E, ccmRowId: "ccm_sleep_01", closureAttestationHash: HASH_B },
  ];
  const milestones = [
    ["M1", [MOVE_VARIANT.variantId]], ["M2", [SLEEP_VARIANT.variantId]], ["M3", [MOVE_VARIANT.variantId]], ["M4", [MOVE_VARIANT.variantId]], ["M5", [SLEEP_VARIANT.variantId]],
    ["M6", [SLEEP_VARIANT.variantId]], ["M7", [MOVE_VARIANT.variantId]], ["M8", [MOVE_VARIANT.variantId]], ["M9", [MOVE_VARIANT.variantId]], ["M10", [MOVE_VARIANT.variantId]],
  ].map(([id, requiredVariantIds]) => ({ id, monitorId: `portfolio_${id.toLowerCase()}_native_persisted_v1`, requiredVariantIds }));
...
test("candidate closure manifests are signed, finite, topology-suffixed and never portfolio runs", () => {
  ...
  const tampered = validateCandidateClosureManifest({ ...signed, variant: { ...signed.variant, variantId: "move_to_tile" } }, { signingKey: KEY });
  ...
  const portfolioShaped = validateCandidateClosureManifest({ ...signed, portfolioRunId: "portfolio_run_01" }, { signingKey: KEY });
...
test("candidate and final registry admission expose only exact topology-scoped variants", () => {
  ...
  assert.equal(admitCandidateRegistry({ manifest, signingKey: KEY, requestedVariantId: "travel__single_player_native_companion" }).reasonCode, "portfolio_candidate_registry_variant_not_authorized");
  assert.equal(admitCandidateRegistry({ manifest, signingKey: KEY, requestedDomainHash: HASH_B }).reasonCode, "portfolio_candidate_registry_domain_not_authorized");
  ...
  assert.equal(admitPortfolioRegistry({ dsm: dsm(), ccm: ccm(), dsmSigningKey: KEY, ccmSigningKey: KEY, scope: SCOPE, requestedVariantId: "travel__single_player_native_companion" }).reasonCode, "portfolio_registry_variant_not_closed");
  assert.equal(admitPortfolioRegistry({ dsm: dsm(), ccm: ccm(), dsmSigningKey: KEY, ccmSigningKey: KEY, scope: SCOPE, requestedVariantId: MOVE_VARIANT.variantId, requestedDomainHash: HASH_B }).reasonCode, "portfolio_registry_domain_not_closed");
});
...
test("DSM is frozen only with all ten monitors and locked finite content scope", () => {
  ...
  assert.deepEqual([...PORTFOLIO_MILESTONE_MONITORS.keys()], ["M1", "M2", "M3", "M4", "M5", "M6", "M7", "M8", "M9", "M10"]);
  ...
  const missingMonitor = validatePortfolioDsm({ ...signed, milestones: signed.milestones.slice(0, 9) }, { signingKey: KEY });
...
test("CCM publication requires independent closed candidate rows and contains no receipt/evidence payload", () => {
  ...
  assert.equal(result.valid, true);
  assert.equal(getPortfolioRegistry({ dsm: dsm(), ccm: signed, dsmSigningKey: KEY, ccmSigningKey: KEY }).variants.length, 2);

  const injectedReceipt = validatePortfolioCcm({ ...signed, rows: signed.rows.map((row) => ({ ...row, receiptId: "execution_01" })) }, { signingKey: KEY });
  assert.equal(injectedReceipt.valid, false);
  assert.ok(injectedReceipt.errors.includes("portfolio_ccm_unknown_row_field"));

  const open = validatePortfolioCcm({ ...signed, rows: signed.rows.map((row) => ({ ...row, liveGate: "BLOCKED" })) }, { signingKey: KEY });
  assert.equal(open.valid, false);
  assert.ok(open.errors.includes("portfolio_ccm_row_liveGate_not_closed"));

  const wrongBinding = validatePortfolioCcm({ ...signed, rows: signed.rows.map((row) => ({ ...row, bindingGeneration: 2 })) }, { signingKey: KEY, scope: SCOPE });
...
test("candidate receipts cannot advance a portfolio monitor; final admission requires same run, closure, evidence, postcondition and fresh native fields", () => {
  ...
  const rejected = admitPortfolioMonitorReceipt({ milestoneId: "M1", dsm: finalDsm, ccm: finalCcm, dsmSigningKey: KEY, ccmSigningKey: KEY, currentScope: SCOPE, receipt: candidate, postcondition: postcondition(), observation: freshObservation() });
  assert.equal(rejected.state, "REJECTED");
  assert.equal(rejected.reasonCode, "portfolio_monitor_candidate_receipt_forbidden");

  const accepted = admitPortfolioMonitorReceipt({ milestoneId: "M1", dsm: finalDsm, ccm: finalCcm, dsmSigningKey: KEY, ccmSigningKey: KEY, currentScope: SCOPE, receipt: receipt({ manifestHash: finalDsm.manifestHash }), postcondition: postcondition(), observation: freshObservation() });
  assert.equal(accepted.state, "ACCEPTED", JSON.stringify(accepted));
  assert.equal(accepted.executionId, "execution_01");

  const stale = admitPortfolioMonitorReceipt({ milestoneId: "M1", dsm: finalDsm, ccm: finalCcm, dsmSigningKey: KEY, ccmSigningKey: KEY, currentScope: SCOPE, receipt: receipt({ manifestHash: finalDsm.manifestHash }), postcondition: postcondition(), observation: freshObservation({ capturedAfterExecutionId: "other_execution" }) });
  assert.equal(stale.state, "REJECTED");
  assert.equal(stale.reasonCode, "portfolio_monitor_observation_not_after_receipt");

  const reboundScope = { ...SCOPE, bindingGeneration: 2, bindingHash: computePortfolioBindingHash({ ...SCOPE, bindingGeneration: 2 }) };
  const rebound = admitPortfolioMonitorReceipt({ milestoneId: "M1", dsm: finalDsm, ccm: finalCcm, dsmSigningKey: KEY, ccmSigningKey: KEY, currentScope: reboundScope, receipt: receipt({ manifestHash: finalDsm.manifestHash }), postcondition: postcondition(), observation: freshObservation() });
...
test("candidate closure receipts and Portfolio checkpoints remain contract-specific", () => {
  ...
  const reboundCheckpoint = admitPortfolioCheckpoint({
    ...
    dsmSigningKey: KEY,
    currentScope: { ...SCOPE, bindingGeneration: 2, bindingHash: computePortfolioBindingHash({ ...SCOPE, bindingGeneration: 2 }) },
  });
  assert.equal(reboundCheckpoint.state, "REJECTED");
  assert.equal(reboundCheckpoint.reasonCode, "portfolio_checkpoint_dsm_invalid");
  assert.equal(admitPortfolioCheckpoint({ dsm: dsm(), checkpoint: checkpoint(), dsmSigningKey: KEY }).reasonCode, "portfolio_checkpoint_current_scope_required");
});
...
test("authoritative completion requires same-execution succeeded receipt, evidence and fresh action postcondition", () => {
  ...
  const candidate = receipt({ contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: candidateManifest().manifestHash });
  assert.equal(evaluateAuthoritativeCompletion({ receipt: candidate, postcondition: postcondition() }).reasonCode, "portfolio_authoritative_completion_candidate_receipt_forbidden");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: { ...receipt(), contractKind: "not_a_contract" }, postcondition: postcondition() }).state, "REJECTED");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: receipt(), postcondition: postcondition({ observedRevision: 1 }) }).state, "REJECTED");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: receipt({ state: "accepted" }), postcondition: postcondition() }).state, "REJECTED");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: receipt({ evidence: null }), postcondition: postcondition() }).state, "REJECTED");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: receipt(), postcondition: postcondition({ executionId: "other_execution" }) }).state, "REJECTED");
  assert.equal(evaluateAuthoritativeCompletion({ receipt: receipt({ topology: "native_ai_farmhand_multiplayer" }), postcondition: postcondition() }).state, "REJECTED");
});

test("receipt and checkpoint contracts reject Farmhand/cross-run data and preserve exact required fields", () => {
  assert.equal(validatePortfolioReceipt(receipt(), { contractKind: "portfolio_run", contractId: "portfolio_run_01", manifestHash: receipt().manifestHash, scope: SCOPE }).valid, true);
  ...
  const candidateWithDsm = validatePortfolioReceipt({ ...receipt(), contractKind: "candidate_closure", contractId: "candidate_01", manifestHash: receipt().manifestHash }, { contractKind: "candidate_closure", contractId: "candidate_01", manifestHash: receipt().manifestHash, scope: SCOPE });
  ...
  assert.equal(validatePortfolioCheckpoint(checkpoint(), { contractKind: "portfolio_run", contractId: "portfolio_run_01", manifestHash: checkpoint().manifestHash, scope: SCOPE }).valid, true);
...
test("durable ledger keeps candidate and portfolio namespaces physically separate and survives reopen", async (t) => {
  ...
  const finalLedger = await createPortfolioExecutionLedger({ dataRoot: root, contractKind: "portfolio_run", contractId: "portfolio_run_01", manifestHash: finalDsm.manifestHash, scope: SCOPE });
  const candidateLedger = await createPortfolioExecutionLedger({ dataRoot: root, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: candidateManifest().manifestHash, scope: SCOPE, variantId: MOVE_VARIANT.variantId });
  await assert.rejects(
    () => createPortfolioExecutionLedger({ dataRoot: root, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: candidateManifest().manifestHash, scope: SCOPE, variantId: SLEEP_VARIANT.variantId }),
  ...
  const parallelReceipt = (requestId, executionId) => receipt({
    requestId,
    executionId,
    evidence: { schemaRevision: PORTFOLIO_EVIDENCE_SCHEMA_REVISION, executionId, evidenceHash: HASH_E },
  });
  const parallelA = parallelReceipt("request_parallel_01", "execution_parallel_a");
  const parallelB = parallelReceipt("request_parallel_01", "execution_parallel_b");
  const conflictingWrites = await Promise.allSettled([finalLedger.appendReceipt(parallelA), finalLedger.appendReceipt(parallelB)]);
  assert.equal(conflictingWrites.filter((outcome) => outcome.status === "fulfilled" && outcome.value.state === "written").length, 1);
  assert.equal(conflictingWrites.filter((outcome) => outcome.status === "rejected" && /portfolio_receipt_replay_conflict/.test(outcome.reason.message)).length, 1);
  const winningReceipt = conflictingWrites[0].status === "fulfilled" ? parallelA : parallelB;
  assert.deepEqual(await finalLedger.readReceipt("request_parallel_01"), winningReceipt);

  const identicalParallelReceipt = parallelReceipt("request_parallel_same_01", "execution_parallel_same_01");
  const identicalWrites = await Promise.all([finalLedger.appendReceipt(identicalParallelReceipt), finalLedger.appendReceipt(identicalParallelReceipt)]);
  assert.deepEqual(new Set(identicalWrites.map((outcome) => outcome.state)), new Set(["written", "replayed"]));
  await assert.rejects(() => candidateLedger.appendReceipt(finalReceipt), /portfolio_receipt_contract_kind_mismatch/);

  const reopened = await createPortfolioExecutionLedger({ dataRoot: root, contractKind: "portfolio_run", contractId: "portfolio_run_01", manifestHash: finalDsm.manifestHash, scope: SCOPE });
  ...
  await assert.rejects(
    () => createPortfolioExecutionLedger({ dataRoot: root, contractKind: "portfolio_run", contractId: "portfolio_run_01", manifestHash: HASH_A, scope: SCOPE }),
...
test("contract protocol is a separate schema-only seam with exact contract/run identity", () => {
  const scope = { ...SCOPE };
  const envelope = createPortfolioContractEnvelope("candidate_execution_request", {
    topology: PORTFOLIO_TOPOLOGY,
    contractKind: "candidate_closure",
    contractId: "candidate_move_01",
    manifestHash: candidateManifest().manifestHash,
    bindingHash: SCOPE.bindingHash,
    bindingGeneration: SCOPE.bindingGeneration,
  }, { actionVariant: MOVE_VARIANT.variantId, target: { id: "target_01" }, deadlineMs: Date.now() + 10_000, idempotencyKey: "request_01" }, "request_01");
  assert.equal(envelope.protocolVersion, PORTFOLIO_CONTRACT_PROTOCOL_VERSION);
  assert.equal(validatePortfolioContractEnvelope(envelope, { ...scope, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: envelope.contract.manifestHash }).valid, true);
  const wrongRun = validatePortfolioContractEnvelope({ ...envelope, contract: { ...envelope.contract, contractId: "portfolio_run_01" } }, { ...scope, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: envelope.contract.manifestHash });
  assert.equal(wrongRun.valid, false);
  assert.ok(wrongRun.errors.includes("portfolio_contract_identity_mismatch"));
  assert.equal(validatePortfolioContractEnvelope({ ...envelope, type: "execution_request" }, { ...scope, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: envelope.contract.manifestHash }).valid, false);
  assert.ok(validatePortfolioContractEnvelope({ ...envelope, type: "candidate_checkpoint_request", payload: { checkpointId: "checkpoint_01", monitorId: "portfolio_m1_native_persisted_v1", idempotencyKey: "request_01" } }, { ...scope, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: envelope.contract.manifestHash }).errors.includes("portfolio_candidate_checkpoint_request_forbidden"));
  assert.ok(validatePortfolioContractEnvelope({ ...envelope, contract: { ...envelope.contract, contractId: "portfolio_run_01" } }, { ...scope, contractKind: "candidate_closure", contractId: "candidate_move_01", manifestHash: envelope.contract.manifestHash }).errors.includes("portfolio_contract_identity_candidate_run_id_invalid"));
});
