...
try {
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_item_target", snapshot: summarize(before), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    if (accepted.state !== "accepted") throw new Error(`pickup_item_not_accepted:${accepted.state}:${accepted.reasonCode}`);
    ...
    const evidenceMatches = evidence.location === before.location
    ...
    const passed = receipt.state === "succeeded" && receipt.reasonCode === "item_picked_up" && evidenceMatches && !stillPresent;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "item_picked_up" : receipt.reasonCode, target, receipt: summarizeReceipt(receipt), evidence, inventoryDelta, before: summarize(before), after: summarize(after), durationMs: Date.now() - startedAt }));
...
async function waitForTerminalReceipt(client, receipts, executionId, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    try { await client.observe(); } catch { }
  }
...
function chooseTarget(snapshot) {
  ...
  return snapshot.itemTargets.find((entry) => typeof entry.targetId === "string" && Number.isInteger(entry.x) && Number.isInteger(entry.y) && typeof entry.qualifiedItemId === "string" && Number.isSafeInteger(entry.stack) && entry.stack > 0) ?? null;
}
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, itemTargets: snapshot.itemTargets?.length ?? 0, inventorySlots: snapshot.inventorySlots ?? null };
}
