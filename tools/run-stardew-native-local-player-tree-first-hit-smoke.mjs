...
function validateConfig(value) {
  ...
  if (
    ...
    fixture?.FixtureScenario !== "native_tree_first_hit_v1"
  ) {
  ...
  if (
    ...
    !same(value.EnabledActions, ["move_to_tile", "travel", "equip_tool", "tree_first_hit"])
  ) {
...
async function travelToFarm(snapshot) {
  ...
  const completion = await waitForReceiptAndSnapshot(
...
function chooseTree(snapshot) {
  ...
  if (targets.length !== 1)
    throw new Error(targets.length ? "ambiguous_live_full_health_tree_target" : "no_live_full_health_tree_target");
  return targets[0];
}

function findSameTree(snapshot, target) {
  return (snapshot.treeShakeSourceTargets ?? []).find(
    (entry) => entry?.targetId === target.targetId && entry.x === target.x && entry.y === target.y && validTree(entry),
  );
}
