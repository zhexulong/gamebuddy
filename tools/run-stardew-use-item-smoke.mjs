...
try {
  ...
  const target = Array.isArray(snapshot.foodTargets)
    ? snapshot.foodTargets.find((entry) => Number.isInteger(entry.slot) && typeof entry.qualifiedItemId === "string" && entry.stack > 0 && entry.edibility >= -299) ?? null
    : null;
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_food_target", snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    const passed = receipt.state === "succeeded"
      ...
      && (stackAfter === 0
        ? after.foodTargets?.some((entry) => entry.slot === target.slot && entry.qualifiedItemId === target.qualifiedItemId) !== true
        : after.foodTargets?.some((entry) => entry.slot === target.slot && entry.qualifiedItemId === target.qualifiedItemId && entry.stack === stackAfter) === true);
  ...
  const reasonCode = error instanceof FormalActionGateError ? error.reasonCode : String(error instanceof Error ? error.message : error).slice(0, 256);
  console.error(JSON.stringify({
    state: "blocked",
    reasonCode,
    failure: classifyStardewGateFailure(reasonCode, { nativeReasonCode: client.state.latestReceipt?.reasonCode ?? null }),
...
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, stamina: snapshot.stamina, health: snapshot.health, foodTargets: snapshot.foodTargets?.length ?? 0 };
}
function summarizeTarget(target) {
  return { slot: target.slot, qualifiedItemId: target.qualifiedItemId, stack: target.stack, edibility: target.edibility, isDrink: target.isDrink };
}
