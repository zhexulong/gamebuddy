...
test("fixture-backed descriptor coverage is explicit rather than inferred", () => {
  ...
  assert.deepEqual(fixtureBacked.map((gate) => gate.actionId), [
    ...
    "machine_inspect", "machine_load", "machine_collect_output", "collect_animal_product", "feed_animal", "use_item", "harvest_crop",
    "refill_watering_can", "break_rock_source", "clear_hoedirt", "dig_artifact_spot", "chop_tree_source", "place_wood_fence", "place_crab_pot",
  ]);
  assert.deepEqual(
