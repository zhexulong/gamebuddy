...
const root = resolve(".");
const script = resolve("tools/check-stardew-portfolio-m3-forage-source-audit.mjs");
function run() {
...
test("M3 source-audit CLI revalidates authority, provenance metadata, and its bounded anchors", async () => {
  const result = await run();
  assert.equal(result.code, 0, result.stderr);
  const output = JSON.parse(result.stdout);
  assert.equal(output.anchorCount, 6);
