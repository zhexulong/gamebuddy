...
import { STARDEW_ACTION_ADAPTERS, STARDEW_ACTION_TOOL_NAMES } from "../host/dist-test/action-registry.js";
import {
  consumeDeferredMultiSourceReceiptClaim,
  runTopologyPreflight,
} from "./stardew-navigation-topology-preflight.mjs";
import { replayNavigationOperation } from "./replay-stardew-navigation-operation.mjs";
...
function hasOnlyProductionData(options) {
  ...
  for (const name of ["gamePath", "releaseDir", "transitionArtifact", "multiSourceTransitionArtifact", "navigationSrcDir", "integrationSrcDir", "replayFramesPath", "requestedNavigationScope"]) {
    if (options[name] !== undefined && typeof options[name] !== "string") return false;
  }
  return options.replayFrames === undefined || Array.isArray(options.replayFrames) && isJsonData(options.replayFrames);
}
...
export async function runAgentLivePreflight(options = {}) {
  if (!hasOnlyProductionData(options)) {
    return blockedReport([item("input", "blocked", "invalid input")]);
  }
  const items = [];
  ...
  let topology = null;
  try {
    topology = await runTopologyPreflight({
...
async function readText(filePath) {
  ...
  const navigateAdapter = STARDEW_ACTION_ADAPTERS.find((a) => a.actionId === "navigate_to_destination");
  const toolNameOk = STARDEW_ACTION_TOOL_NAMES.navigate_to_destination === NAVIGATE_TOOL_NAME;
  ...
  if (Array.isArray(replayFrames)) {
    ...
    try {
      replayReport = await replayNavigationOperation(replayFrames, replayOpts);
    } catch {
      replayReport = null;
  ...
  for (const rel of FIXTURE_PATH_REQUIREMENTS) {
    if (!(await defaultExists(TOOLS_DIR + rel))) fixtureMissing.push(rel);
  }
  let fixtureLibOk = true;
  if (await defaultExists(TOOLS_DIR + "lib/stardew-native-local-player-fixture.mjs")) {
  ...
  try {
    const entries = await defaultReadNavigationDirectory(navSrcDir);
  ...
  for (const name of [...EXECUTION_PATH_FILES, ...navigationSourceNames]) {
    ...
    try {
      const text = await defaultReadNavigationSource(dir, name);
  ...
  if (requestedNavigationScope === "multi_hop_ordinary_warp") {
    ...
    const consumed = await consumeDeferredMultiSourceReceiptClaim(topology);
    items.push(item("multi_source_receipt", consumed ? "ready" : "blocked", consumed ? "multi-source receipt consumed" : "receipt consumption failed"));
    if (!consumed) return blockedReport(items);
  }
  return readyReport(items);
}
function bindCli(args, name) {
...
async function cliMain(args) {
  const report = await runAgentLivePreflight({
    gamePath: bindCli(args, "--game-path"),
    releaseDir: bindCli(args, "--release-dir"),
    transitionArtifact: bindCli(args, "--transition-artifact"),
    multiSourceTransitionArtifact: bindCli(args, "--multi-source-artifact"),
    navigationSrcDir: bindCli(args, "--navigation-src-dir"),
    integrationSrcDir: bindCli(args, "--integration-src-dir"),
    replayFramesPath: bindCli(args, "--replay-frames"),
    requestedNavigationScope: bindCli(args, "--requested-navigation-scope"),
  });
  console.log(JSON.stringify({ state: report.state, topology: report.topology, items: report.items, mutationCount: report.mutationCount, executionReceiptCount: report.executionReceiptCount }));
