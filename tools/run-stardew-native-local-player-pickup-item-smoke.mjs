...
async function observeActionable() {
  return requireActionableSnapshot(await client.observe());
}
async function waitForFreshActionablePostcondition(timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (String(error instanceof Error ? error.message : error) !== "native_local_pickup_item_player_not_actionable")
        throw error;
      await delay(100);
    }
  }
...
function requireActionableSnapshot(snapshot) {
  if (!snapshot.actionable || snapshot.activeExecution != null)
    throw new Error("native_local_pickup_item_player_not_actionable");
  if (
...
function requireExactCapabilities(snapshot) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected))
    throw new Error("native_local_pickup_item_capability_not_isolated");
}
...
async function waitForTerminal(executionId, requestId, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const receipt = receipts.find(
      (entry) => entry.executionId === executionId && entry.requestId === requestId && isTerminal(entry.state),
    );
