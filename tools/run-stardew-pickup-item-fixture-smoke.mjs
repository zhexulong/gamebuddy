...
try {
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_item_target", trace, snapshot: summarize(await client.observe()), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    const receipt = await client.execute({
      ...
      action: "pickup_item",
      args: { x: refreshed.x, y: refreshed.y, expectedQualifiedItemId: refreshed.qualifiedItemId, expectedTargetId: refreshed.targetId },
    ...
    const inventoryDelta = Number.isSafeInteger(inventoryBefore) && Number.isSafeInteger(inventoryAfter) ? inventoryAfter - inventoryBefore : null;
    const evidenceMatches = evidence.location === snapshot.location
    ...
    const passed = terminalReceipt.state === "succeeded" && terminalReceipt.reasonCode === "item_picked_up" && evidenceMatches && targetGone;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "item_picked_up" : terminalReceipt.reasonCode, target: refreshed, receipt: summarizeReceipt(terminalReceipt), evidence, inventoryDelta, targetGone, trace, before: summarize(snapshot), after: summarize(after), durationMs: Date.now() - startedAt }));
...
async function travelToFarm(snapshot) {
  ...
  const receipt = await client.execute({ requestId: `item_fixture_travel_${Date.now()}`, idempotencyKey: `item_fixture_travel_idem_${Date.now()}`, action: "travel", args: { x: warp.sourceX, y: warp.sourceY }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
...
async function waitForActionable(snapshot, timeoutMs) { const deadline = Date.now() + timeoutMs; let latest = snapshot; while (Date.now() < deadline) { if (latest.actionable && latest.activeExecution == null) return latest; await delay(150); latest = await client.observe(); } return latest; }
async function waitForReceiptAndSnapshot(executionId, predicate, timeoutMs) { const deadline = Date.now() + timeoutMs; let latest = await client.observe(); while (Date.now() < deadline) { const receipt = receipts.find((candidate) => candidate.executionId === executionId && isTerminal(candidate.state)); if (receipt && receipt.state !== "succeeded") throw new Error(`navigation_failed:${receipt.reasonCode}`); if (receipt?.state === "succeeded" && predicate(latest)) return latest; await delay(200); latest = await client.observe(); } throw new Error(`navigation_timeout:${executionId}`); }
async function waitForTerminalReceiptAndSnapshot(executionId, timeoutMs) { const deadline = Date.now() + timeoutMs; let latest = await client.observe(); while (Date.now() < deadline) { const receipt = receipts.find((candidate) => candidate.executionId === executionId && isTerminal(candidate.state)); if (receipt) return { receipt, snapshot: latest }; await delay(200); latest = await client.observe(); } throw new Error(`pickup_item_terminal_timeout:${executionId}`); }
function chooseTarget(snapshot) { return snapshot.itemTargets?.find((entry) => typeof entry.targetId === "string" && Number.isInteger(entry.x) && Number.isInteger(entry.y) && typeof entry.qualifiedItemId === "string" && Number.isInteger(entry.stack) && entry.stack > 0) ?? null; }
...
function summarize(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, itemTargets: snapshot.itemTargets?.length ?? 0, inventorySlots: snapshot.inventorySlots ?? null, activeExecution: snapshot.activeExecution ?? null }; }
