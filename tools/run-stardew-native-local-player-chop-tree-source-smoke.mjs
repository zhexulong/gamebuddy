...
try {
  ...
  if (accepted.state !== "succeeded" || accepted.reasonCode !== "tree_source_chopped")
    throw new Error(`chop_tree_source_failed:${accepted.reasonCode}`);
...
function validateConfig(value) {
  ...
  if (
    ...
    fixture.FixtureScenario !== "native_chop_tree_source_v1" ||
    value.ActionPolicyVersion !== 0 ||
    !same(value.EnabledActions, ["move_to_tile", "travel", "equip_tool", "chop_tree_source"])
  )
    throw new Error("native_local_chop_tree_source_fixture_config_invalid");
  if (
    ...
    throw new Error("native_local_chop_tree_source_topology_invalid");
}
function requireCapabilities(snapshot) {
  if (
    !same(
      [...(snapshot.capabilities ?? [])].sort(),
      ["cancel_active_execution", "chop_tree_source", "equip_tool", "inspect_self", "move_to_tile", "travel"].sort(),
    )
  )
    throw new Error("native_local_chop_tree_source_capability_not_isolated");
}
async function actionableSnapshot() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null || !Number.isInteger(snapshot.revision))
    throw new Error("native_local_chop_tree_source_snapshot_not_actionable");
  return snapshot;
}
async function execute(phase, action, args, snapshot) {
  ...
  const requestId = `native_local_chop_tree_source_${phase}_${Date.now()}_${trace.length}`;
  ...
  for (let radius = 1; radius <= 12; radius++)
    for (let dx = -radius; dx <= radius; dx++)
      for (let dy = -radius; dy <= radius; dy++) {
        ...
        try {
          snapshot = await move(snapshot, target, "move_to_chop_tree_fixture");
          if (chopTreeCandidates(snapshot).length === 1) return snapshot;
        } catch (error) {
          const reason = String(error instanceof Error ? error.message : error);
          if (!reason.includes("no_native_path")) throw error;
          snapshot = await actionableSnapshot();
          if (chopTreeCandidates(snapshot).length === 1) return snapshot;
        }
      }
...
function summary(snapshot) {
  return {
    ...
    treeChopSourceTargets: snapshot.treeChopSourceTargets?.length ?? 0,
    treeChopResultTargets: snapshot.treeChopResultTargets?.length ?? 0,
