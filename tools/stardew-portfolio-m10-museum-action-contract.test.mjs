...
function mutate(source, fn) {
  return load(source).then((value) => {
    fn(value);
    return value;
  });
}

test("M10 production loader ignores a third-argument injected reader and preserves truthful blocked output", async () => {
  const contract = await load(CONTRACT);
  const fixture = await load(FIXTURE);
  let injectedReaderCalled = false;
  const injectedReader = async () => {
    injectedReaderCalled = true;
    throw new Error("injected reader must not be called");
  };
  assert.doesNotThrow(() => validateMuseumActionContract(contract));
  assert.doesNotThrow(() => validateMuseumFixtureContract(fixture));
  assert.deepEqual(
    await checkMuseumActionContract(CONTRACT, path.resolve("."), {
      fs: { readFile: injectedReader, lstat: injectedReader, stat: injectedReader },
      platform: "win32",
      execPath: injectedReader,
    }),
    {
      ...
      fixture: {
        fixtureId: "portfolio_m10_museum_nonterminal_v1",
        state: "fixture_needed",
        provisioningState: "unprovisioned",
        liveClosure: "none",
      },
      sourceAudit: {
        auditId: "portfolio_m10_museum_source_audit_v1",
        anchorCount: 8,
...
test("fresh opaque finite-domain and exact-field boundaries fail closed", async () => {
  const invalidPiece = await mutate(CONTRACT, (value) => {
    value.pieceDomain.values = ["(O)80"];
  });
  assert.throws(() => validateMuseumActionContract(invalidPiece), /values must stay empty/);
  const fakeTarget = await mutate(CONTRACT, (value) => {
    value.actions[0].target.piece.value = "(O)80";
  });
  assert.throws(() => validateMuseumActionContract(fakeTarget), /unmaterialized fresh opaque/);
  const extra = await mutate(CONTRACT, (value) => {
...
test("inventory, collection, reward and forbidden fallback distinctions cannot be weakened", async () => {
  const inventory = await mutate(CONTRACT, (value) => {
    value.actions[0].freshPostcondition.inventoryDelta = "unchanged";
  });
  assert.throws(() => validateMuseumActionContract(inventory), /postcondition must prove inventory/);
  const reward = await mutate(CONTRACT, (value) => {
    value.actions[1].commit.collectionEffect = "one_piece_added";
  });
  assert.throws(() => validateMuseumActionContract(reward), /separate from donation/);
  const ui = await mutate(CONTRACT, (value) => {
    value.forbiddenBehavior.pop();
  });
...
test("fixture rejects provisioning, terminal facts, and result artifacts", async () => {
  const provisioned = await mutate(FIXTURE, (value) => {
    value.status.provisioningState = "provisioned";
  });
  assert.throws(() => validateMuseumFixtureContract(provisioned), /unprovisioned/);
  const terminal = await mutate(FIXTURE, (value) => {
    value.startingState.terminalFacts = "reward_eligible";
  });
  assert.throws(() => validateMuseumFixtureContract(terminal), /terminal/);
  const result = await mutate(FIXTURE, (value) => {
    value.receipt = "succeeded";
  });
...
test("CLI contract path is explicit and malformed/missing contract fails closed", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => checkMuseumActionContract(path.join(directory, "missing.json")),
      /Unable to read or parse M10 action contract/,
    );
  } finally {
