...
try {
  ...
  const targets = snapshot.debrisTargets.map((target) => targetSummary(target, matchingTool(target, snapshot.toolSlots)));
  if (targets.length === 0) {
...
function validateNativeLocalConfig(value) {
  ...
  if (value.NativeLocalPlayerFixture.Bootstrap?.Enable === true) throw new Error("native_local_fixture_bootstrap_enabled");
...
function requireDiscoverySnapshot(snapshot) {
  if (!snapshot.actionable || snapshot.activeExecution != null) throw new Error("native_local_clear_debris_player_not_actionable");
  if (!Number.isInteger(snapshot.revision) || !Number.isInteger(snapshot.tile?.x) || !Number.isInteger(snapshot.tile?.y)) throw new Error("native_local_clear_debris_snapshot_invalid");
  if (!Array.isArray(snapshot.capabilities) || !Array.isArray(snapshot.debrisTargets) || !Array.isArray(snapshot.toolSlots)) throw new Error("native_local_clear_debris_snapshot_facts_missing");
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error("native_local_clear_debris_capability_not_isolated");
  if (!snapshot.debrisTargets.every(validTarget) || !snapshot.toolSlots.every(validToolSlot)) throw new Error("native_local_clear_debris_snapshot_facts_invalid");
...
function validTarget(target) {
  return typeof target?.targetId === "string" && target.targetId.length > 0
    && Number.isInteger(target.slot) && target.slot >= 0
    && Number.isInteger(target.x) && target.x >= 0 && Number.isInteger(target.y) && target.y >= 0
    && Number.isInteger(target.parentSheetIndex) && target.parentSheetIndex >= 0
    && (target.toolKind === "axe" || target.toolKind === "pickaxe")
    && Number.isInteger(target.requiredUpgradeLevel) && target.requiredUpgradeLevel >= 0;
}
function validToolSlot(tool) { return Number.isInteger(tool?.slot) && tool.slot >= 0 && typeof tool.label === "string" && tool.label.length > 0; }
