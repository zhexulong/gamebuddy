...
try {
  ...
  if (!snapshot.capabilities.includes("machine_inspect")) throw new Error("snapshot_machine_inspect_capability_missing");
  const target = Array.isArray(snapshot.machineTargets) ? (snapshot.machineTargets.find((entry) =>
    typeof entry.targetId === "string" && Number.isInteger(entry.x) && Number.isInteger(entry.y)) ?? null) : null;
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_machine_target", snapshot: summarize(snapshot) }));
    process.exitCode = 2;
  } else {
    ...
    const reread = Array.isArray(after.machineTargets)
      ...
      : null;
    const sameLiveState = reread !== null
      && reread.x === target.x
      && reread.y === target.y
    ...
    const passed = receipt.state === "succeeded"
      ...
      && evidenceMatches
      && sameLiveState;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "machine_inspected" : receipt.reasonCode, target: summarizeTarget(target), receipt: summarizeReceipt(receipt), evidence, reread: summarizeTarget(reread), sameLiveState, after: summarize(after) }));
...
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, machineTargets: snapshot.machineTargets?.length ?? 0 };
}
...
function summarizeTarget(target) {
  ...
  return { targetId: target.targetId, x: target.x, y: target.y, qualifiedItemId: target.qualifiedItemId, readyForHarvest: target.readyForHarvest, minutesUntilReady: target.minutesUntilReady, heldObjectQualifiedItemId: target.heldObjectQualifiedItemId ?? null, lastInputQualifiedItemId: target.lastInputQualifiedItemId ?? null };
}
