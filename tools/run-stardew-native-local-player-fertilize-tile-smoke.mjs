...
try {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null)
    throw new Error("player_not_actionable_before_fertilize");
  ...
  const sameExecution =
    typeof receipt.executionId === "string" && receipt.executionId.length > 0 && receipt.requestId === requestId;
  ...
  console.log(
    JSON.stringify({
      ...
      reasonCode: passed ? "fertilizer_applied" : receipt.reasonCode,
      target,
...
async function execute(
  ...
  args,
  snapshot,
  requestId = `native_local_fertilize_tile_${phase}_${Date.now()}`,
) {
  if (!snapshot.actionable || snapshot.activeExecution != null) throw new Error(`${phase}_player_not_actionable`);
  const receipt = await client.execute({
    requestId,
    idempotencyKey: `native_local_fertilize_tile_${phase}_idem_${Date.now()}`,
    action,
    args,
...
async function waitForReceiptAndSnapshot(executionId, requestId, predicate, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (terminal?.state === "succeeded" && !hasNonemptyEvidence(terminal))
      throw new Error("navigation_succeeded_evidence_missing");
...
function requireExactCapabilities(snapshot) {
  ...
  const expected = ["cancel_active_execution", "fertilize_tile", "inspect_self", "move_to_tile", "travel"].sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected))
    throw new Error("native_local_fertilize_capability_not_isolated");
}
...
function summarize(snapshot) {
  return {
    ...
    fertilizerTargets: snapshot.fertilizerTargets?.length ?? 0,
