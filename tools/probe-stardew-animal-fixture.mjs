...
const client = await withTimeout(LocalStardewBridgeClient.connect({
  integrationId: "stardew",
  saveId: config.SaveId,
  worldId: config.WorldId,
  playerId: config.PlayerId,
  companionId: config.CompanionId,
}, config.PipeName, config.BridgeToken), "bridge_connect");
try {
  ...
  console.log(JSON.stringify({
    state: snapshot.actionable && (snapshot.animalProductTargets?.length ?? 0) > 0 ? "target_ready" : "target_unavailable",
