...
test("fertilize runner uses the shared receipt and fresh-reread mechanics", async () => {
  ...
  const { client, receipts } = actionClient(
    { revision: 1, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, fertilizerTargets: [target] },
    (request) => {
      ...
      return {
        snapshot: { revision: 2, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, fertilizerTargets: [] },
        terminal: {
          state: "succeeded",
          reasonCode: "fertilizer_applied",
          revision: 2,
          evidence: { detail: "location=Farm;target=fertilizer-target;tile=2,1;item=(O)369;fertilizer_before=none;fertilizer_after=(O)369;inventory_before=2;inventory_after=1" },
        },
      };
    },
  );
...
test("plant runner retains typed target and evidence validation with shared mechanics", async () => {
  ...
  const { client, receipts } = actionClient(
    { revision: 1, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, seedTargets: [target] },
    (request) => {
      assert.equal(request.action, "plant_seed");
      assert.equal(request.args.expectedTargetId, target.targetId);
      return {
        snapshot: { revision: 2, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, seedTargets: [] },
        terminal: {
          state: "succeeded",
          reasonCode: "seed_planted",
          revision: 2,
          evidence: { detail: "crop=24;inventory_after=1;inventory_before=2;item=(O)472;location=Farm;target=seed-target;tile=2,1" },
        },
      };
    },
  );
...
test("harvest runner retains target-specific postcondition with shared mechanics", async () => {
  ...
  const target = { x: 2, y: 1, targetId: "harvest-target", qualifiedHarvestItemId: "(O)24", cropId: "24", regrowsAfterHarvest: false };
  const { client, receipts } = actionClient(
    { revision: 1, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, warps: [], harvestTargets: [target], currentTool: null },
    (request) => {
      assert.equal(request.action, "harvest_crop");
      assert.equal(request.args.expectedTargetId, target.targetId);
      return {
        snapshot: { revision: 2, location: "Farm", tile: { x: 1, y: 1 }, actionable: true, activeExecution: null, capabilities, warps: [], harvestTargets: [], currentTool: null },
        terminal: {
          state: "succeeded",
          reasonCode: "crop_harvested",
          revision: 2,
          evidence: { detail: "target=harvest-target;item=(O)24;regrows=false;native_accepted=true;inventory_gained=true;inventory_before=1;inventory_after=2;crop_present_after=false" },
        },
      };
    },
  );
