...
function sameScope(left, right) {
  return exact(left, SCOPE_KEYS) && exact(right, SCOPE_KEYS) && SCOPE_KEYS.every((key) => left[key] === right[key]);
}
...
export function validateM1RouteBlockerFixture(fixture) {
  if (
    ...
    fixture.artifactKind !== "portfolio_route_blocker_fixture" ||
    fixture.fixtureId !== "portfolio_m1_leave_and_return_route_unprovisioned_v1" ||
    fixture.topology !== "single_player_native_companion" ||
    fixture.state !== "unprovisioned" ||
    fixture.mutation !== false ||
    fixture.terminalOutcome !== false ||
    fixture.checkpoint !== null ||
    fixture.receipt !== null ||
    fixture.nativeEvidence !== null
  )
...
export function validateM1RouteBlocker({ request, checkpoint, receipt, expectedScope, nowMs = Date.now() } = {}) {
  if (
    ...
    JSON.stringify(request.composition) !== JSON.stringify(COMPOSITION) ||
    !sameScope(request.scope, expectedScope)
  )
  ...
  if (
    ...
    !ID.test(checkpoint.observationId ?? "") ||
    !ID.test(checkpoint.opaqueCheckpoint ?? "") ||
    checkpoint.revision !== request.expectedRevision ||
    !sameScope(checkpoint.scope, expectedScope) ||
    checkpoint.fresh !== true ||
    checkpoint.playerAvailable !== true ||
    checkpoint.worldReady !== true ||
    checkpoint.policyAllowed !== true
  )
  ...
  if (
    ...
    !ID.test(receipt.executionId ?? "") ||
    receipt.state !== "blocked" ||
    receipt.revision !== checkpoint.revision ||
    receipt.reasonCode !== BLOCKER ||
    receipt.sourceAuditId !== SOURCE_AUDIT ||
    receipt.opaqueCheckpoint !== checkpoint.opaqueCheckpoint
  )
    return blocked("m1_route_blocker_handoff_invalid");
  return Object.freeze({
    state: "BLOCKED",
    code: BLOCKER,
    producer: SOURCE_AUDIT,
    consumer: ACTION,
    verifier: "m1_route_static_preflight",
