...
test("accounts every exact source/content input path without semantic classifications", () => {
  ...
  assert.deepEqual(Object.keys(report).sort(), ["architectureAccountingState", "boundaryRegister", "contentAccounting", "inputAccountingState", "missingRootFamilies", "rootRegister", "sourceAccounting"]);
});
...
test("fails closed on duplicate or unsafe exact paths instead of silently dropping them", () => {
  assert.throws(() => accountExactPaths({
