...
try {
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_resource_clump", snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "debris_cleared" : receipt?.reasonCode ?? "debris_target_unavailable", trace, after: summarize(after), durationMs: Date.now() - startedAt }));
...
function chooseTarget(snapshot, targetId = null) {
  ...
  return snapshot.debrisTargets.find((entry) => (targetId === null || entry.targetId === targetId)
    && Number.isInteger(entry.slot) && Number.isInteger(entry.x) && Number.isInteger(entry.y)
    && typeof entry.targetId === "string") ?? null;
}
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, debrisTargets: snapshot.debrisTargets?.length ?? 0, toolSlots: snapshot.toolSlots?.length ?? 0 };
}
