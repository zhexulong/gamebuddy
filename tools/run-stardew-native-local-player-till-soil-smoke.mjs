...
try {
  ...
  if (snapshot.location !== "Farm" || !snapshot.soilTiles.some((entry) => entry.x === target.x && entry.y === target.y))
  ...
  const freshTargetGone =
    Array.isArray(after.soilTiles) && !after.soilTiles.some((entry) => entry?.x === target.x && entry?.y === target.y);
  const passed =
...
async function travelToFarm(snapshot) {
  ...
  return (
    await waitForReceiptAndSnapshot(
      accepted,
...
async function move(snapshot, target, phase) {
  ...
  if (!snapshot.actionable)
    throw new Error(
      `${phase}_player_not_actionable:location=${snapshot.location};tile=${snapshot.tile?.x},${snapshot.tile?.y};active=${snapshot.activeExecution?.state ?? "none"}`,
    );
...
async function observe() {
  ...
  requireExactCapabilities(snapshot.capabilities, "observe");
  return snapshot;
}
...
async function waitForTillPostcondition(receipt, before, target, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (latest.revision > receipt.revision)
      throw new Error(`till_postcondition_revision_mismatch:${latest.revision}:${receipt.revision}`);
    if (latest.revision === receipt.revision) {
      if (latest.actionable !== true || latest.activeExecution != null)
        throw new Error("till_postcondition_player_not_stable");
      if (latest.location !== before.location || !sameTile(latest.tile, before.tile))
        throw new Error("till_postcondition_player_moved");
...
function findTerminalReceipt(accepted) {
  ...
  return (
    receipts.find(
      (receipt) =>
        receiptMatchesRequest(receipt, accepted.requestId, accepted.executionId) && isTerminal(receipt.state),
    ) ?? null
  );
}
...
function requireExactCapabilities(actual, source) {
  if (!Array.isArray(actual) || JSON.stringify([...actual].sort()) !== JSON.stringify(EXPECTED_CAPABILITIES))
    throw new Error(`native_local_till_soil_${source}_capability_not_isolated`);
}
