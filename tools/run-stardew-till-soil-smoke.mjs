...
try {
  ...
  const hoeSlot = Number.isInteger(config.HoeSlot)
    ? config.HoeSlot
    : (preparedToolSlot(initial));
  ...
  if (prepared.currentTool === null || !prepared.currentTool.toLowerCase().includes("hoe")) throw new Error("hoe_postcondition_missing");
  const target = chooseSoilTile(prepared);
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_soil_tile", snapshot: summarize(prepared), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    const targetRemoved = !Array.isArray(after.soilTiles) || !after.soilTiles.some((tile) => tile?.x === target.x && tile?.y === target.y);
    const passed = accepted.state === "succeeded"
      && accepted.reasonCode === "soil_tilled"
      && evidence.before === "none"
      && evidence.after === "HoeDirt"
      && targetRemoved;
    trace.push({ phase: "terminal", receipt: summarizeReceipt(accepted), evidence, targetRemoved, after: summarize(after) });
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "soil_tilled" : accepted.reasonCode, durationMs: Date.now() - startedAt, trace }));
...
function preparedToolSlot(snapshot) {
  ...
  return snapshot.toolSlots.find((entry) => typeof entry.label === "string" && entry.label.toLowerCase().includes("hoe"))?.slot ?? null;
}
...
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, soilTiles: snapshot.soilTiles?.length ?? 0, currentTool: snapshot.currentTool ?? null };
}
