...
try {
  ...
  const equipped = await client.execute({ requestId: `till_fixture_equip_${Date.now()}`, idempotencyKey: `till_fixture_equip_idem_${Date.now()}`, action: "equip_tool", args: { slot: hoeSlot }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_soil_tile", trace, snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    const passed = receipt.state === "succeeded" && receipt.reasonCode === "soil_tilled"
      && evidence.before === "none" && evidence.after === "HoeDirt" && targetGone;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "soil_tilled" : receipt.reasonCode, target, receipt: summarizeReceipt(receipt), evidence, targetGone, trace, before: summarize(snapshot), after: summarize(after), durationMs: Date.now() - startedAt }));
...
async function travelToFarm(snapshot) {
  ...
  const accepted = await client.execute({ requestId: `till_fixture_travel_${Date.now()}`, idempotencyKey: `till_fixture_travel_idem_${Date.now()}`, action: "travel", args: { x: warp.sourceX, y: warp.sourceY }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
...
function chooseSoilTile(snapshot) { return snapshot.soilTiles?.find((tile) => Number.isInteger(tile.x) && Number.isInteger(tile.y)) ?? null; }
