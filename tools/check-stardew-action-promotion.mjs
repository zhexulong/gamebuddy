...
for (const action of published) {
  ...
  if (!contents.gameTools.includes(`isVisible("${action}")`))
    failures.push(`published_missing_host_tool_gate:${action}`);
}
for (const action of modPublished)
  if (!published.includes(action)) failures.push(`mod_published_missing_registry:${action}`);
for (const action of published) {
...
else if (Number(countMatch[1]) !== published.length)
  failures.push(`registry_published_count_mismatch:${countMatch[1]}!=${published.length}`);
...
for (const action of published)
  if (!gateActionIds.includes(action)) failures.push(`published_missing_gate_descriptor:${action}`);
for (const gate of gates) {
  ...
  if (gate.fixtureScenario !== null) {
    if (!fixtureAllowlist.has(gate.fixtureScenario))
      failures.push(`gate_fixture_missing_mod_allowlist:${gate.actionId}:${gate.fixtureScenario}`);
    if (!fixtureInitializerAllowlist.has(gate.fixtureScenario))
      failures.push(`gate_fixture_missing_initializer_allowlist:${gate.actionId}:${gate.fixtureScenario}`);
    if (!transactionAllowlist.has(gate.fixtureScenario))
      failures.push(`gate_fixture_missing_transaction_allowlist:${gate.actionId}:${gate.fixtureScenario}`);
  }
}
