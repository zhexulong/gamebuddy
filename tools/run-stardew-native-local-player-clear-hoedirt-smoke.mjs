...
/** Execute the clear-hoedirt contract against an already-connected bridge session. */
export async function runClearHoeDirtSmoke(
  client,
  receipts,
  config,
  { terminalTimeoutMs = 15_000, stabilizeTimeoutMs = 10_000, moveTimeoutMs = 55_000 } = {},
) {
  const trace = [];
  ...
  async function travelToFarm(snapshot) {
    ...
    trace.push({ phase: "terminal", receipt: summarizeReceipt(terminal) });
...
if (import.meta.main) {
  ...
  try {
    const result = await runClearHoeDirtSmoke(session.client, session.receipts, config);
