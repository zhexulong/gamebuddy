...
try {
  ...
  const accepted = await execute(
    "clear_hoedirt",
    "clear_hoedirt",
    { slot: pickaxe.slot, x: freshTarget.x, y: freshTarget.y, expectedTargetId: freshTarget.targetId },
    snapshot,
  );
  if (accepted.state !== "succeeded" || accepted.reasonCode !== "hoedirt_cleared")
    throw new Error(`clear_hoedirt_failed:${accepted.reasonCode}`);
  ...
  console.log(
    JSON.stringify({
      ...
      target: freshTarget,
      receipt: summaryReceipt(accepted),
      evidence,
      trace,
...
function validateConfig(value) {
  ...
  if (
    ...
    fixture.FixtureScenario !== "native_clear_hoedirt_v1" ||
    value.ActionPolicyVersion !== 0 ||
    !same(value.EnabledActions, ["move_to_tile", "travel", "equip_tool", "clear_hoedirt"])
  )
...
function requireCapabilities(snapshot) {
  if (
    !same(
      ...
      ["cancel_active_execution", "clear_hoedirt", "equip_tool", "inspect_self", "move_to_tile", "travel"].sort(),
    )
  )
    throw new Error("native_local_clear_hoedirt_capability_not_isolated");
}
async function actionableSnapshot() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null || !Number.isInteger(snapshot.revision))
    throw new Error("native_local_clear_hoedirt_snapshot_not_actionable");
  return snapshot;
}
async function execute(phase, action, args, snapshot) {
  ...
  const requestId = `native_local_clear_hoedirt_${phase}_${Date.now()}_${trace.length}`;
  ...
  trace.push({ phase, action, args, receipt: summaryReceipt(receipt) });
  return receipt;
}
...
async function move(snapshot, target, phase) {
  ...
  return (
    await waitForTerminal(accepted, (latest) => latest.activeExecution == null && adjacent(latest.tile, target), 55_000)
  ).snapshot;
}
async function waitForTerminal(accepted, predicate, timeoutMs = 15_000) {
  const deadline = Date.now() + timeoutMs;
  let latest = await client.observe();
  while (Date.now() < deadline) {
    const terminal = receipts.find(
      (entry) =>
        entry.executionId === accepted.executionId && entry.requestId === accepted.requestId && isTerminal(entry.state),
    );
    if (terminal?.state !== "succeeded") {
      if (terminal) throw new Error(`navigation_failed:${terminal.reasonCode}`);
    }
    if (terminal?.state === "succeeded" && predicate(latest)) {
      trace.push({ phase: "terminal", receipt: summaryReceipt(terminal) });
...
function chooseHoeDirt(snapshot) {
  const targets = hoeDirtCandidates(snapshot);
  if (targets.length !== 1)
    throw new Error(targets.length ? "ambiguous_live_clear_hoedirt_target" : "no_live_clear_hoedirt_target");
  return targets[0];
}
function findSameHoeDirt(snapshot, target) {
  return (snapshot.clearHoeDirtTargets ?? []).find(
    (entry) =>
      entry?.targetId === target.targetId && entry.x === target.x && entry.y === target.y && validHoeDirt(entry),
  );
}
...
function parseStrictEvidence(receiptEvidence) {
  ...
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return {};
  return evidence;
}
...
function summary(snapshot) {
  return {
    ...
    clearHoeDirtTargets: snapshot.clearHoeDirtTargets?.length ?? 0,
