...
const scope = Object.freeze({
  integrationId: "stardew_portfolio", topology: "single_player_native_companion",
  saveId: "save", worldId: "world", localPlayerId: "player", companionId: "companion",
  bindingGeneration: 1, bindingHash: "hash",
});
function lawfulGiven(overrides = {}) {
  return {
    source: "target_version_native_sleep_probe", readOnly: true,
    saveMutationObserved: false, gameplayMutationObserved: false,
    topology: "single_player_native_companion", singlePlayer: true,
    currentNativeLocalPlayer: true, sleepEligible: true, activeMenu: false,
    activeEvent: false, activeMinigame: false, terminalOutcomePresent: false, scope,
    ...overrides,
  };
}
...
test("Given accepts only a fresh read-only lawful native sleep observation", async () => {
  ...
  for (const invalid of [
    lawfulGiven({ sleepEligible: false }), lawfulGiven({ gameplayMutationObserved: true }),
    lawfulGiven({ terminalOutcomePresent: true }), lawfulGiven({ scope: { ...scope, forged: true } }),
  ]) {
    assert.equal((await readSleepDayGiven({ expectedScope: scope, observeNative: async () => invalid })).state, "BLOCKED");
  }
});
...
test("no unavailable or malformed native probe can be mistaken for a runnable preflight", async () => {
  ...
  assert.equal((await runSleepDayPreflight({ expectedScope: scope, observeNative: async () => { throw new Error("unavailable"); } })).code, "sleep_day_native_probe_failed");
  assert.equal((await runSleepDayPreflight({ expectedScope: scope, observeNative: async () => lawfulGiven({ readOnly: false }) })).code, "sleep_day_given_invalid");
});
