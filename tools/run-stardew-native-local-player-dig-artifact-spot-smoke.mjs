...
try {
  ...
  console.log(
    JSON.stringify({
      ...
      target,
      receipt: summaryReceipt(terminal),
      evidence,
      trace,
...
function validateConfig(value) {
  ...
  if (
    ...
    fixture.FixtureScenario !== "native_dig_artifact_spot_v1" ||
...
function requireCapabilities(snapshot) {
  if (
    !same(
      ...
      ["cancel_active_execution", "dig_artifact_spot", "equip_tool", "inspect_self", "move_to_tile", "travel"].sort(),
    )
  )
    throw new Error("native_local_dig_artifact_spot_capability_not_isolated");
}
async function actionableSnapshot() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null || !Number.isInteger(snapshot.revision))
    throw new Error("native_local_dig_artifact_spot_snapshot_not_actionable");
  return snapshot;
}
...
function exactReceiptPair(accepted) {
