...
import { assertDirectLocalReportPath, parseArgs } from "./derive-stardew-csharp-syntax-structural-canary.mjs";
...
function recursivelyHasForbiddenKey(value) {
  const forbidden = new Set(["action", "primitive", "operation", "contract", "receipt", "projection", "reuse", "semanticFamily", "directCall", "fieldWrite", "stateMutation"]);
...
test("CLI accepts exactly one game path and local report path", () => {
  assert.deepEqual(parseArgs(["--game-path", "C:/Stardew", "--out", ".worktree/report.json"]), {
    "game-path": "C:/Stardew",
    out: ".worktree/report.json",
  });
  assert.throws(() => parseArgs(["--game-path", "C:/Stardew", "--out", "report.json", "--other", "value"]), { code: "invalid_argument" });
...
test("CLI report paths must be direct local .worktree children", () => {
  assert.equal(assertDirectLocalReportPath(".worktree/report.json"), "report.json");
  for (const outputPath of ["report.json", ".worktree/", ".worktree/nested/report.json", ".worktree/../report.json", "C:/outside/report.json"]) {
    assert.throws(() => assertDirectLocalReportPath(outputPath), { code: "output_path_invalid" });
  }
});
...
test("extracts only neutral C# syntax facts while retaining overloads, nesting, and guarded syntax", async () => {
  ...
  assert.equal(report.declarations.filter((item) => item.declarationSyntaxKind === "method_declaration" && item.identifierSyntax === "Run").length, 3);
  assert.equal(report.declarations.filter((item) => item.declarationSyntaxKind === "method_declaration" && item.identifierSyntax === "Run" && item.bodyLocator).length, 2);
  assert.equal(report.declarations.filter((item) => item.declarationSyntaxKind === "struct_declaration" && item.identifierSyntax === "Nested").length, 1);
  assert.deepEqual(report.controlSyntax.map((item) => item.syntaxKind).sort(), ["if_statement", "if_statement", "return_statement", "return_statement", "switch_statement"]);
  ...
  const contentShapedInvocation = report.invocationSyntax.find((item) => item.argumentSyntaxCount === 1 && item.calleeSyntaxKind === "member_access_expression");
  ...
  assert.ok(report.declarations.every((item) => fixtureBytes.subarray(item.locator.startByte, item.locator.endByte).toString("utf8").length > 0));
  assert.ok(report.controlSyntax.every((item) => fixtureBytes.subarray(item.locator.startByte, item.locator.endByte).toString("utf8").length > 0));
