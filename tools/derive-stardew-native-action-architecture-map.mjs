...
const ROOT_REGISTER = Object.freeze([
  {
    id: "root:control-action",
    family: "player-control",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "public static bool pressActionButton(",
  },
  {
    id: "root:control-tool",
    family: "player-control",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "public static bool pressUseToolButton()",
  },
  {
    id: "root:menu-host",
    family: "menu-event-minigame",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "public static void updateActiveMenu(GameTime gameTime)",
  },
  {
    id: "root:game-update",
    family: "game-update",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "private void _update(GameTime gameTime)",
  },
  {
    id: "root:world-location",
    family: "world-location",
    sourcePath: "StardewValley/GameLocation.cs",
    anchor: "public virtual bool performAction(string fullActionString",
  },
  {
    id: "root:content-loader",
    family: "content-dispatch",
    sourcePath: "StardewValley/DataLoader.cs",
    anchor: "public static class DataLoader",
  },
  {
    id: "root:save",
    family: "save-load",
    sourcePath: "StardewValley/SaveGame.cs",
    anchor: "public static IEnumerator<int> Save()",
  },
  {
    id: "root:new-day",
    family: "day-progression",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "public static void NewDay(float timeToPause)",
  },
  {
    id: "root:network",
    family: "network",
    sourcePath: "StardewValley/Multiplayer.cs",
    anchor: "Game1.client.receiveMessages();",
  },
]);

const BOUNDARY_REGISTER = Object.freeze([
  {
    id: "boundary:world-location-content-text",
    family: "content-boundary",
    sourcePath: "StardewValley/GameLocation.cs",
    anchor: "Game1.content.LoadString(",
  },
  {
    id: "boundary:world-location-event-host",
    family: "event-boundary",
    sourcePath: "StardewValley/GameLocation.cs",
    anchor: "public virtual void startEvent(Event evt)",
  },
  {
    id: "boundary:menu-host-current",
    family: "menu-boundary",
    sourcePath: "StardewValley/Game1.cs",
    anchor: "public static void updateActiveMenu(GameTime gameTime)",
  },
  {
    id: "boundary:save-iterator",
    family: "save-boundary",
    sourcePath: "StardewValley/SaveGame.cs",
    anchor: "public static IEnumerator<int> getSaveEnumerator()",
  },
  {
    id: "boundary:network-client",
    family: "network-boundary",
    sourcePath: "StardewValley/Multiplayer.cs",
    anchor: "Game1.client.receiveMessages();",
  },
  {
    id: "boundary:content-loader",
    family: "content-boundary",
    sourcePath: "StardewValley/DataLoader.cs",
    anchor: "public static class DataLoader",
  },
]);
const REQUIRED_ROOT_FAMILIES = Object.freeze([
  "player-control",
  "menu-event-minigame",
  "game-update",
  "world-location",
...
function configurationDigest() {
  return hash(JSON.stringify({ tool: "ilspycmd", options: DECOMPILE_OPTIONS, target: ASSEMBLY_NAME }));
}
...
async function inspectTarget(gamePath) {
  ...
  if (assemblySha256 !== EXPECTED_SHA256)
    fail("target_assembly_hash_mismatch", "Supplied assembly differs from exact locked target.", {
      expected: EXPECTED_SHA256,
      actual: assemblySha256,
    });
  const versionResult = await execFileAsync(
    "powershell.exe",
    ["-NoProfile", "-Command", "$p=$env:GAMEBUDDY_INSPECT_ASSEMBLY;(Get-Item -LiteralPath $p).VersionInfo.FileVersion"],
    { encoding: "utf8", env: { ...process.env, GAMEBUDDY_INSPECT_ASSEMBLY: assemblyPath } },
  );
  const fileVersion = versionResult.stdout.trim();
  if (fileVersion !== EXPECTED_VERSION)
    fail("target_assembly_version_mismatch", "Supplied assembly differs from locked file version.", {
  ...
  try {
    ...
    fail("content_manifest_missing", "Could not read Content/ContentHashes.json.");
  }
  return Object.freeze({
    assemblyPath,
    target: { relativePath: ASSEMBLY_NAME, fileVersion, lengthBytes: assembly.size, sha256: assemblySha256 },
    contentHashes,
    contentHashesSha256: await sha256File(contentHashesPath),
  });
}
async function decompile(assemblyPath) {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), "gamebuddy-stardew-architecture-"));
  const tool = process.env.ILSPYCMD_PATH || "ilspycmd";
  try {
    const version = await execFileAsync(tool, ["--version"], { encoding: "utf8" });
    const toolVersion = version.stdout.trim().split(/\r?\n/)[0];
    if (toolVersion !== "ilspycmd: 9.1.0.7988")
      fail("decompiler_version_mismatch", "Architecture map requires the locked ilspycmd version.", {
        actual: toolVersion,
      });
    await execFileAsync(tool, [...DECOMPILE_OPTIONS, "-o", outputRoot, assemblyPath], {
      encoding: "utf8",
      maxBuffer: 256 * 1024,
    });
    return Object.freeze({
...
async function sourceRecords(root) {
  ...
  for (const filePath of files) {
    ...
    manifestRows.push(`${relativePath}\t${sha256}`);
  }
  return Object.freeze({
    records: Object.freeze(records),
    fileCount: records.length,
    manifestSha256: hash(`${manifestRows.join("\n")}\n`),
  });
}
function contentPaths(contentHashes) {
  return Object.keys(contentHashes)
    .map((value) => value.replaceAll("\\", "/"))
    .sort();
}
function contentManifestSha256(contentHashes) {
  const rows = Object.entries(contentHashes)
...
async function main() {
  ...
  const target = await inspectTarget(path.resolve(args["game-path"]));
  const decompilation = await decompile(target.assemblyPath);
  try {
    const sources = await sourceRecords(decompilation.outputRoot);
    const paths = contentPaths(target.contentHashes);
    ...
    const report = Object.freeze({
      ...
      inputUniverse: {
        ...
        contentPathCount: paths.length,
        contentHashesSha256: target.contentHashesSha256,
        contentManifestSha256: contentManifestSha256(target.contentHashes),
      },
      accounting,
      analysisBoundary: {
