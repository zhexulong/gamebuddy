...
try {
  if (!client.state.capabilities.includes("collect_animal_product")) throw new Error("collect_animal_product_capability_missing");
  ...
  if (!snapshot.capabilities.includes("collect_animal_product")) throw new Error("snapshot_collect_animal_product_capability_missing");
  const target = Array.isArray(snapshot.animalProductTargets)
    ? snapshot.animalProductTargets.find((entry) => Number.isInteger(entry.slot)
      ...
      && (entry.produceStack === 1 || entry.produceStack === 2)) ?? null
    : null;
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_animal_product_target", snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    if (accepted.state !== "accepted") throw new Error(`collect_animal_product_not_accepted:${accepted.state}:${accepted.reasonCode}`);
    ...
    console.log(JSON.stringify({
      ...
      target: summarizeTarget(target), receipt: summarizeReceipt(receipt),
      before: summarize(snapshot), after: summarize(after), durationMs: Date.now() - startedAt,
    }));
...
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, animalProductTargets: snapshot.animalProductTargets?.length ?? 0 };
}
function summarizeTarget(target) {
  return { targetId: target.targetId, slot: target.slot, x: target.x, y: target.y, animalType: target.animalType, qualifiedProduceItemId: target.qualifiedProduceItemId, toolKind: target.toolKind, produceStack: target.produceStack };
}
