...
try {
  ...
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_forage_target", trace, snapshot: summarize(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    ...
    const inventoryDelta = Number.isSafeInteger(beforeEvidence) && Number.isSafeInteger(afterEvidence) ? afterEvidence - beforeEvidence : null;
    const passed = receipt.state === "succeeded" && receipt.reasonCode === "forage_picked_up"
      && evidenceMatches && inventoryDelta === 1 && targetGone;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "forage_picked_up" : receipt.reasonCode, target, beforeCount, receipt: summarizeReceipt(receipt), evidence, inventoryDelta, targetGone, trace, before: summarize(snapshot), after: summarize(after), durationMs: Date.now() - startedAt }));
...
async function travelToFarm(snapshot) {
  ...
  const receipt = await client.execute({ requestId: `forage_fixture_travel_${Date.now()}`, idempotencyKey: `forage_fixture_travel_idem_${Date.now()}`, action: "travel", args: { x: warp.sourceX, y: warp.sourceY }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
...
async function moveToArrivalCandidate(snapshot, arrival) {
  ...
  for (const target of candidates) {
    ...
    try { return await move(latest, target, "move_to_native_forage_fixture"); }
    catch (error) {
...
function chooseForageTarget(snapshot) {
  return snapshot.forageTargets?.find((entry) => typeof entry.targetId === "string" && Number.isInteger(entry.x) && Number.isInteger(entry.y) && typeof entry.qualifiedItemId === "string" && entry.qualifiedItemId.length > 0) ?? null;
}
...
async function waitForReceiptAndSnapshot(executionId, predicate, timeoutMs) {
  const deadline = Date.now() + timeoutMs; let latest = await client.observe();
...
function summarize(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, forageTargets: snapshot.forageTargets?.length ?? 0, inventorySlots: snapshot.inventorySlots ?? null, activeExecution: snapshot.activeExecution ?? null }; }
