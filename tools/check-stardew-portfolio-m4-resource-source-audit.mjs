...
const root = resolve(".");
const modelPath = resolve(root, "tools/stardew-portfolio-m4-resource-source-audit.json");
...
try {
  ...
  if (sha256(await readFile(scopePath)) !== model.charterAuthority.scopeDocumentSha256)
    throw Object.assign(new Error("M4 source audit Charter authority hash drifted."), {
      code: "portfolio_m4_source_audit_authority_hash_mismatch",
    });
  ...
  for (const expected of [model.auditSource.localSnapshotContentManifestSha256, model.auditSource.targetAssemblySha256])
    if (!provenanceText.includes(expected))
      throw Object.assign(new Error("M4 source audit provenance hash is not recorded in design/13."), {
        code: "portfolio_m4_source_audit_provenance_mismatch",
      });
    ...
    await Promise.all(
      model.anchors.map(async ({ relativePath }) => {
        ...
        if (
          ...
          throw Object.assign(new Error(`M4 source audit anchor escapes source root: ${relativePath}.`), {
            code: "portfolio_m4_source_audit_path_escape",
          });
        return [relativePath, await readFile(sourcePath)];
      }),
    ),
  );
  const result = validatePortfolioM4ResourceSourceAudit(model, sourceFiles);
  process.stdout.write(
    `${JSON.stringify({ ...result, authorityHashVerified: true, provenanceMetadataVerified: true }, null, 2)}\n`,
  );
} catch (error) {
  process.stderr.write(`${error.code ?? "portfolio_m4_source_audit_check_failed"}: ${error.message}\n`);
  process.exitCode = 1;
}
