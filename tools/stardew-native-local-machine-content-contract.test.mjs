...
test("probe shape validation accepts complete evidence and rejects missing or unknown shape evidence", () => {
  const valid = { machinesContent: { state: "loaded", entries: [{ machineId: "(BC)12", shapeFailures: [], shapeEvidence: [{ path: "machine[(BC)12].outputRules[0].outputs[0].stackModifiers", status: "documented_absent", member: "StackModifiers", declaredType: "System.Collections.Generic.List`1[[StardewValley.GameData.QuantityModifier]]", sourceOptional: true, runtimeType: null }] }] } };
  ...
  const unknownShape = { machinesContent: { state: "loaded", entries: [{ machineId: "(BC)12", shapeFailures: ["machine[(BC)12].readyTimeModifiers:unsupported_shape:System.String"] }] } };
  assert.equal(validateProbeShapes(unknownShape), "machine_content_shape_unknown");

  const missingMachineField = { machinesContent: { state: "loaded", entries: [{ machineId: "(BC)12", shapeFailures: ["machine[(BC)12].preventTimePass:missing_member"], shapeEvidence: [] }] } };
  assert.equal(validateProbeShapes(missingMachineField), "machine_content_shape_unknown");

  const nonNullableNull = { machinesContent: { state: "loaded", entries: [{ machineId: "(BC)12", shapeFailures: ["machine[(BC)12].additionalConsumedItems:nonnullable_null"], shapeEvidence: [] }] } };
  assert.equal(validateProbeShapes(nonNullableNull), "machine_content_shape_unknown");

  const untrustedAbsent = { machinesContent: { state: "loaded", entries: [{ machineId: "(BC)12", shapeFailures: [], shapeEvidence: [{ path: "machine[(BC)12].additionalConsumedItems", status: "documented_absent", member: "AdditionalConsumedItems", declaredType: "System.Collections.Generic.List`1", sourceOptional: true, runtimeType: null }] }] } };
...
test("content contract retains target hash, decoded-table digest, and an empty configuration-ingress universe gate", async () => {
  ...
  assert.match(contract, /EXPECTED_MACHINE_DIGEST = "1516b6a7a7d80263081c1ca5ec5a79cfef808be7f167fc679cab5de2850d245b"/);
