...
try {
  ...
  const canSlot = Number.isInteger(config.WateringCanSlot) ? config.WateringCanSlot : findToolSlot(initial, "wateringcan");
  ...
  if (equipped.state !== "succeeded") throw new Error(`watering_can_equip_failed:${equipped.state}:${equipped.reasonCode}`);
  const prepared = await client.observe();
  if (typeof prepared.currentTool !== "string" || !prepared.currentTool.toLowerCase().includes("wateringcan")) throw new Error("watering_can_postcondition_missing");
  const target = chooseTarget(prepared);
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_unwatered_crop", snapshot: summarize(prepared), durationMs: Date.now() - startedAt, trace }));
    process.exitCode = 2;
  } else {
    ...
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "crop_watered" : receipt.reasonCode, target, receipt: summarizeReceipt(receipt), before: summarize(prepared), after: summarize(after), durationMs: Date.now() - startedAt, trace }));
...
function findToolSlot(snapshot, needle) {
  ...
  return snapshot.toolSlots.find((entry) => typeof entry.label === "string" && entry.label.toLowerCase().replaceAll(" ", "").includes(needle))?.slot ?? null;
}
function chooseTarget(snapshot) {
  if (!Array.isArray(snapshot.cropTargets)) return null;
  return snapshot.cropTargets.find((entry) => Number.isInteger(entry.x) && Number.isInteger(entry.y) && typeof entry.targetId === "string") ?? null;
}
function summarize(snapshot) {
  return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, cropTargets: snapshot.cropTargets?.length ?? 0, currentTool: snapshot.currentTool ?? null };
}
