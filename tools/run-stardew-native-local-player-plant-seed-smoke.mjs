...
try {
  ...
  const sourceDisappeared = Array.isArray(after.seedTargets)
    && after.seedTargets.every((entry) => entry.targetId !== target.targetId
      && (entry.x !== target.x || entry.y !== target.y));
  ...
  const evidenceBound = evidence.location === snapshot.location
    ...
    && typeof evidence.crop === "string" && evidence.crop.length > 0 && evidence.crop !== "none";
  const freshPostcondition = Number.isInteger(after.revision) && after.revision === terminal.revision
    && after.location === snapshot.location && sameTile(after.tile, snapshot.tile)
    && after.actionable === true && after.activeExecution == null && sourceDisappeared;
  const passed = terminal.state === "succeeded" && terminal.reasonCode === "seed_planted"
    && evidenceBound && inventoryDeltaProven && freshPostcondition;
  console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "seed_planted" : terminal.reasonCode, target, receipt: summarizeReceipt(terminal), evidence, sourceDisappeared, inventoryDeltaProven, freshPostcondition, trace, before: summarize(snapshot), after: summarize(after), durationMs: Date.now() - startedAt }));
...
async function execute(phase, action, args, snapshot, requestId = `native_local_plant_seed_${phase}_${Date.now()}`, idempotencyKey = `native_local_plant_seed_${phase}_idem_${Date.now()}`) {
  ...
  if (!receipts.some((entry) => entry.executionId === receipt.executionId && entry.requestId === receipt.requestId && entry.state === receipt.state)) receipts.push(receipt);
...
function chooseReachableSeedTarget(snapshot) { return snapshot.seedTargets?.find((target) => Number.isInteger(target.slot) && Number.isInteger(target.x) && Number.isInteger(target.y) && typeof target.targetId === "string" && target.targetId.length > 0 && typeof target.qualifiedItemId === "string" && target.qualifiedItemId.length > 0 && adjacent(snapshot.tile, target)) ?? null; }
...
async function waitForPlantPostcondition(receipt, target, evidence, before, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    const fresh = Number.isInteger(latest.revision) && latest.revision === receipt.revision
      && latest.location === before.location && sameTile(latest.tile, before.tile)
      && latest.actionable === true && latest.activeExecution == null
      && Array.isArray(latest.seedTargets)
      && latest.seedTargets.every((entry) => entry.targetId !== target.targetId && (entry.x !== target.x || entry.y !== target.y));
    if (fresh) return latest;
    await delay(200); latest = await client.observe();
  }
  throw new Error(`plant_postcondition_timeout:${receipt.executionId}`);
}
function findTerminalReceipt(executionId, requestId) { return receipts.find((receipt) => receipt.executionId === executionId && receipt.requestId === requestId && TERMINAL_STATES.has(receipt.state)); }
async function requireExactTerminalReceipt(receipt, requestId, phase) {
  if (typeof receipt?.executionId !== "string" || receipt.executionId.length === 0 || receipt.requestId !== requestId) throw new Error(`${phase}_receipt_identity_mismatch`);
...
function requireExactCapabilities(actual, source) { if (!Array.isArray(actual) || JSON.stringify([...actual].sort()) !== JSON.stringify(EXPECTED_CAPABILITIES)) throw new Error(`native_local_plant_seed_${source}_capability_not_isolated`); }
function assertNativeLocalFixtureConfig(value) {
  const fixture = value.NativeLocalPlayerFixture;
  if (fixture?.Enable !== true || fixture.Bootstrap?.Enable === true || fixture.FixtureScenario !== "native_plant_seed_v1" || !validFixtureSlotRelationship(fixture.LogicalSaveName, fixture.ObservedSaveSlot)) throw new Error("native_local_fixture_config_invalid");
  ...
  if (value.ActionPolicyVersion !== 0 || JSON.stringify(value.EnabledActions) !== JSON.stringify(EXPECTED_ACTIONS)) throw new Error("native_local_fixture_action_policy_invalid");
}
...
function parseEvidence(evidence) {
  ...
  const result = {};
  for (const part of detail.split(";")) { const index = part.indexOf("="); const key = index > 0 ? part.slice(0, index) : ""; const value = index > 0 ? part.slice(index + 1) : ""; if (index <= 0 || index === part.length - 1 || !/^[a-z][a-z0-9_]{0,63}$/.test(key) || value.length > 512 || Object.hasOwn(result, key)) throw new Error("invalid_plant_seed_evidence"); result[key] = value; }
  if (JSON.stringify(Object.keys(result).sort()) !== JSON.stringify(EXPECTED_EVIDENCE_KEYS)) throw new Error("plant_seed_evidence_keys_mismatch");
  return result;
}
function sameTile(left, right) { return Number.isInteger(left?.x) && Number.isInteger(left?.y) && Number.isInteger(right?.x) && Number.isInteger(right?.y) && left.x === right.x && left.y === right.y; }
function adjacent(left, right) { return Math.abs(left.x - right.x) <= 1 && Math.abs(left.y - right.y) <= 1; }
function summarize(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, seedTargets: snapshot.seedTargets?.length ?? 0, activeExecution: snapshot.activeExecution ?? null }; }
