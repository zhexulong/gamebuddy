...
function validateSelection(value) {
  ...
  if (
    value.dispatchBinding.required !== true ||
    value.dispatchBinding.resolution !== "before_native_invocation" ||
    value.dispatchBinding.immutable !== true ||
    value.dispatchBinding.provenance !== "fresh_observation_bound_to_signed_dsm"
  )
...
function validateAction(value, expectedId) {
  ...
  if (value.actionId !== expectedId || value.actionClass !== "primitive")
  ...
  exact(
    value.input,
    expectedId === ACTIONS[0] ? ["itemIdentity", "quality", "stack", "boundAtDispatch"] : ["reward", "boundAtDispatch"],
    `${expectedId}.input`,
  );
  if (expectedId === ACTIONS[0]) {
    opaque(
      value.input.itemIdentity,
      `${expectedId}.input.itemIdentity`,
      "opaque_runtime_item_identity",
      "fresh_observation",
    );
  ...
  if (
    value.guards.gameThreadRevalidation !== true ||
    value.guards.topology !== TOPOLOGY ||
    value.guards.freshObservation !== true ||
    value.guards.selectedDomain !== "signed_dsm_bounded_domain"
  )
    fail(`${expectedId} guards are not strict.`);
  for (const field of [
    "player",
    "world",
    "scope",
  ...
  if (
    value.terminalReceipt.state !== "succeeded" ||
    value.terminalReceipt.evidence !== "non_empty_action_specific" ||
    value.terminalReceipt.sameExecution !== true ||
    value.terminalReceipt.actionSpecific !== true
  )
    fail(`${expectedId} receipt policy is not strict.`);
  exact(
    value.freshPostcondition,
    ["provenance", "requiresRevisionAfterReceipt", "facts"],
    `${expectedId}.freshPostcondition`,
  );
  if (
    value.freshPostcondition.provenance !== "fresh_native_observation" ||
    value.freshPostcondition.requiresRevisionAfterReceipt !== true ||
    !Array.isArray(value.freshPostcondition.facts) ||
    value.freshPostcondition.facts.length === 0
  )
    fail(`${expectedId} fresh postcondition is invalid.`);
  exact(value.lifecycle, ["cancellation", "replay", "saveReopen"], `${expectedId}.lifecycle`);
  if (
    value.lifecycle.cancellation !== "local_stop_no_success" ||
    value.lifecycle.replay !== "idempotent_same_request_only" ||
    value.lifecycle.saveReopen !== "required_when_composed"
  )
...
export function validateM7BundleFixtureContract(fixture) {
  ...
  if (
    ...
    fixture.artifactKind !== "portfolio_m7_unprovisioned_fixture_contract" ||
    fixture.fixtureId !== "portfolio_m7_bundle_unprovisioned_v1" ||
    fixture.topology !== TOPOLOGY ||
    fixture.targetVersion !== TARGET_VERSION ||
    fixture.provisioningState !== "unprovisioned_example"
  )
    fail("M7 fixture identity, topology, version, or provisioning state is invalid.");
  string(fixture.purpose, "fixture.purpose");
  exact(
    fixture.startingState,
    ["saveCreation", "communityCenter", "bundleSlot", "playerInventory", "bundleReward", "receipts", "terminalFacts"],
    "fixture.startingState",
  );
  if (
    fixture.startingState.saveCreation !== "target_version_native_only" ||
    fixture.startingState.communityCenter !== "ordinary_nonterminal_state" ||
    fixture.startingState.bundleSlot !== "unselected_not_precompleted" ||
    fixture.startingState.playerInventory !== "ordinary_native_inventory" ||
    fixture.startingState.bundleReward !== "not_available_unclaimed" ||
    fixture.startingState.receipts !== "none" ||
    fixture.startingState.terminalFacts !== "absent"
  )
    fail("M7 fixture starting state is terminal or synthetic.");
  exact(
    fixture.freshSelectionRequirements,
    ["slot", "acceptedItemDomain", "quality", "stack", "dispatchBinding", "baseline"],
    "fixture.freshSelectionRequirements",
  );
  if (
    fixture.freshSelectionRequirements.slot !== "fresh_opaque_observation" ||
    fixture.freshSelectionRequirements.acceptedItemDomain !== "signed_dsm_bounded_domain" ||
    fixture.freshSelectionRequirements.quality !== "dsm_bound_0_to_4_at_dispatch" ||
    fixture.freshSelectionRequirements.stack !== "dsm_bound_1_to_999_at_dispatch" ||
    fixture.freshSelectionRequirements.dispatchBinding !==
      "freeze_slot_item_identity_quality_stack_before_native_invocation" ||
    fixture.freshSelectionRequirements.baseline !== "capture_progress_inventory_reward_before_each_transition"
  )
    fail("M7 fixture selection requirements are invalid.");
  exactArray(
...
export function validateM7BundleActionContract(contract) {
  exact(
    ...
    "M7 bundle action contract",
  );
  if (
    contract.schemaVersion !== 1 ||
    contract.artifactKind !== "portfolio_static_action_contract" ||
    contract.contractId !== "portfolio_m7_bundle_actions_v1" ||
    contract.milestone !== "M7" ||
    contract.topology !== TOPOLOGY ||
    contract.targetVersion !== TARGET_VERSION
  )
    fail("M7 action contract identity, milestone, topology, or version is invalid.");
  exact(
  ...
  if (
    ...
    contract.sourceAudit.auditId !== AUDIT_ID ||
    contract.sourceAudit.traceFamilyId !== TRACE_FAMILY_ID ||
    contract.sourceAudit.projectionState !== "blocked" ||
    contract.sourceAudit.liveState !== "not_performed"
  )
    fail("M7 source audit identity or non-promotable state is invalid.");
  ...
  if (
    ...
    JSON.stringify(contract.actions.map((action) => action.actionId)) !== JSON.stringify(ACTIONS)
  )
    fail("M7 actions must contain contribution then independent reward claim.");
  validateAction(contract.actions[0], ACTIONS[0]);
  validateAction(contract.actions[1], ACTIONS[1]);
  if (
    contract.actions[0].nativeBoundary.progress !== "slot_progress_commit_only" ||
    contract.actions[0].nativeBoundary.reward !== "observe_availability_only" ||
    contract.actions[0].nativeBoundary.presentation !== "no_menu_or_callback"
  )
    fail("contribution must preserve progress/reward/presentation boundaries.");
  if (
    contract.actions[1].nativeBoundary.progress !== "observe_existing_progress_only" ||
    contract.actions[1].nativeBoundary.reward !== "reward_claim_commit_only" ||
    contract.actions[1].nativeBoundary.presentation !== "typed_non_ui_inventory_transfer"
  )
    fail("reward claim must preserve progress/reward/presentation boundaries.");
  if (
    contract.actions[0].commit.transaction !== "contribute_one_selected_bundle_slot" ||
  ...
  exact(
    ...
    [
      "freshObservation",
      "opaqueSelectedSlot",
      "boundedDsmItemIdentityQualityStack",
      "scope",
      "gameThread",
      "revision",
      "policy",
  ...
  if (
    contract.implementationStatus !== "implementation_needed" ||
    contract.fixtureStatus !== "fixture_needed" ||
    contract.liveClosure !== "none" ||
    contract.publication !== "none"
  )
    fail("M7 status must remain implementation_needed/fixture_needed/liveClosure none.");
    ...
    await Promise.all(
      audit.anchors.map(async ({ relativePath }) => {
        ...
        if (!source.startsWith(`${sourceRoot}${path.sep}`))
          fail(`M7 source audit anchor escapes source root: ${relativePath}.`, "portfolio_m7_source_audit_path_escape");
...
export async function checkM7BundleActionContract(
  contractPath = path.resolve("tools/stardew-portfolio-m7-bundle-action-contract.json"),
  root = path.resolve("."),
) {
  ...
  if (audit.auditId !== AUDIT_ID || audit.traceFamilyId !== TRACE_FAMILY_ID || audit.topology !== TOPOLOGY)
    fail("M7 source audit identity does not match the contract.", "portfolio_m7_source_audit_identity_mismatch");
  ...
  if (auditResult.projectionState !== "blocked" || auditResult.liveState !== "not_performed")
    fail("M7 source audit promoted unexpectedly.");
  return Object.freeze({
    ...result,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const contractPath = process.argv[2]
      ...
      : path.resolve("tools/stardew-portfolio-m7-bundle-action-contract.json");
