...
const sourceFiles = { "StardewValley/Game1.cs": { text: game, sha256: h(game) }, "StardewValley/Farmer.cs": { text: farmer, sha256: h(farmer) } };
const attestation = { targetAssemblySha256: "a".repeat(64), sourceManifestSha256: "b".repeat(64), decompilerConfigurationDigest: "c".repeat(64) };
test("derives only source-attested direct control roots and preserves dynamic/uninventoried exits", () => { const register = deriveNativeNormalPlayerControlSlice({ sourceFiles, attestation }); const checked = validateNativeNormalPlayerIngressRegister(register, { expectedAttestation: attestation, sourceFiles }); assert.equal(checked.rootCount, 5); assert.equal(checked.gapCount, 3); assert.equal(checked.closureState, "partial"); assert.deepEqual(register.roots.map((root) => root.rootId), ["root:game1-press-action", "root:game1-press-use-tool", "root:game1-press-switch-tool", "root:farmer-fire-tool", "root:farmer-set-moving"]); });
test("fails if exact source anchors do not have a unique native source witness", () => { assert.throws(() => deriveNativeNormalPlayerControlSlice({ sourceFiles: { ...sourceFiles, "StardewValley/Game1.cs": { text: `${game}\n${game}`, sha256: h(`${game}\n${game}`) } }, attestation }), { code: "normal_player_ingress_anchor_missing" }); });
