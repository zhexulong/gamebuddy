...
function transition(value, label, errors, expected) {
  ...
  if (value?.from !== expected.from || value?.to !== expected.to) errors.push(`${label} has an invalid state transition.`);
  if (!Array.isArray(value?.nativeFacts) || value.nativeFacts.length === 0 || !value.nativeFacts.every((entry) => typeof entry === "string"))
...
function action(action, expectedId, errors) {
  exactFields(action, ["actionId", "actionClass", "target", "input", "eligibility", "commit", "receiptEvidence", "freshPostcondition", "doesNotDo"], `${expectedId} action`, errors);
  ...
  exactFields(action?.input, expectedId === "accept_special_order_offer" ? ["offerTarget", "generation"] : ["orderKey", "generation", "reward"], `${expectedId}.input`, errors);
  for (const value of Object.values(action?.input ?? {})) exactString(value, "selectedDomain.reference", `${expectedId}.input`, errors);
  exactFields(action?.receiptEvidence, ["sameExecution", "successState", "evidenceNonEmpty", "actionSpecific"], `${expectedId}.receiptEvidence`, errors);
  if (action?.receiptEvidence?.sameExecution !== true || action?.receiptEvidence?.successState !== "succeeded" || action?.receiptEvidence?.evidenceNonEmpty !== true || action?.receiptEvidence?.actionSpecific !== true)
    errors.push(`${expectedId}.receiptEvidence is not authoritative-shaped.`);
  if (!Array.isArray(action?.freshPostcondition) || action.freshPostcondition.length === 0 || !action.freshPostcondition.every((entry) => typeof entry === "string"))
...
export function validateM9SpecialOrderActionContract(contract) {
  ...
  exactFields(contract, [
    "schemaVersion", "artifactKind", "contractId", "milestone", "topology", "target", "sourceAudit",
    "selectedDomain", "objectiveBoundary", "actions", "stateMachine", "sharedRequestGuards", "sharedEvidenceRules",
    "status", "forbiddenBehavior", "nonClaims", "fixtureContractRef",
  ], "contract", errors);
  ...
  for (const [field, expected] of Object.entries(TARGET)) exactString(contract?.target?.[field], expected, `target.${field}`, errors);

  exactFields(contract?.sourceAudit, ["auditId", "traceFamilyId", "sourceAuditFile", "anchorIds", "projectionState", "liveState"], "sourceAudit", errors);
  exactString(contract?.sourceAudit?.auditId, "portfolio_m9_special_order_source_audit_v1", "sourceAudit.auditId", errors);
  exactString(contract?.sourceAudit?.traceFamilyId, "portfolio_m9_special_order_to_reward", "sourceAudit.traceFamilyId", errors);
  ...
  exactFields(contract?.selectedDomain, ["offerTarget", "orderKey", "generation", "objectiveSet", "reward", "selectionRule"], "selectedDomain", errors);
  for (const field of ["offerTarget", "orderKey", "generation", "objectiveSet", "reward"]) selector(contract?.selectedDomain?.[field], `selectedDomain.${field}`, errors);
  exactString(contract?.selectedDomain?.selectionRule, "DSM selects finite content; fresh observation supplies opaque offer and generation; no static ID is accepted", "selectedDomain.selectionRule", errors);

  exactFields(contract?.objectiveBoundary, ["sourceDerivedStages", "objectiveWorkPolicy", "allowedObjectiveIngress", "genericProgressAction", "genericCompleteAction", "completionObservation"], "objectiveBoundary", errors);
  exactArray(contract?.objectiveBoundary?.sourceDerivedStages, [
  ...
  ], "objectiveBoundary.sourceDerivedStages", errors);
  exactFields(contract?.objectiveBoundary?.objectiveWorkPolicy, ["mode", "selectionRequirement", "runtimeClaim"], "objectiveBoundary.objectiveWorkPolicy", errors);
  exactString(contract?.objectiveBoundary?.objectiveWorkPolicy?.mode, "reuse_or_separately_selected", "objectiveWorkPolicy.mode", errors);
  exactString(contract?.objectiveBoundary?.objectiveWorkPolicy?.selectionRequirement, "each objective must map to an existing equivalent typed action or an independently selected typed objective action", "objectiveWorkPolicy.selectionRequirement", errors);
  exactString(contract?.objectiveBoundary?.objectiveWorkPolicy?.runtimeClaim, "unselected objective domains are observable only and cannot be claimed", "objectiveWorkPolicy.runtimeClaim", errors);
  exactString(contract?.objectiveBoundary?.allowedObjectiveIngress, "existing_equivalent_action_or_separately_selected_typed_objective_action", "allowedObjectiveIngress", errors);
  ...
  exactFields(contract?.objectiveBoundary?.completionObservation, ["requiresAllSelectedObjectivesComplete", "requiresNativeCompleteState", "requiresRewardEntitlement", "doesNotClaimReward"], "completionObservation", errors);
  for (const field of ["requiresAllSelectedObjectivesComplete", "requiresNativeCompleteState", "requiresRewardEntitlement", "doesNotClaimReward"]) if (contract?.objectiveBoundary?.completionObservation?.[field] !== true) errors.push(`completionObservation.${field} must be true.`);

  if (!Array.isArray(contract?.actions) || contract.actions.length !== 2) errors.push("actions must contain exactly the two distinct M9 actions.");
  else {
    if (JSON.stringify(contract.actions.map((entry) => entry?.actionId)) !== JSON.stringify(EXPECTED_ACTIONS)) errors.push("actions must be ordered accept then claim and contain no generic action.");
    action(contract.actions[0], EXPECTED_ACTIONS[0], errors);
    action(contract.actions[1], EXPECTED_ACTIONS[1], errors);
    exactFields(contract.actions[0]?.eligibility, ["offerFresh", "offerSelectedByDsm", "generationMatchesOffer", "orderTypeNotAlreadyAccepted", "playerEligible", "unselectedDomainRejected"], "accept eligibility", errors);
    for (const field of ["offerFresh", "offerSelectedByDsm", "generationMatchesOffer", "orderTypeNotAlreadyAccepted", "playerEligible", "unselectedDomainRejected"]) if (contract.actions[0]?.eligibility?.[field] !== true) errors.push(`accept eligibility.${field} must be true.`);
    exactFields(contract.actions[0]?.commit, ["transaction", "nativeEffects", "separateFromRewardClaim"], "accept commit", errors);
    exactString(contract.actions[0]?.commit?.transaction, "accept_offer", "accept commit.transaction", errors);
    exactArray(contract.actions[0]?.commit?.nativeEffects, ["accepted_special_order_type_added", "generated_order_key_and_generation_added_to_native_team_state"], "accept commit.nativeEffects", errors);
    if (contract.actions[0]?.commit?.separateFromRewardClaim !== true) errors.push("accept must be separate from reward claim.");
    exactFields(contract.actions[1]?.eligibility, ["nativeOrderComplete", "participantRewardUnclaimed", "selectedOrderMatches", "alreadyClaimedRejected", "unselectedDomainRejected"], "claim eligibility", errors);
    for (const field of ["nativeOrderComplete", "participantRewardUnclaimed", "selectedOrderMatches", "alreadyClaimedRejected", "unselectedDomainRejected"]) if (contract.actions[1]?.eligibility?.[field] !== true) errors.push(`claim eligibility.${field} must be true.`);
    exactFields(contract.actions[1]?.commit, ["transaction", "nativeEffects", "separateFromOfferAcceptance"], "claim commit", errors);
    exactString(contract.actions[1]?.commit?.transaction, "claim_reward", "claim commit.transaction", errors);
    exactArray(contract.actions[1]?.commit?.nativeEffects, ["participant_reward_entitlement_consumed", "native_reward_granted"], "claim commit.nativeEffects", errors);
    if (contract.actions[1]?.commit?.separateFromOfferAcceptance !== true) errors.push("claim must be separate from offer acceptance.");
  }
  ...
  const expectedTransitions = [["fresh_offer_available", "accepted_in_progress"], ["accepted_in_progress", "completed_reward_unclaimed"], ["completed_reward_unclaimed", "completed_reward_claimed"]];
  if (!Array.isArray(contract?.stateMachine?.transitions) || contract.stateMachine.transitions.length !== expectedTransitions.length) errors.push("stateMachine.transitions must contain exactly three native transitions.");
  else contract.stateMachine.transitions.forEach((entry, index) => transition(entry, `stateMachine.transitions[${index}]`, errors, { from: expectedTransitions[index][0], to: expectedTransitions[index][1] }));
  exactString(contract?.stateMachine?.unselectedDomainRule, "unselected offer/objective/reward state may be observed but has no runtime claim route", "stateMachine.unselectedDomainRule", errors);

  sharedBooleanMap(contract?.sharedRequestGuards, ["freshObservation", "opaqueOffer", "opaqueGeneration", "scope", "gameThread", "revision", "policy", "deadline", "idempotency", "cancel", "gameState", "noUiOrRawFallback"], "sharedRequestGuards", errors);
  sharedBooleanMap(contract?.sharedEvidenceRules, ["receiptState", "receiptEvidence", "postcondition", "sameExecution", "objectiveSpecific", "rewardEntitlement", "saveReopen"], "sharedEvidenceRules", errors);
...
export function validateM9SpecialOrderUnprovisionedFixture(fixture) {
  const errors = [];
  exactFields(fixture, ["schemaVersion", "artifactKind", "fixtureId", "topology", "targetVersion", "purpose", "startingState", "freshSelectionRequirements", "antiFinalStepAssertions", "forbiddenBehavior", "nonClaims", "status"], "fixture", errors);
  ...
  exactFields(fixture?.startingState, ["saveCreatedBy", "specialOrderState", "selectedOffer", "selectedGeneration", "selectedObjectives", "rewardEntitlement", "terminalFacts"], "fixture.startingState", errors);
  for (const [field, expected] of Object.entries({ saveCreatedBy: "target_version_native_only", specialOrderState: "ordinary_nonterminal_or_absent", selectedOffer: "unselected", selectedGeneration: "unselected", selectedObjectives: "unselected", rewardEntitlement: "absent", terminalFacts: "absent" })) exactString(fixture?.startingState?.[field], expected, `fixture.startingState.${field}`, errors);
  exactFields(fixture?.freshSelectionRequirements, ["offer", "generation", "objectiveSet", "reward", "revalidation"], "fixture.freshSelectionRequirements", errors);
  for (const [field, expected] of Object.entries({ offer: "fresh_opaque_observation", generation: "fresh_opaque_observation_matching_offer", objectiveSet: "frozen_dsm_finite_set_then_fresh_native_observation", reward: "frozen_dsm_content_then_fresh_native_observation", revalidation: "game_thread_before_each_typed_transaction" })) exactString(fixture?.freshSelectionRequirements?.[field], expected, `fixture.freshSelectionRequirements.${field}`, errors);
  ...
  ], "fixture.antiFinalStepAssertions", errors);
...
function issue(message) { return `m9-special-order-action-contract: ${message}`; }
async function readJson(file) {
  try { return JSON.parse(await readFile(file, "utf8")); }
  catch (error) { throw new Error(`Unable to read or parse ${file}: ${error.message}`); }
}
async function loadAndValidate(contractFile, root = process.cwd()) {
  ...
  const sourceFiles = Object.fromEntries(await Promise.all(audit.anchors.map(async (anchor) => [anchor.relativePath, await readFile(path.resolve(root, SOURCE_ROOT, anchor.relativePath))])));
  const auditResult = validatePortfolioM9SpecialOrderSourceAudit(audit, sourceFiles);
  if (audit.auditId !== contract.sourceAudit.auditId || audit.traceFamilyId !== contract.sourceAudit.traceFamilyId) throw new Error(issue("contract/audit identity mismatch."));
...
export async function checkM9SpecialOrderActionContract(contractFile, root = process.cwd()) { return loadAndValidate(contractFile, root); }
function parseContractArg(argv) {
  const index = argv.indexOf("--contract");
  if (index < 0 || !argv[index + 1] || argv[index + 1].startsWith("--") || argv.slice(index + 2).length) throw new Error("Usage: node tools/stardew-portfolio-m9-special-order-action-contract.mjs --contract <path>");
  return path.resolve(argv[index + 1]);
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { console.log(JSON.stringify(await checkM9SpecialOrderActionContract(parseContractArg(process.argv.slice(2))), null, 2)); }
  catch (error) { console.error(`${error.code ?? "m9_special_order_action_contract_check_failed"}: ${error.message}`); process.exitCode = 1; }
}
