...
async function travelFreshHop(snapshot, expectedOrigin, expectedLocation, phase) {
  ...
  requireExactCapabilities(after);
  if (after.location !== expectedLocation || after.tile.x !== warp.targetX || after.tile.y !== warp.targetY)
    throw new Error(`${phase}_travel_postcondition_missing`);
  return after;
}
async function moveToLiveWarp(expectedLocation, phase) {
  ...
  if (after.revision < terminal.revision || !adjacent(after.tile, warpSource(warp)))
...
async function moveToLiveTarget(target, phase) {
  ...
  const after = await observeActionable();
  requireExactCapabilities(after);
  if (after.revision < terminal.revision || !adjacent(after.tile, current))
    throw new Error(`${phase}_postcondition_missing`);
  return after;
}
async function waitForFreshActionablePostcondition(minRevision, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (
        String(error instanceof Error ? error.message : error) !== "native_local_npc_relationship_player_not_actionable"
      )
        throw error;
      latestError = error;
    }
...
async function observeActionable() {
  ...
  if (!snapshot.actionable || snapshot.activeExecution != null)
    throw new Error("native_local_npc_relationship_player_not_actionable");
  if (
    ...
    (snapshot.npcRelationshipTargets != null && !Array.isArray(snapshot.npcRelationshipTargets))
  )
...
function requireExactCapabilities(snapshot) {
  if (JSON.stringify([...snapshot.capabilities].sort()) !== JSON.stringify([...EXPECTED_CAPABILITIES].sort()))
    throw new Error(`native_local_npc_relationship_capability_not_isolated:${snapshot.capabilities.join(",")}`);
}
