...
const defaultCatalogPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../design/gameplay-capability-catalog.json");
...
const PROHIBITED_BRIDGE_TERMS = /(?:\bui\b|visual|window(?:\s+focus)?|keyboard|mouse|os input|input injection|raw coordinate|arbitrary (?:action|string|native))/i;
...
function validRoute(route, knownBasis) {
  ...
  if (route.kind === "primitive") {
    return typeof route.basisPrimitiveId === "string"
  ...
  if (route.kind === "composite") {
    return typeof route.compositeGraphRef === "string"
  ...
  return typeof route.coordinationRef === "string"
...
export function validateDirectGameplaySurfaceReport(report, catalog = null) {
  ...
  if (report?.target?.version !== "1.6.15" || report?.target?.build !== 24356 || report?.assembly?.fileVersion !== "1.6.15.24356") {
    errors.push("Surface report target does not match Stardew 1.6.15 build 24356.");
  }
  if (!report?.assembly?.sha256 || !Number.isInteger(report?.assembly?.lengthBytes) || report.assembly.lengthBytes <= 0) {
    errors.push("Surface report is missing the target assembly length/SHA-256 attestation.");
  }
  if (!report?.attestation?.extractedAtUtc || !report?.decompilation?.tool || !report?.decompilation?.toolVersion || !report?.decompilation?.configurationDigest || !report?.sourceSnapshot?.contentManifestSha256) {
  ...
  const bridgeEquivalenceGaps = bridgeEquivalenceAudit.filter((finding) => finding?.state !== "source_guard_equivalence_candidate");
  ...
  if (graph.schemaVersion !== 1 || !Array.isArray(graph.ingressRoots) || !Array.isArray(graph.reachableEdges) || !Array.isArray(graph.commandPaths)
    || !Array.isArray(graph.commandPathCandidates) || !Array.isArray(graph.supportingPaths) || !Array.isArray(graph.nonGameplayPaths)
    || !Array.isArray(graph.unknownReachableEdges) || !Array.isArray(graph.pendingCommandCandidates)) {
  ...
  const malformedReachableEdges = graph.reachableEdges.filter((edge) =>
    typeof edge?.edgeId !== "string" || edge.edgeId.length === 0
      ...
      || !["command_path", "supporting_path", "non_gameplay_path", "unknown", "candidate_dispatch_edge"].includes(edge?.classification),
  );
  const unresolvedReachableEdges = graph.reachableEdges.filter((edge) =>
    edge?.classification === "unknown" || edge?.classification === "candidate_dispatch_edge",
  );
  ...
  const candidateIds = graph.commandPathCandidates.map((candidate) => candidate?.candidateId).filter((id) => typeof id === "string");
  const duplicateCommandPathCandidateIds = candidateIds.filter((id, index) => candidateIds.indexOf(id) !== index);
  const malformedCommandPathCandidates = graph.commandPathCandidates.filter((candidate) =>
    typeof candidate?.candidateId !== "string" || candidate.candidateId.length === 0
      || typeof candidate?.ingressId !== "string" || candidate.ingressId.length === 0
      || typeof candidate?.semanticFamily !== "string" || candidate.semanticFamily.length === 0
      || typeof candidate?.nativeRuleBoundaryCandidate !== "string" || candidate.nativeRuleBoundaryCandidate.length === 0
      ...
      || typeof candidate.sourceEvidence?.sourceType !== "string" || candidate.sourceEvidence.sourceType.length === 0
      || typeof candidate.sourceEvidence?.sourceFile !== "string" || candidate.sourceEvidence.sourceFile.length === 0
      || typeof candidate.sourceEvidence?.sourceMethod !== "string" || candidate.sourceEvidence.sourceMethod.length === 0
      || !arrayOfStrings(candidate.sourceEvidence?.requiredFragments)
      || !candidate.sourceEvidence?.anchorPositions || typeof candidate.sourceEvidence.anchorPositions !== "object",
  );
  const duplicateCommandPathIds = commandPaths
    .map((path) => path?.prcpId)
    .filter((id) => typeof id === "string")
    .filter((id, index, all) => all.indexOf(id) !== index);
  const malformedCommandPaths = commandPaths.filter((commandPath) =>
    typeof commandPath?.prcpId !== "string" || commandPath.prcpId.length === 0
      || typeof commandPath?.ingressId !== "string" || commandPath.ingressId.length === 0
      || typeof commandPath?.nativeRuleBoundary !== "string" || commandPath.nativeRuleBoundary.length === 0
  ...
  const malformedSupportingPaths = graph.supportingPaths.filter((path) =>
    typeof path?.reason !== "string" || path.reason.length === 0
      || (graph.state === "complete" && (typeof path?.parentCommandPathId !== "string" || !commandPathIds.has(path.parentCommandPathId))),
  );
  const malformedNonGameplayPaths = graph.nonGameplayPaths.filter((path) =>
    typeof path?.reason !== "string" || path.reason.length === 0,
  );

  if (graph.state !== "complete") errors.push(`Player-command graph state is ${graph.state ?? "unknown"}, not complete.`);
  if (ingressUnknown.length) errors.push(`${ingressUnknown.length} normal-player ingress roots are unclassified.`);
  if (malformedReachableEdges.length) errors.push(`${malformedReachableEdges.length} ingress-reachable graph edges have invalid identity or classification.`);
  if (unresolvedReachableEdges.length) errors.push(`${unresolvedReachableEdges.length} ingress-reachable graph edges are unresolved.`);
  if (unknownReachableEdges.length) errors.push(`${unknownReachableEdges.length} ingress-reachable graph edges are unknown.`);
  if (pendingCommandCandidates.length) errors.push(`${pendingCommandCandidates.length} normal-player command candidates have not reached a native rule boundary.`);
  if (malformedCommandPathCandidates.length) errors.push(`${malformedCommandPathCandidates.length} source-derived command-boundary candidates have invalid identity or native boundary evidence.`);
  if (duplicateCommandPathCandidateIds.length) errors.push(`Duplicate source-derived command-boundary candidate IDs: ${[...new Set(duplicateCommandPathCandidateIds)].join(", ")}.`);
  if (malformedCommandPaths.length) errors.push(`${malformedCommandPaths.length} command paths lack a canonical PRCP identity, native boundary, or typed bridge equivalence route.`);
  if (duplicateCommandPathIds.length) errors.push(`Duplicate canonical PRCP IDs: ${[...new Set(duplicateCommandPathIds)].join(", ")}.`);
  if (malformedSupportingPaths.length) errors.push(`${malformedSupportingPaths.length} supporting paths lack an auditable parent command path or reason.`);
  if (malformedNonGameplayPaths.length) errors.push(`${malformedNonGameplayPaths.length} non-gameplay paths lack a source classification reason.`);
  if (prohibitedBridgeRoutes.length) errors.push(`${prohibitedBridgeRoutes.length} command paths contain prohibited UI/input/raw-dispatch bridge terms.`);
  if (bridgeEquivalenceGaps.length) errors.push(`${bridgeEquivalenceGaps.length} source-derived bridge equivalence audits have unresolved guard gaps.`);
...
async function main() {
  ...
  if (!reportPath || process.argv.includes("--help") || process.argv.includes("-h")) {
    process.stderr.write("Usage: node tools/check-stardew-gameplay-completeness.mjs --report <direct-inspector-report.json> [--catalog <catalog.json>]\n");
  ...
  try {
    ...
    if (result.errors.length) fail("player_command_graph_diagnostic_incomplete", "Direct target-game inspection found an intentionally incomplete PRCP research graph; this does not decide the gameplay capability set.", result.details);
    process.stdout.write(`${JSON.stringify({ state: "passed", diagnosticState: "bounded_native_audit_complete", capabilitySetDecision: "not_decided_by_this_diagnostic", ...result.details })}\n`);
    return 0;
  } catch (error) {
