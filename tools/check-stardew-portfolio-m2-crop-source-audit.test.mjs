...
const root = resolve(".");
const script = resolve("tools/check-stardew-portfolio-m2-crop-source-audit.mjs");
...
test("M2 source-audit CLI verifies authority, recorded provenance metadata, and bounded anchors without claiming realization", async () => {
  const result = await run();
  assert.equal(result.code, 0);
  const output = JSON.parse(result.stdout);
  assert.equal(output.anchorCount, 9);
