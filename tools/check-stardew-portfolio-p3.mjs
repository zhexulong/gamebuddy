...
const isolationPath = resolve(root, "tools/check-stardew-portfolio-isolation.mjs");
const [contract, contractTest, packageSource, isolation] = await Promise.all([
  readFile(contractPath, "utf8"),
  readFile(contractTestPath, "utf8"),
  readFile(packagePath, "utf8"),
  readFile(isolationPath, "utf8"),
]);
const failures = [];
const required = [
  "PORTFOLIO_TOPOLOGY",
...
for (const forbidden of [
  "local-stardew-bridge",
  "portfolio-stardew-bridge",
  "action-registry",
  "SaveGame.Load",
  "Game1.player",
  "spawn",
...
if (!packageSource.includes('"test:stardew-portfolio-p3": "node --test tools/stardew-portfolio-contracts.test.mjs"'))
  failures.push("portfolio_p3_test_script_missing");
if (!packageSource.includes('"check:stardew-portfolio-p3": "node tools/check-stardew-portfolio-p3.mjs"'))
  failures.push("portfolio_p3_check_script_missing");
if (!isolation.includes("check-stardew-portfolio-p3.mjs")) failures.push("portfolio_p3_isolation_not_chained");
    ...
    JSON.stringify(
      {
        ...
        phase: "P3_contract_schema_and_ledger_only",
        failures,
      },
      null,
      2,
