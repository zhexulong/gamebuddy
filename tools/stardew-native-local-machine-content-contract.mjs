...
export function validateProbeShapes(probe) {
  ...
  for (const evidence of machine.shapeEvidence) {
    const validPath = typeof evidence?.path === "string" && /^machine\[\(BC\)12\]\.outputRules\[\d+\]\.outputs\[\d+\]\.(stackModifiers|qualityModifiers|priceModifiers|modData|customData)$/.test(evidence.path);
    if (!evidence || evidence.status !== "documented_absent" || !allowed.test(evidence.member) || evidence.sourceOptional !== true || evidence.runtimeType !== null || typeof evidence.declaredType !== "string" || evidence.declaredType.length === 0 || !validPath || evidence.path.split(".").pop()?.toLowerCase() !== evidence.member.toLowerCase()) return "machine_shape_evidence_unknown";
  }
  return null;
}
...
const { stdout } = await execFileAsync("dotnet", ["run", "--project", probeProject, "--", resolve(gamePath)], { encoding: "utf8", maxBuffer: 8 * 1024 * 1024 });
const probe = JSON.parse(stdout);
const machines = probe?.machinesContent;
if (machines?.state !== "loaded" || machines.digest !== EXPECTED_MACHINE_DIGEST || !Array.isArray(machines.entries)) fail("machines_content_probe_invalid");
const shapeFailure = validateProbeShapes(probe);
if (shapeFailure) fail(shapeFailure);
const requiredMachineFields = ["unknownFields", "additionalConsumedItems", "preventTimePass", "readyTimeModifiers", "readyTimeModifierMode", "clearContentsOvernightCondition", "interactMethod"];
const requiredRuleFields = ["unknownFields", "triggers", "outputs", "minutesUntilReady", "daysUntilReady", "invalidCountMessage", "useFirstValidOutput", "recalculateOnCollect"];
const requiredOutputFields = ["unknownFields", "customData", "copyColor", "copyPrice", "copyQuality", "preserveType", "preserveId", "incrementMachineParentSheetIndex", "priceModifiers", "priceModifierMode"];
function requireFields(value, fields, reason) { if (!value || fields.some((field) => !Object.prototype.hasOwnProperty.call(value, field))) fail(reason); }

const machine = machines.entries.find((entry) => entry?.machineId === CONTRACT.machineId);
requireFields(machine, requiredMachineFields, "machine_rule_container_fields_missing");
if (!machine || machine.unknownFields?.length) fail(machine ? "machine_rule_container_unknown_fields" : "machine_rule_container_missing");
if (machines.entries.length !== CONTRACT.targetMachineEntryCount) fail("machines_entry_count_mismatch");
const configuredInteractMethods = machines.entries.filter((entry) => typeof entry?.interactMethod === "string" && entry.interactMethod.length > 0);
if (configuredInteractMethods.length !== CONTRACT.targetMachineInteractMethodCount) fail("machine_interact_method_universe_changed");
...
if (!rule || rule.unknownFields?.length || rule.minutesUntilReady !== CONTRACT.minutesUntilReady || rule.daysUntilReady !== CONTRACT.daysUntilReady || rule.invalidCountMessage !== CONTRACT.invalidCountMessage || rule.recalculateOnCollect !== CONTRACT.recalculateOnCollect || rule.useFirstValidOutput !== CONTRACT.useFirstValidOutput) fail(rule?.unknownFields?.length ? "machine_rule_unknown_fields" : "machine_rule_timing_mismatch");
if (JSON.stringify(rule.triggers) !== JSON.stringify([{ unknownFields: [], trigger: "ItemPlacedInMachine", requiredItemId: CONTRACT.input.qualifiedItemId, requiredCount: CONTRACT.input.requiredCount, requiredTags: [], condition: "" }])) fail("machine_rule_trigger_mismatch");
const expectedOutput = { itemId: CONTRACT.output.qualifiedItemId, ...Object.fromEntries(Object.entries(CONTRACT.output).filter(([key]) => key !== "qualifiedItemId")) };
...
if (JSON.stringify(rule.outputs?.map(({ unknownFields, ...output }) => output)) !== JSON.stringify([expectedOutput])) fail("machine_rule_output_mismatch");
console.log(JSON.stringify({ state: "verified", artifactKind: "stardew_native_local_machine_content_contract_v2", xnbPath: "Content/Data/Machines.xnb", xnbSha256, machineTableDigest: machines.digest, contract: CONTRACT }));
...
function fail(reasonCode) { console.error(JSON.stringify({ state: "blocked", reasonCode })); process.exit(2); }
}
