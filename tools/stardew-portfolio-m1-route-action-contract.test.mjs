...
import {
  checkM1RouteBlockerFixture,
  validateM1RouteBlocker,
  validateM1RouteBlockerFixture,
} from "./stardew-portfolio-m1-route-action-contract.mjs";
...
test("M1 Given → When → Then preserves the source-bound blocker producer, typed composition consumer, and exact verifier handoff", () => {
  ...
  assert.deepEqual(result, {
    ...
    producer: "portfolio_m1_route_source_audit_v1",
    consumer: "m1_leave_and_return_route",
...
test("M1 rejects generic travel/warp widening, stale Given facts, and synthetic success", () => {
  assert.equal(
    validateM1RouteBlocker({
      request: { ...request, composition: ["travel"] },
      checkpoint,
      receipt,
      expectedScope: scope,
    }).code,
    "m1_route_request_invalid",
  );
  assert.equal(
    validateM1RouteBlocker({ request, checkpoint: { ...checkpoint, fresh: false }, receipt, expectedScope: scope })
      .code,
    "m1_route_given_invalid",
  );
  assert.equal(
    validateM1RouteBlocker({ request, checkpoint, receipt: { ...receipt, state: "succeeded" }, expectedScope: scope })
      .code,
    "m1_route_blocker_handoff_invalid",
  );
});
test("M1 fixture is non-mutating and cannot provide an outcome, checkpoint, receipt, or native evidence", async () => {
  const fixture = JSON.parse(
    await readFile(
      new URL("../fixtures/stardew/portfolio-m1-route-contract.unprovisioned.json", import.meta.url),
      "utf8",
    ),
  );
