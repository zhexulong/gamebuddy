...
import { loadKnowledgeBundle } from "../host/dist/knowledge.js";
...
const runtimeRoot = process.argv.includes("--runtime-root") ? option("--runtime-root") : await mkdtemp(join(tmpdir(), "gamebuddy-stardew-agent-"));
...
try {
  ...
  if ((knowledgeBundlePath === undefined) !== (gameVersion === undefined)) throw new Error("knowledge_version_required");
  const knowledge = knowledgeBundlePath === undefined ? undefined : await loadKnowledgeBundle(resolve(knowledgeBundlePath), gameVersion);
  ...
  const unsubscribeAgent = connected.runtime.session.subscribe((event) => {
    ...
    if (event.type === "tool_execution_end") {
      ...
      toolResults.push({
        ...
        text: Array.isArray(result?.content) ? result.content.filter((part) => part?.type === "text").map((part) => redactError(part.text)).join(" ").slice(0, 512) : null,
      });
    }
    ...
    if (event.type === "agent_settled") {
      ...
      settledCount += 1;
      agentSettled.push({ count: settledCount, isIdle: connected.runtime.session.isIdle, error: sessionError === null ? null : redactError(sessionError) });
    }
  });
  try {
    ...
    if (agentTools.includes("stardew_equip_tool")) await waitForReceipt(connected.bridge, 30_000);
  } finally {
    unsubscribeFact();
    unsubscribeAgent();
  }
  ...
  const evidenceDetail = evidence !== null && evidence !== undefined && typeof evidence === "object" && typeof evidence.detail === "string" ? evidence.detail : "";
  const evidenceComplete = /(?:^|;)before=[^;]+;expected=[^;]+;after=[^;]+(?:;|$)/.test(evidenceDetail);
  const terminalReceipt = receipt !== null && !["accepted", "running", "meaningful_progress", "blocked"].includes(receipt.state);
  const tracePassed = traceOnly
  ...
  const summary = {
    ...
    model: modelId,
    thinkingLevel: "high",
    gameplaySubagent: process.argv.includes("--enable-gameplay-subagent") ? { provider: "cpa-oai", model: "gpt-5.6-luna", thinkingLevel: "medium" } : null,
  ...
  console.error(JSON.stringify({
    ...
    agentSettled,
    sessionError: sessionError === undefined || sessionError === null ? null : redactError(sessionError),
    bridge: { connected: connected?.bridge.state.connected ?? false, snapshotRevision: connected?.bridge.state.snapshot?.revision ?? null, latestReceipt: connected?.bridge.state.latestReceipt?.state ?? null, latestReasonCode: connected?.bridge.state.latestReasonCode ?? null },
  }));
  process.exitCode = 2;
} finally {
...
async function waitForReceipt(bridge, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (bridge.state.latestReceipt !== null) return bridge.state.latestReceipt;
    await new Promise((resolvePromise) => setTimeout(resolvePromise, 250));
  }
