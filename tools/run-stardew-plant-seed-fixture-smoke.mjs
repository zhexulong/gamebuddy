...
try {
  ...
  if (
    !snapshot.capabilities.includes("plant_seed") ||
    !snapshot.capabilities.includes("move_to_tile") ||
    !snapshot.capabilities.includes("travel")
  ) {
...
async function travelToFarm(snapshot) {
  ...
  const accepted = await client.execute({
    requestId: `plant_fixture_travel_${Date.now()}`,
    idempotencyKey: `plant_fixture_travel_idem_${Date.now()}`,
    action: "travel",
