...
try {
  ...
  if (!snapshot.capabilities.includes("plant_seed") || !snapshot.capabilities.includes("move_to_tile") || !snapshot.capabilities.includes("travel")) {
    throw new Error("fixture_navigation_or_plant_capability_missing");
  }
  if (snapshot.location !== "Farm") {
    snapshot = await travelToFarm(snapshot);
  }
  snapshot = await waitForActionable(snapshot, 5_000);
  const candidate = chooseEmptyDirtCandidate(snapshot);
  if (candidate === null) throw new Error("no_native_empty_dirt_candidate");
  snapshot = await moveToCandidate(snapshot, candidate);
  snapshot = await waitForActionable(snapshot, 3_000);
  const target = chooseSeedTarget(snapshot);
  if (target === null) {
    console.log(JSON.stringify({ state: "blocked", reasonCode: "no_live_seed_target", trace, snapshot: summarizeSnapshot(snapshot), durationMs: Date.now() - startedAt }));
    process.exitCode = 2;
  } else {
    const receipt = await client.execute({
      requestId: `plant_seed_fixture_${Date.now()}`,
      idempotencyKey: `plant_seed_fixture_idem_${Date.now()}`,
      action: "plant_seed",
    ...
    trace.push({ phase: "plant", receipt: summarizeReceipt(receipt), target });
    const after = await client.observe();
    const evidence = parseEvidence(receipt.evidence);
    const targetGone = after.seedTargets?.every((entry) => entry.targetId !== target.targetId) === true;
    const passed = receipt.state === "succeeded" && receipt.reasonCode === "seed_planted"
      && evidence.item === target.qualifiedItemId && evidence.crop && evidence.crop !== "none"
      && Number(evidence.inventory_after) === Number(evidence.inventory_before) - 1 && targetGone;
    console.log(JSON.stringify({ state: passed ? "passed" : "blocked", reasonCode: passed ? "seed_planted" : receipt.reasonCode, target, receipt: summarizeReceipt(receipt), evidence, targetGone, trace, before: summarizeSnapshot(snapshot), after: summarizeSnapshot(after), durationMs: Date.now() - startedAt }));
...
async function travelToFarm(snapshot) {
  ...
  const accepted = await client.execute({ requestId: `plant_fixture_travel_${Date.now()}`, idempotencyKey: `plant_fixture_travel_idem_${Date.now()}`, action: "travel", args: { x: warp.sourceX, y: warp.sourceY }, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 30_000 });
...
function chooseEmptyDirtCandidate(snapshot) {
  // The fixture's target-version RemoveDirt/SpreadDirt sequence deterministically
  // leaves this native ground dirt region empty; production still discovers its opaque target.
  return snapshot.location === "Farm" ? { x: 48, y: 31 } : null;
}
async function moveToCandidate(snapshot, candidate) {
  const adjacentTarget = { x: candidate.x, y: candidate.y + 1 };
  return move(snapshot, adjacentTarget, "move_to_native_empty_dirt");
}
async function move(snapshot, target, phase) {
  const accepted = await client.execute({ requestId: `${phase}_${Date.now()}`, idempotencyKey: `${phase}_idem_${Date.now()}`, action: "move_to_tile", args: target, expectedRevision: snapshot.revision, deadlineMs: Date.now() + 45_000 });
...
async function waitForActionable(snapshot, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    await delay(150); latest = await client.observe();
  }
  return latest;
}
async function waitForReceiptAndSnapshot(executionId, predicate, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const terminal = receipts.find((receipt) => receipt.executionId === executionId && receipt.state !== "accepted" && isTerminal(receipt.state));
...
function chooseSeedTarget(snapshot) {
...
function summarizeSnapshot(snapshot) { return { revision: snapshot.revision, location: snapshot.location, tile: snapshot.tile, actionable: snapshot.actionable, seedTargets: snapshot.seedTargets?.length ?? 0, activeExecution: snapshot.activeExecution ?? null }; }
