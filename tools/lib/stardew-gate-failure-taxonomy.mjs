...
const RULES = [
  {
    ...
    nextStep:
      "Inspect the allowlisted initializer and target-version native fixture facts; restore instead of starting an AI action run.",
  },
  {
    ...
    nextStep:
      "Inspect the signed readiness report, effective Host Mod config, session scope, and launch freshness before retrying attachment.",
  },
  {
    category: "attachment",
    ...
    nextStep:
      "Inspect effective SMAPI config, Host save/load, native LAN readiness, and Farmhand provisioning before running a production action.",
  },
  {
    ...
    nextStep:
      "Re-establish a formal ready bridge; do not infer action success after disconnect or replay a consumed request.",
  },
  {
    ...
    nextStep:
      "Inspect fresh snapshot discovery and fixture readiness facts; do not submit a production request without a live target.",
  },
  {
    ...
    nextStep:
      "Take a fresh snapshot and rediscover the target; audit discovery/execution timing if it repeatedly becomes stale.",
  },
  {
    ...
    nextStep:
      "Inspect request shape, live capability, policy, scope, and bridge protocol before retrying with a new request id.",
  },
  {
    category: "execution_lifecycle",
    pattern:
      /(?:terminal_timeout|navigation_timeout|_timeout$|^execution_(blocked|invalidated|cancelled|expired|rejected|uncertain))/,
  ...
  },
  {
    ...
    nextStep:
      "Keep the native receipt, then inspect the fresh post-receipt snapshot and action-specific evidence predicate.",
  },
  {
    ...
    nextStep:
      "Inspect the effective Host/AI Mod configs, scenario allowlist, live capability, and fixture profile transaction.",
  },
];
...
export function classifyStardewGateFailure(reasonCode, { nativeReasonCode = null } = {}) {
  ...
  return Object.freeze({
    ...
    nativeReasonCode: native,
    nextStep:
      "Preserve and inspect the authoritative native receipt/log evidence; do not infer a retry strategy from an unclassified reason.",
  });
}

function normalizeReasonCode(value) {
  return typeof value === "string" && /^[A-Za-z0-9_:-]{1,256}$/.test(value) ? value : "invalid_or_missing_reason_code";
}
