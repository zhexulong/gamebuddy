...
export function accountExactPaths({ paths, ownerForPath, kind }) {
  ...
  const rows = normalized.map((relativePath) => Object.freeze({ relativePath, ownerCluster: ownerForPath(relativePath) }));
  if (rows.some((row) => !row.ownerCluster)) fail("architecture_path_unaccounted", `An input ${kind} path has no ownership cluster.`);
  const clusters = new Map();
  for (const row of rows) clusters.set(row.ownerCluster, (clusters.get(row.ownerCluster) ?? 0) + 1);
  return Object.freeze({
    rows: Object.freeze(rows),
    clusters: Object.freeze([...clusters].map(([ownerCluster, pathCount]) => Object.freeze({ ownerCluster, pathCount })).sort((a, b) => a.ownerCluster.localeCompare(b.ownerCluster))),
...
function assertAnchoredRecord(record, sourceByPath, kind) {
  if (!record?.id || !record?.family || !record?.sourcePath || !record?.anchor) fail("architecture_register_invalid", `Invalid ${kind} register row.`, { record });
  const source = sourceByPath.get(record.sourcePath);
  if (!source) fail("architecture_register_source_missing", `${kind} source is outside the attested source universe.`, { id: record.id, sourcePath: record.sourcePath });
  if (!source.text.includes(record.anchor)) fail("architecture_register_anchor_missing", `${kind} anchor was not found in the exact target source.`, { id: record.id, sourcePath: record.sourcePath, anchor: record.anchor });
  return Object.freeze({ id: record.id, family: record.family, sourcePath: record.sourcePath, ownerCluster: sourceOwnerCluster(record.sourcePath), anchorSha256: sha256Text(record.anchor) });
}

export function deriveArchitectureAccounting({ sourceRecords, contentPaths, rootRegister, boundaryRegister, requiredRootFamilies }) {
  if (!Array.isArray(rootRegister) || !Array.isArray(boundaryRegister) || !Array.isArray(requiredRootFamilies)) fail("architecture_register_required", "Architecture registers and required root families must be arrays.");
  const sourceByPath = indexSources(sourceRecords);
  const sourceAccounting = accountExactPaths({ paths: [...sourceByPath.keys()], ownerForPath: sourceOwnerCluster, kind: "source" });
