...
export function computePortfolioBindingHash({ saveId, worldId, localPlayerId, companionId, bindingGeneration }) {
  if (![saveId, worldId, localPlayerId, companionId].every((value) => /^[A-Za-z0-9_-]{1,128}$/.test(value ?? ""))
    || !Number.isSafeInteger(bindingGeneration) || bindingGeneration <= 0) {
    throw new Error("portfolio_binding_hash_input_invalid");
  }
  return createHash("sha256").update(`${PORTFOLIO_TOPOLOGY}\n${saveId}\n${worldId}\n${localPlayerId}\n${companionId}\n${PORTFOLIO_TARGET_VERSION}\n${PORTFOLIO_TARGET_BUILD_NUMBER}\n${bindingGeneration}`, "utf8").digest("hex");
}
