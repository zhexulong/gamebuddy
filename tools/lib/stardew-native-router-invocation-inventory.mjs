...
function exactMethod(source, relativePath, signature) {
  if (
    ...
    fail("router_inventory_argument_invalid", "source, relativePath, and signature are required.");
  const start = source.indexOf(signature);
  if (start < 0 || source.indexOf(signature, start + signature.length) >= 0)
    fail("router_inventory_method_anchor_missing", "Method signature must occur exactly once.", { signature });
  const brace = source.indexOf("{", start + signature.length);
  if (brace < 0) fail("router_inventory_method_malformed", "Method signature lacks an opening brace.", { signature });
  let depth = 0;
  let state = "code";
  for (let index = brace; index < source.length; index += 1) {
    ...
    if (char === "}" && --depth === 0) {
      ...
      const endByte = Buffer.byteLength(source.slice(0, index + 1), "utf8");
      ...
      return Object.freeze({
        ...
        sourceFileSha256: sourceHash(source),
      });
    }
  }
  fail("router_inventory_method_malformed", "Method body is not closed.", { signature });
}
function belongsTo(locator, owner) {
  return (
    locator.relativePath === owner.relativePath &&
    locator.startByte >= owner.startByte &&
    locator.endByte <= owner.endByte
  );
}
function overlaps(locator, owner) {
  return (
    locator.relativePath === owner.relativePath &&
    locator.startByte < owner.endByte &&
    locator.endByte > owner.startByte
  );
}
...
export async function deriveNativeRouterInvocationInventory({ source, relativePath, signature } = {}) {
  ...
  const routerParseGaps = [...parsed.parse.errorNodes, ...parsed.parse.missingNodes]
    .filter((entry) => overlaps(entry, owner))
    .map((entry) => Object.freeze({ ...entry, sourceFileSha256: owner.sourceFileSha256 }));
  const invocations = parsed.invocationSyntax
    .filter((entry) => belongsTo(entry.locator, owner))
    .map((entry, sequence) =>
      Object.freeze({
        ...
        calleeLocator: entry.calleeLocator
          ? Object.freeze({ ...entry.calleeLocator, sourceFileSha256: owner.sourceFileSha256 })
          : null,
  ...
  return Object.freeze({
    schemaVersion: 1,
    artifactKind: "native_router_invocation_inventory",
    routerDeclaration: owner,
    parser: parsed.parser,
    parse: parsed.parse,
    routerParseGaps: Object.freeze(routerParseGaps),
    syntaxInventoryState,
    invocationCount: invocations.length,
    invocations: Object.freeze(invocations),
    analysisBoundary: Object.freeze({
      syntaxInvocationInventory: "performed",
      sourceCompleteness:
        syntaxInventoryState === "partial_syntax_only" ? "blocked_by_exact_router_parse_gap" : "not_inferred",
      overloadResolution: "not_performed",
      targetResolution: "not_performed",
      inputProvenance: "not_performed",
