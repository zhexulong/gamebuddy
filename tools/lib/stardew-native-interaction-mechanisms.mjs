...
const CONTENT_INTERPRETER_PATTERNS = Object.freeze([
  ...
  Object.freeze({
    id: "content:load",
    expression: /\b(?:[A-Za-z_]\w*\.)?(?:temporaryContent|content|festivalContent)\.Load(?:String)?(?:<[^>]+>)?\s*\(/g,
  }),
  Object.freeze({
    id: "content:map-property",
    expression: /\b(?:doesTileHaveProperty|GetTilePropertySplitBySpaces|TileIndexProperties|TouchAction)\b/g,
  }),
  Object.freeze({
    id: "content:event",
    expression:
      /\b(?:SetupEventCommandsIfNeeded|TryGetEventCommandHandler|tryEventCommand|ParseCommands|RegisterCommand)\b/g,
  }),
  Object.freeze({
    id: "content:trigger",
    expression: /\b(?:TriggerActionManager\.|TryRunAction|TryRunActions|GetActionsByTrigger|ParseAction)\b/g,
  }),
...
const CONTINUATION_PATTERNS = Object.freeze([
  ...
  Object.freeze({
    id: "continuation:event-menu-minigame",
    expression: /\b(?:currentEvent|activeClickableMenu|currentMinigame|IClickableMenu|IMinigame)\b/g,
  }),
  Object.freeze({
    id: "continuation:network",
    expression: /\b(?:Multiplayer|receiveMessages|sendMessages|NetEvent\w*|\.Poll\s*\()\b/g,
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    expression: /\b(?:Activator\.CreateInstance|Type\.GetType\s*\(|\.GetMethods\s*\()\b/g,
  }),
]);
...
function ownerFor(declarations, locator) {
  const owners = declarations
    .filter((item) =>
      ["class_declaration", "struct_declaration", "interface_declaration", "record_declaration"].includes(
        item.declarationSyntaxKind,
      ),
    )
    .filter((item) => item.locator.startByte <= locator.startByte && item.locator.endByte >= locator.endByte)
    .sort(
      (left, right) =>
        left.locator.endByte - left.locator.startByte - (right.locator.endByte - right.locator.startByte),
    );
...
function structuralMechanisms(parsed, fileSyntaxState) {
  return parsed.declarations
    .filter((item) =>
      ["interface_declaration", "event_declaration", "event_field_declaration"].includes(item.declarationSyntaxKind),
    )
    .map((item) =>
      Object.freeze({
...
function hostMechanisms(parsed, fileSyntaxState) {
  ...
  return parsed.declarations
    .filter(
      (item) => item.declarationSyntaxKind === "method_declaration" && HOST_MEMBER_NAMES.has(item.identifierSyntax),
    )
    ...
    .map((item) => {
      ...
      return Object.freeze({
        ...
        declarationShape: item.modifiersSyntax.some((modifier) => ["virtual", "override", "new"].includes(modifier))
          ? "polymorphic-syntax"
          : "non-polymorphic-syntax",
        fileSyntaxState,
    ...
    for (const match of source.matchAll(pattern.expression)) {
      ...
      const memberOwner =
        declarations
          ...
          .filter(
            (item) =>
              item.locator.startByte <= sourceLocator.startByte && item.locator.endByte >= sourceLocator.endByte,
          )
          .sort(
            (left, right) =>
              left.locator.endByte - left.locator.startByte - (right.locator.endByte - right.locator.startByte),
...
export async function enumerateNativeInteractionMechanisms(sourceRecords) {
  if (!Array.isArray(sourceRecords) || sourceRecords.length === 0)
    fail("mechanism_source_records_required", "Provide non-empty exact source records.");
  const sourcePaths = new Set();
  const mechanisms = [];
  const parseGaps = [];
  let parser = null;
  for (const record of [...sourceRecords].sort((left, right) => left.relativePath.localeCompare(right.relativePath))) {
    if (!record || typeof record.relativePath !== "string" || typeof record.text !== "string")
      fail("mechanism_source_record_invalid", "Invalid source record.");
    ...
    const fileSyntaxState =
      parsed.parse.hasError || parsed.parse.errorNodes.length || parsed.parse.missingNodes.length
        ? "partial_syntax_only"
        : "parse_clean";
    ...
    mechanisms.push(
      ...patternMechanisms(
        sourcePath,
        record.text,
        CONTENT_INTERPRETER_PATTERNS,
        "content-interpreter-syntax",
        fileSyntaxState,
        parsed.declarations,
      ),
    );
  ...
  for (const mechanism of mechanisms) {
    ...
    if (prior && JSON.stringify(prior) !== JSON.stringify(mechanism))
      fail("mechanism_id_duplicate", "Mechanism enumeration generated conflicting identities.", {
        mechanismId: mechanism.mechanismId,
      });
    uniqueMechanisms.set(mechanism.mechanismId, mechanism);
  }
  const ordered = [...uniqueMechanisms.values()].sort((left, right) =>
  ...
  return Object.freeze({
    ...
    familyCounts: Object.freeze(
      [...byFamily]
        .map(([family, count]) => Object.freeze({ family, count }))
        .sort((a, b) => a.family.localeCompare(b.family)),
    ),
    analysisBoundary: Object.freeze({
      sourceSyntaxEnumeration: "performed",
