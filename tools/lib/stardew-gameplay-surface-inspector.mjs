...
function typeBlock(source, fullTypeName) {
  ...
  const pattern = new RegExp(`(?:class|struct|interface|enum)\\s+${shortName}\\b[\\s\\S]*?(?=\\n(?:public |internal |private |protected |class |struct |interface |enum )|\\s*$)`);
...
function methodsFromSource(source, fullTypeName) {
  ...
  const pattern = /\b(?:public|private|protected|internal|protected\s+internal|private\s+protected)?\s*(?:static\s+|virtual\s+|override\s+|abstract\s+|sealed\s+|async\s+)*[A-Za-z_][\w<>,.?\[\]]*\s+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:\{|=>)/g;
...
async function inspectInstalledAssembly(gamePath) {
  ...
  let file;
  try { file = await stat(assemblyPath); } catch { fail("target_assembly_missing", `Missing ${ASSEMBLY_NAME} under the supplied game path.`); }
  ...
  try {
    ...
    fail("assembly_metadata_unavailable", `Could not read Windows file metadata for ${ASSEMBLY_NAME}.`, { cause: error.message });
  }
  const versions = parseVersion(resourceText);
  if (versions.fileVersion !== `${EXPECTED_VERSION}.${EXPECTED_BUILD}`) {
    fail("target_installation_mismatch", `Expected Stardew ${EXPECTED_VERSION} build ${EXPECTED_BUILD}; got file version ${versions.fileVersion ?? "unknown"}.`, versions);
  }
  return {
...
async function decompileAssembly(assemblyPath) {
  ...
  try {
    ...
    fail("assembly_decompilation_failed", "Could not decompile the supplied target assembly with ilspycmd.", { cause: error.message });
  }
}
...
async function inspectSourceSnapshot(sourceRoot) {
  ...
  return { root: sourceRoot, fileCount: records.length, contentManifestSha256: createHash("sha256").update(`${records.join("\n")}\n`).digest("hex") };
}

const contentProbeProject = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../stardew-content-probe/ContentProbe.csproj");
...
async function probeTargetContent(gamePath) {
  try {
    ...
    if (parsed?.state !== "probed" || parsed?.gameAssemblyVersion !== EXPECTED_ASSEMBLY_VERSION || !Array.isArray(parsed?.tables)) {
...
function classifyProbedTables(probe) {
  return probe.tables.map((table) => {
    const classification = table.state === "loaded"
...
async function inspectContent(gamePath) {
  ...
  let contentHashes = {};
  try { contentHashes = JSON.parse(await readFile(hashesPath, "utf8")); } catch { fail("content_manifest_missing", "Missing or invalid Content/ContentHashes.json."); }
  ...
  const relevantAssetManifestSha256 = createHash("sha256").update(`${relevantAssets.join("\\n")}\\n`).digest("hex");
  ...
  for (const assetPath of relevantAssets) {
    ...
    const entry = operationFamilies.get(operationFamily) ?? { operationFamily, assetCount: 0, sampleAssets: [], mappingStatus: "needs_expansion", semanticKinds: new Set(), keyDomains: [] };
    entry.assetCount += 1;
    if (entry.sampleAssets.length < 8) entry.sampleAssets.push(assetPath);
    const domain = contentOperationDomain(assetPath);
    if (!entry.keyDomains.some((candidate) => candidate.domainKind === domain.domainKind && candidate.keyDomain === domain.keyDomain)) {
  ...
  const contentOperationFamilies = [...operationFamilies.values()]
    .map(({ semanticKinds, ...entry }) => ({
      ...entry,
      keyDomains: entry.keyDomains.sort((left, right) => `${left.domainKind}:${left.keyDomain}`.localeCompare(`${right.domainKind}:${right.keyDomain}`)),
...
export async function inspectStardewGameplaySurface({ gamePath } = {}) {
  if (!gamePath) fail("game_path_required", "Provide gamePath or GAMEBUDDY_STARDEW_GAME_PATH; no implicit installation scan is allowed.");
  ...
  try {
    ...
    for (const source of sourceFiles) {
      ...
      for (const methodName of methods) {
        ...
        if (classification) {
          ...
          members.push(member);
          if (classification.mappingStatus === "needs_expansion" && ["performAction", "answerDialogueAction", "answerDialogue", "checkAction"].includes(methodName)) {
            for (const selector of extractLiteralOperationSelectors(sourceText, methodName)) {
              members.push({
                ...
                basisPrimitiveIds: [],
                semanticKind: methodName.startsWith("answerDialogue") ? "dialogue_operation" : "native_operation_selector",
    ...
    const sourceIndex = Object.fromEntries(await Promise.all(
      sourceFiles
        .filter((source) => relevantGraphTypes.has(source.typeName))
        .map(async (source) => [source.typeName, { source: await readFile(source.file, "utf8"), sourceFile: source.sourceFile }]),
    ));
    const playerCommandGraph = game1Source
      ...
      : {
        ...
        ingressRoots: [], commandPaths: [], supportingPaths: [], nonGameplayPaths: [],
        unknownReachableEdges: [{ reason: "game1_source_missing" }], pendingCommandCandidates: [],
      };
    const bridgeSource = await readFile(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../integrations/stardew/ExecutionManager.cs"), "utf8");
