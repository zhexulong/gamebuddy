...
const EXPECTED_ANCHORS = Object.freeze(
  new Map([
    [
      ...
      ["special_order_offer_acceptance_commit", "StardewValley.Menus/SpecialOrdersBoard.cs"],
    ],
    [
      "special_order_objective_registration",
      ["special_order_objective_registration", "StardewValley.SpecialOrders/SpecialOrder.cs"],
    ],
    [
      "special_order_objective_progress_commit",
      ["special_order_objective_progress_commit", "StardewValley.SpecialOrders.Objectives/OrderObjective.cs"],
    ],
    [
      "special_order_completion_reward_availability",
      ["special_order_completion_reward_availability", "StardewValley.SpecialOrders/SpecialOrder.cs"],
    ],
    [
      "special_order_completion_state_commit",
      ["special_order_completion_state_commit", "StardewValley.SpecialOrders/SpecialOrder.cs"],
    ],
    [
      "special_order_reward_claim_consumption",
      ["special_order_reward_claim_consumption_commit", "StardewValley.SpecialOrders/SpecialOrder.cs"],
    ],
    ["special_order_reward_grant", ["special_order_reward_grant", "StardewValley.SpecialOrders/SpecialOrder.cs"]],
    [
      "special_order_failure_state",
      ["special_order_failure_state_commit", "StardewValley.SpecialOrders/SpecialOrder.cs"],
    ],
  ]),
);
...
function record(value, name) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    fail(`${name} must be an object.`, "portfolio_m9_source_audit_invalid");
  return value;
}
function exactKeys(value, expected, name) {
  ...
  if (actual.length !== wanted.length || actual.some((key, index) => key !== wanted[index]))
    fail(`${name} has unknown or missing fields.`, "portfolio_m9_source_audit_invalid");
}
function text(value, name) {
  if (typeof value !== "string" || value.trim() === "")
    fail(`${name} must be a non-empty string.`, "portfolio_m9_source_audit_invalid");
  return value;
}
function hash(bytes) {
  return createHash("sha256").update(bytes).digest("hex");
}
/** Revalidates a deliberately non-publishable M9 local source-snapshot audit aid. */
export function validatePortfolioM9SpecialOrderSourceAudit(model, sourceFiles) {
  exactKeys(
    model,
    [
      "schemaVersion",
  ...
  if (
    ...
    model.auditId !== "portfolio_m9_special_order_source_audit_v1" ||
    model.traceFamilyId !== "portfolio_m9_special_order_to_reward" ||
    model.topology !== "single_player_native_companion"
  )
    fail("M9 source audit identity/topology is invalid.", "portfolio_m9_source_audit_invalid");
  ...
  if (
    ...
    fail("M9 source audit is not bound to the Portfolio Charter authority.", "portfolio_m9_source_audit_invalid");
  exactKeys(
    model.auditSource,
  ...
  if (
    ...
    fail(
      "M9 source audit must retain its unverified audit-aid boundary.",
      "portfolio_m9_source_audit_boundary_invalid",
    );
  if (!Array.isArray(model.anchors) || model.anchors.length !== EXPECTED_ANCHORS.size)
    fail("M9 source audit must retain its complete focused anchor set.", "portfolio_m9_source_audit_anchor_invalid");
  const ids = new Set();
  for (const anchor of model.anchors) {
    ...
    if (
      ids.has(id) ||
      !expected ||
      expected[0] !== anchor.semanticRole ||
      expected[1] !== anchor.relativePath ||
      ...
      fail("M9 source audit anchor identity or shape is invalid.", "portfolio_m9_source_audit_anchor_invalid");
    ids.add(id);
    const bytes = sourceFiles?.[anchor.relativePath];
    if (!Buffer.isBuffer(bytes))
      fail(
        `M9 source audit source is unavailable: ${anchor.relativePath}.`,
        "portfolio_m9_source_audit_source_missing",
      );
    ...
    if (
      ...
      fail(`M9 source audit anchor drifted: ${id}.`, "portfolio_m9_source_audit_anchor_drift");
  }
  if (ids.size !== EXPECTED_ANCHORS.size)
    fail("M9 source audit anchor set is incomplete.", "portfolio_m9_source_audit_anchor_invalid");
  if (
    ...
    fail(
      "M9 source audit must preserve semantic constraints and unresolved blockers.",
      "portfolio_m9_source_audit_invalid",
    );
  for (const question of model.unresolvedQuestions) {
    ...
    if (!["blocks_projection", "requires_signed_dsm_selection"].includes(question.disposition))
      fail("M9 source audit has an invalid unresolved-question disposition.", "portfolio_m9_source_audit_invalid");
  }
  ...
  if (
    ...
    fail(
      "M9 source audit cannot be promoted to realization, projection, or live closure.",
      "portfolio_m9_source_audit_boundary_invalid",
    );
