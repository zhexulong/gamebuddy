...
const [
  { createDeterministicBridgePair },
  { CompanionIntegrationClient },
  { STARDEW_INTEGRATION_MODULE },
  { newEnvelope },
  { createCompanionRuntime },
] = await Promise.all([
  loadSelectedHostProductionModule(selectedProductionArtifact, "bridge.js"),
  loadSelectedHostProductionModule(selectedProductionArtifact, "integration.js"),
  loadSelectedHostProductionModule(selectedProductionArtifact, "stardew-integration-module.js"),
  loadSelectedHostProductionModule(selectedProductionArtifact, "protocol.js"),
...
const scope = Object.freeze({
  ...
  saveId: "save_trace",
  worldId: "world_trace",
  playerId: "farmhand_trace",
  companionId: "companion_trace",
});
  ...
  if (message.type === "hello") {
    mod.send(
      newEnvelope(
        "hello_ack",
        scope,
        { sessionId: "subagent_trace_session", capabilities: ["equip_tool"] },
...
try {
  ...
  const passed =
    runtime.session.agent.state.model?.provider === "cpa-oai" &&
  ...
  console.log(
    JSON.stringify({
      ...
      parentModel: {
        provider: runtime.session.agent.state.model?.provider ?? null,
        model: runtime.session.agent.state.model?.id ?? null,
        thinkingLevel: runtime.session.agent.state.thinkingLevel,
      },
      gameplaySubagentModel: child.modelConfig,
      parentTools,
      childPresentationToolsExposedToParent: parentTools.filter(
        (name) => name === "companion_speak" || name === "companion_text",
      ),
        ...
        receipt === null
          ...
          : {
              ...
              evidenceComplete:
                ...
                ["before", "expected", "after"].every(
                  (key) => typeof receipt.evidence?.[key] === "string" && receipt.evidence[key].length > 0,
                ),
            },
  ...
  console.error(
    JSON.stringify({
      state: "blocked",
      reasonCode:
        error instanceof Error ? error.message.replace(/\s+/g, " ").slice(0, 256) : "gameplay_subagent_trace_failed",
    }),
  );
  process.exitCode = 1;
} finally {
