...
async function waitForFreshActionablePostcondition(timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (String(error instanceof Error ? error.message : error) !== "native_local_forage_player_not_actionable")
        throw error;
      await delay(100);
    }
  }
...
function requireActionableSnapshot(snapshot) {
  if (!snapshot.actionable || snapshot.activeExecution != null)
    throw new Error("native_local_forage_player_not_actionable");
  if (
...
function requireExactCapabilities(snapshot) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected))
    throw new Error("native_local_forage_capability_not_isolated");
}
...
function snapshotSummary(snapshot) {
  return {
    ...
    forageTargets: snapshot.forageTargets?.map(targetSummary) ?? [],
