...
const EVIDENCE_KEYS = Object.freeze([
  ...
  "quality",
  "rewardId",
  "rewardInventoryIdentity",
  "rewardInventoryStackBefore",
  "rewardInventoryStackAfter",
  "mutexBefore",
  "mutexAfter",
  "slotBefore",
  "slotAfter",
...
function sameScope(actual, expected) {
  return (
    exact(actual, SCOPE_KEYS) &&
    exact(expected, SCOPE_KEYS) &&
    SCOPE_KEYS.every((field) => actual[field] === expected[field])
  );
}
...
function validIdentity(value) {
  return record(value) && validId(value.requestId) && validId(value.traceId) && validId(value.executionId);
}
...
export function validateM7ClaimGiven(given, expectedScope) {
  if (
    !record(given) ||
    given.source !== "target_version_native_bundle_reward_reader" ||
    given.fresh !== true ||
    given.readOnly !== true ||
    ...
    !validId(given.targetId) ||
    !validId(given.observationId) ||
    !validId(given.rewardId) ||
    given.mutexState !== "free" ||
    given.slotState !== "contributed" ||
    given.rewardState !== "available" ||
...
export function validateM7ClaimThen({ request, receipt, freshReward, expectedScope } = {}) {
  ...
  if (
    ...
    receipt.reasonCode !== "bundle_action_completed" ||
  ...
  if (
    ...
    evidence.rewardId !== request.rewardId ||
    evidence.rewardInventoryIdentity !== request.rewardId ||
    !Number.isSafeInteger(evidence.rewardInventoryStackBefore) ||
    !Number.isSafeInteger(evidence.rewardInventoryStackAfter) ||
    evidence.rewardInventoryStackAfter <= evidence.rewardInventoryStackBefore ||
    !sameScope(evidence.identity, scope) ||
    evidence.mutexBefore !== "free" ||
    evidence.mutexAfter !== "released" ||
    evidence.slotBefore !== "contributed" ||
    evidence.slotAfter !== "contributed" ||
    evidence.rewardBefore !== "available" ||
    evidence.rewardAfter !== "claimed" ||
    evidence.inventoryChanged !== true ||
    evidence.progressChanged !== false ||
    evidence.rewardAvailabilityChanged !== false ||
    ...
    postcondition.action !== ACTION ||
    postcondition.targetId !== request.target.targetId ||
    postcondition.progressChanged !== false ||
    postcondition.rewardAvailable !== false ||
    postcondition.rewardClaimed !== true ||
    postcondition.inventoryChanged !== true
  )
    return blocked("m7_claim_receipt_postcondition_invalid");
  const terminal = evidence.phaseTrace.at(-1);
  if (
    !exact(terminal, ["requestId", "traceId", "executionId", "phase", "revision", "reasonCode"]) ||
    terminal.requestId !== receipt.requestId ||
    terminal.traceId !== receipt.traceId ||
    terminal.executionId !== receipt.executionId ||
    terminal.phase !== "terminal" ||
    terminal.revision !== receipt.revision ||
    terminal.reasonCode !== receipt.reasonCode
  )
    return blocked("m7_claim_terminal_correlation_invalid");
  if (
    !record(freshReward) ||
    freshReward.source !== "target_version_native_bundle_reward_reader" ||
    freshReward.fresh !== true ||
    freshReward.readOnly !== true ||
    freshReward.gameplayMutationObserved !== false ||
    freshReward.saveMutationObserved !== false ||
    !sameScope(freshReward.scope, scope) ||
    !validIdentity(freshReward) ||
    freshReward.requestId !== receipt.requestId ||
    freshReward.traceId !== receipt.traceId ||
    freshReward.executionId !== receipt.executionId ||
    freshReward.targetId !== request.target.targetId ||
    freshReward.rewardId !== request.rewardId ||
    freshReward.slotState !== "contributed" ||
    freshReward.rewardState !== "claimed" ||
    freshReward.inventoryChanged !== true ||
    !Number.isSafeInteger(freshReward.revision) ||
    freshReward.revision < receipt.revision
  )
...
export function runM7ClaimRewardPreflight(input = {}) {
  ...
  if (
    input.request?.expectedRevision !== given.facts.revision ||
    input.request?.target?.targetId !== given.facts.targetId ||
    input.request?.target?.observationId !== given.facts.observationId ||
    input.request?.rewardId !== given.facts.rewardId
  )
    return Object.freeze({ state: "BLOCKED", given, then: blocked("m7_claim_given_request_correlation_invalid") });
  const then = validateM7ClaimThen({
    request: input.request,
    receipt: input.receipt,
    freshReward: input.freshReward,
  ...
  return Object.freeze({
    ...
    code: SOURCE_BLOCKER,
    sourceFact:
      "JunimoNoteMenu reward collection enters ItemGrabMenu and clears bundleRewards through its presentation callback; no approved typed non-UI semantic edge is established.",
  });
}
...
if (process.argv[1]?.endsWith("run-stardew-portfolio-m7-claim-reward-preflight.mjs"))
  console.log(
    JSON.stringify(
      blocked(SOURCE_BLOCKER, {
        action: ACTION,
        topology: TOPOLOGY,
        sourceFact: "No approved typed non-UI target-version reward-claim semantic edge.",
      }),
    ),
  );
