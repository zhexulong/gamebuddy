...
function coordinatorFixture(
  ...
  const connection: IntegrationConnection = {
    ...
    module: {
      ...base,
      cancelExecution: (connection_, requestId, executionId, reasonCode) => {
...
test("coordinator receiveWake shares one fail-closed wake normalization", () => {
  ...
  assert.equal(coordinator.receiveWake({ kind: "terminal", requestId: "", executionId: "x", state: "s", reasonCode: "r" }), null);
