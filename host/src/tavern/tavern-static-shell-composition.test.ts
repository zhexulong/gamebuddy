...
import type { MountedChatSnapshot, MountedChatReader } from "./mounted-chat-reader.js";
import { startTavernStaticShellComposition } from "./tavern-static-shell-composition.js";
...
function fakeFacade(): MountedChatReader {
  const state: MountedChatSnapshot = Object.freeze({
...
test("static shell uses one origin for verified static bytes and the exact v1 API", async () => {
  const fixture = await artifactFixture();
  const server = await startTavernStaticShellComposition({
  ...
  try {
    ...
    assert.equal(
      (
        await fetch(`${server.origin}/api/tavern/v1/state`, {
    ...
    assert.equal(
      (
        await fetch(`${server.origin}/api/tavern/v1/draft`, {
          headers: { Cookie: cookie, "Sec-Fetch-Site": "same-origin" },
        })
      ).status,
      200,
...
test("static shell drains an admitted facade read before close resolves", async () => {
  const fixture = await artifactFixture();
  ...
  const readRelease = new Promise<void>((resolveRelease) => {
    release = resolveRelease;
  });
  const base = fakeFacade();
  const facade: MountedChatReader = Object.freeze({
    read: async () => {
      started!();
      await readRelease;
...
test("tavern shell rejects an artifact with an identity other than the fixed browser build identity", async () => {
  const fixture = await artifactFixture();
  await writeFile(
    join(fixture.root, "tavern-browser-artifact-manifest.json"),
    JSON.stringify({
      schemaVersion: 1,
      browserContract: "tavern_browser_api/v1",
      profileId: "other",
  ...
  try {
    await assert.rejects(
      startTavernStaticShellComposition({
        artifactRoot: fixture.root,
        inspector: inspector(),
        facade: fakeFacade(),
        profile,
