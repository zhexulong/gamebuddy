...
test("title management projects only player-safe metadata and maps revision to managementRevision", async () => {
  const calls: unknown[] = [];
  const service = createChatTitleManagementService(
    {
  ...
  assert.deepEqual(await service.setTitle({ title: " A quiet morning ", expectedRevision: 10 }), {
