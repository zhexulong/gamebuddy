...
test("compiles exact v2 Chat catalog with reference-free scope and deterministic sources", async () => {
  const root = await mkdtemp(join(tmpdir(), "gamebuddy-catalog-"));
  ...
  const thread = await threads.createThread({ chatThreadId: "thread", chatSurfaceSessionId: "surface", companionId: "companion", continuityId: "continuity", personaId: "persona", scenarioId: "scenario", stableArtifactBindings: [
