...
import { assertCurrentMountedLeaseAfterDurableRead, createMountedChatReader } from "./mounted-chat-reader.js";
...
test("post-read lease guard rejects when a controlled durable-read completion observes revocation", () => {
...
test(
  "facade uses a real mounted projection, hides durable IDs, and requires the session-scoped identity binding",
  ...
  async () => {
    ...
    try {
      ...
      const facadeUrl = new URL("./mounted-chat-reader.js", import.meta.url).href;
      ...
      const script = `
        ...
        const { bindWindowsStaleLockReclaimer } = await import(new URL("./path-lock.js", runtimeUrl).href);
        const { createBuildWindowsStaleLockReclaimer } = await import(
          new URL("./windows-stale-lock-reclaimer/index.js", runtimeUrl).href,
        );
        ...
        const { createMountedChatReader } = await import(facadeUrl);
        ...
        const state = await (await createMountedChatReader(manifest, lease)).read();
        ...
        try { await createMountedChatReader(manifest, lease); } catch { rootBindingAccepted = false; }
        ...
        const sessionBindingAccepted = await createMountedChatReader(manifest, lease).then(() => true, () => false);
