...
import {
  CANONICAL_DIALOGUE_INPUT_KIND,
  renderCanonicalDialogueEnvelope,
  runMountedProviderInvocation,
...
test("provider invocation serializes the canonical accepted-message envelope without provider facts", () => {
...
test("provider invocation terminalizes an observed prompt without a durable presentation as failed", async () => {
  ...
  const result = await runMountedProviderInvocation(scope);
...
test("provider invocation runs when native assistant output settles before the provider observer", async () => {
  let observer: Observer | undefined;
  const { scope, transitions, presentationTransitions } = createScope({
...
test("provider invocation commits final native assistant content once after durable running and clears its preview", async () => {
  let observer: Observer | undefined;
  ...
  const result = await runMountedProviderInvocation(scope, previewPublisher);
...
test("provider invocation normalizes native preview deltas before the browser event contract and completes the final text", async () => {
  let observer: Observer | undefined;
...
test("provider invocation terminalizes a rejected prompt after an observation as durable runtime failure", async () => {
  let observer: Observer | undefined;
  let promptCalls = 0;
  const { scope, transitions, presentationTransitions } = createScope({
    runtimeSession: Object.freeze({
...
test("provider invocation returns a normal durable Stop winner when an aborted prompt rejects", async () => {
  let promptCalls = 0;
  let stopped = false;
  const { scope, transitions, presentationTransitions } = createScope({
...
test("provider invocation rereads a normal Stop winner that races completion", async () => {
  let observer: Observer | undefined;
  let stopped = false;
  const { scope, presentationTransitions } = createScope({
...
test("provider invocation rereads a normal Stop winner after provider settlement", async () => {
  let observer: Observer | undefined;
  let promptCalls = 0;
  let stopped = false;
  const { scope, transitions, presentationTransitions } = createScope({
