...
async function currentTerminalResult(scope: ProviderInvocationScope): Promise<ProviderInvocationResult | undefined> {
...
/**
 * The exclusive provider invocation consumer. It runs inside the coordinator's close-drained
 ...
 */
export async function runMountedProviderInvocation(
  scope: ProviderInvocationScope,
  previewPublisher?: NativeChatPreviewPublisher,
  ...
  try {
    ...
    nativeObserver = attachNativeCompanionContent(runtimeSession.session, {
      onPreviewDelta: async (delta) => {
        ...
        try {
          ...
          // schema failure must not poison observer drain or strand presentation running.
          return;
        }
        previewPublished = true;
      },
      onFinalText: async (text) => {
        // Pi may emit its final assistant message before the asynchronous provider start
        ...
        try {
          ...
          // Stop winner when one exists; otherwise let the ordinary presentation failure
    ...
    // queued before this boundary subscriber resolves running.
    ...
    if (first.phase === "observation" || first.phase === "native_assistant") {
      ...
      // native snapshot into the sole presentation commit after prompt settlement.
