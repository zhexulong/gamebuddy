...
import { claimMountedProviderAttemptFromFacade } from "./provider-attempt-claim.internal.js";

export type ProviderAttemptClaimer = Readonly<{
  /**
   * Claims the already-accepted exact turn. This is deliberately not a prompt
   * operation; it has no provider, presentation, or session capability.
   */
  claim(): Promise<AttemptStartingTurn>;
}>;

/**
 * Binds the attempt claim to the deployment principal and a current
 * coordinator-branded mount. The caller supplies no turn/runtime facts.
 */
export function createProviderAttemptClaimer(
  manifest: HostDeploymentManifest,
  lease: MountedChatRuntimeLease,
): ProviderAttemptClaimer {
  if (!isCurrentMountedChatRuntimeLease(lease)) throw unavailable();
  return Object.freeze({
    async claim(): Promise<AttemptStartingTurn> {
      try {
        return await claimMountedProviderAttemptFromFacade(manifest, lease);
      } catch (error) {
        if (
