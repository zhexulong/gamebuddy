...
import { createDialogueWebRequestHandler, type DialogueWebOptions } from "../dialogue-web.js";
...
export const TAVERN_BROWSER_ARTIFACT_IDENTITY: TavernStaticArtifactIdentity = Object.freeze({
  browserContract: TAVERN_BROWSER_CONTRACT,
  profileId: "gamebuddy.tavern.browser.v1",
});

export type TavernStaticShellCompositionOptions = DialogueWebOptions &
  Readonly<{
    artifactRoot: string;
    inspector?: WindowsReparseInspectorCapability;
  }>;
export type TavernStaticShellComposition = Readonly<{
  origin: string;
  launchUrl: string;
...
/**
 * Serial composition: one verified immutable browser shell and one closed
 * API dispatcher share the same loopback listener. Neither constituent can
 * discover paths or bind a second listener through this boundary.
 */
export async function startTavernStaticShellComposition(
  options: TavernStaticShellCompositionOptions,
): Promise<TavernStaticShellComposition> {
  const artifact = await verifyTavernStaticArtifact(
    options.artifactRoot,
    TAVERN_BROWSER_ARTIFACT_IDENTITY,
    options.inspector,
  );
  const staticHandler = createTavernStaticArtifactRequestHandler(artifact);
  const apiHandler = createDialogueWebRequestHandler(options);
  let closed = false;
