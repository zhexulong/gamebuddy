...
test("execution validation fails closed for stale, unknown, malformed, and unactionable requests", () => {
  const valid = { requestId: "request_01", idempotencyKey: "idempotency_01", action: "move_to_tile", args: { x: 11, y: 12 }, expectedRevision: 4, deadlineMs: now + 10_000, permissionToken: "permission_token_01" };
