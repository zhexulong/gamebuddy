...
type Outcome = "regular" | "malformed" | "unavailable" | "timeout" | "nonzero" | "stderr" | "overflow";
...
for (const category of RELEASE_CATEGORIES) {
  test(`releaseOwnedLock maps the ${category} native category through the frozen protocol`, async () => {
    const capability = createTestWindowsStaleLockReclaimer(() => syntheticChild(category));
    assert.equal(await releaseOwnedLock(capability, absoluteCandidate, "00000000-0000-4000-8000-000000000000"), category);
  });
}
...
for (const outcome of ["malformed", "unavailable", "timeout", "nonzero", "stderr", "overflow"] as const) {
  test(`reclaimStaleLock ${outcome} child behavior fails closed`, async () => {
    ...
    await assert.rejects(reclaimStaleLock(capability, absoluteCandidate, "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  });
}
...
test("reclaimStaleLock sends exactly the frozen root/segments request grammar for each operation", async () => {
  ...
  let calls = 0;
  const capability = createTestWindowsStaleLockReclaimer(() => syntheticChild(calls++ === 0 ? "reclaimed" : "released", requests));
...
test("reclaimStaleLock rejects invalid capabilities and unsupported paths before any spawn", async () => {
  await assert.rejects(reclaimStaleLock({} as never, absoluteCandidate, "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  ...
  await assert.rejects(reclaimStaleLock(capability, "relative/candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(releaseOwnedLock(capability, "relative/candidate.lock", "00000000-0000-4000-8000-000000000000"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/candidate.json", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/../candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/./candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute//candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/*.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/CoN.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/candidate.lock ", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  await assert.rejects(reclaimStaleLock(capability, "/absolute/candidate.lock.", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  if (process.platform === "win32") {
    await assert.rejects(reclaimStaleLock(capability, "\\\\server\\share\\candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
    await assert.rejects(reclaimStaleLock(capability, "\\\\?\\C:\\candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
    await assert.rejects(reclaimStaleLock(capability, "\\.\\C:\\candidate.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
    await assert.rejects(reclaimStaleLock(capability, "C:relative.lock", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
    await assert.rejects(reclaimStaleLock(capability, "C:\\", "stale_malformed"), /windows_stale_lock_reclaimer_unavailable/);
  }
});
...
function fixtureInventory(helperSha256: string, manifestSha256: string): string {
  ...
  return JSON.stringify({
    ...
    entries: [
      { path: `${publishedPairDestination}/${publishedHelperFileName}`, type: "file", mode: "755", sha256: helperSha256, origin },
      { path: `${publishedPairDestination}/${publishedManifestFileName}`, type: "file", mode: "644", sha256: manifestSha256, origin },
    ],
  });
}
...
test("published adapter requires the Host-TCB generation inventory binding", { skip: process.platform !== "win32" }, async () => {
...
test("published adapter fails closed on a tampered generation inventory", { skip: process.platform !== "win32" }, async () => {
  const fixture = await makePublishedPairFixture();
  try {
    ...
    const tamper = async (mutate: (parsed: any) => void) => {
      ...
      await assert.rejects(createPublishedWindowsStaleLockReclaimer(fixture.root), /windows_stale_lock_reclaimer_unavailable/);
    };
    await tamper((parsed) => { parsed.entries[0].sha256 = "0".repeat(64); });
    await tamper((parsed) => { parsed.entries[0].origin.kind = "typescript_emit"; });
    await tamper((parsed) => { parsed.schema = "gamebuddy-host-production-inventory/v3"; });
    await tamper((parsed) => { parsed.entries = parsed.entries.slice(0, 1); });
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
});

test("published adapter fails closed on a pair-directory junction before mint and before spawn", { skip: process.platform !== "win32" }, async (t) => {
  const fixture = await makePublishedPairFixture();
...
test("published adapter revalidates the helper digest immediately before each spawn", { skip: process.platform !== "win32" }, async () => {
  const fixture = await makePublishedPairFixture();
  try {
    const capability = await createPublishedWindowsStaleLockReclaimer(fixture.root);
...
test("published adapter revalidates the canonical manifest immediately before each spawn", { skip: process.platform !== "win32" }, async () => {
  const fixture = await makePublishedPairFixture();
  try {
    const capability = await createPublishedWindowsStaleLockReclaimer(fixture.root);
...
test("published adapter reclaims a real stale lock through a verified emitted pair", { skip: process.platform !== "win32" }, async (t) => {
  const hostRoot = resolve(fileURLToPath(new URL("../..", import.meta.url)));
  const realHelper = resolve(hostRoot, "native", "windows-stale-lock-reclaimer", ".dist", "win-x64", publishedHelperFileName);
  let helperState;
  try {
...
test("public policy entry does not expose test-only capability minting", async () => {
  const source = await readFile(resolve(fileURLToPath(new URL("../..", import.meta.url)), "src", "windows-stale-lock-reclaimer", "index.ts"), "utf8");
