...
test("default consent exposes only published live capabilities", () => {
  const visible = visiblePublishedActions(["move_to_tile", "equip_tool", "travel", "enter_exit", "pickup_forage", "pickup_item", "water_crop", "refill_watering_can", "plant_seed", "fertilize_tile", "machine_inspect", "use_item", "harvest_crop", "end_day"]);
  assert.deepEqual(visible.map((entry) => entry.actionId), ["move_to_tile", "equip_tool", "travel", "enter_exit", "pickup_forage", "pickup_item", "water_crop", "refill_watering_can", "plant_seed", "fertilize_tile", "machine_inspect", "use_item", "harvest_crop"]);
  assert.equal(PUBLISHED_STARDEW_ACTIONS.length, 24);
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "travel" && entry.actionClass === "primitive" && entry.lifecycle === "published"));
  ...
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "refill_watering_can" && entry.familyId === "farming_crops" && entry.actionClass === "primitive" && entry.lifecycle === "published"));
  ...
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "machine_load" && entry.familyId === "machines_processing" && entry.lifecycle === "published"));
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "machine_collect_output" && entry.familyId === "machines_processing" && entry.lifecycle === "published"));
  assert.equal(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "collect_resource"), false);
  ...
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "break_rock_source" && entry.lifecycle === "published"));
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "clear_hoedirt" && entry.familyId === "farming_crops" && entry.actionClass === "primitive" && entry.lifecycle === "published"));
  assert.ok(STARDEW_ACTION_REGISTRY.some((entry) => entry.actionId === "chop_tree_source" && entry.familyId === "resource_gathering" && entry.actionClass === "primitive" && entry.lifecycle === "published"));
  assert.deepEqual(PUBLISHED_STARDEW_ACTIONS.map((entry) => entry.actionId), ["move_to_tile", "equip_tool", "travel", "enter_exit", "till_soil", "pickup_forage", "pickup_item", "water_crop", "refill_watering_can", "plant_seed", "fertilize_tile", "place_wood_fence", "place_crab_pot", "machine_inspect", "machine_load", "machine_collect_output", "collect_animal_product", "feed_animal", "use_item", "harvest_crop", "break_rock_source", "clear_hoedirt", "dig_artifact_spot", "chop_tree_source"]);
  ...
  assert.ok(STARDEW_ACTION_REGISTRY.every((entry) => isMaterializablePublishedAction(entry) === (entry.lifecycle === "published")));
  assert.equal(STARDEW_ACTION_REGISTRY.some((entry) => entry.lifecycle === "planned"), false);
});

test("retired action identifiers require an explicit fail-closed migration", () => {
  assert.deepEqual(RETIRED_ACTION_POLICY_MIGRATIONS.collect_resource, ["chop_tree_source", "break_rock_source", "pickup_item"]);
  assert.throws(
...
test("refill_watering_can is capability-gated and can be explicitly denied", () => {
  assert.deepEqual(visiblePublishedActions(["move_to_tile"]).map((entry) => entry.actionId), ["move_to_tile"]);
  assert.deepEqual(visiblePublishedActions(["move_to_tile", "refill_watering_can"]).map((entry) => entry.actionId), ["move_to_tile", "refill_watering_can"]);
