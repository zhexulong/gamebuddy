...
test("knowledge bundles are versioned, bounded, and fail closed on duplicates or scope drift", () => {
  ...
  assert.throws(() => parseKnowledgeBundle({ ...bundle, rules: [bundle.rules[0], bundle.rules[0]] }), /duplicate_knowledge_rule/);
  assert.throws(() => parseKnowledgeBundle({ ...bundle, rules: [{ ...bundle.rules[0], gameVersion: "1.6.14" }] }), /invalid_knowledge_rule/);
});
...
test("mounted knowledge exposes advisory rules only for live, version-matched capabilities", async () => {
  ...
  const supported = await tool.execute("knowledge", { capability: "move_to_tile" }, new AbortController().signal, () => {}, {} as never);
  ...
  const unavailable = await tool.execute("knowledge", { capability: "equip_tool" }, new AbortController().signal, () => {}, {} as never);
  assert.equal((unavailable.details as { reasonCode: string }).reasonCode, "capability_not_declared");
  integration.gameVersion = "1.6.14";
  const wrongVersion = await tool.execute("knowledge", { capability: "move_to_tile" }, new AbortController().signal, () => {}, {} as never);
...
test("knowledge cannot disclose a denied action or its advisory rules", () => {
  const actionBundle: KnowledgeBundle = { bundleVersion: 1, integrationId: "stardew", gameVersion: "1.6.15", rules: [{ id: "equip-v1", integrationId: "stardew", gameVersion: "1.6.15", capability: "equip_tool", actionId: "equip_tool", topicId: "tools", text: "Equip a live tool first." }] };
  const denied = decideKnowledge(actionBundle, { ...snapshot, capabilities: ["equip_tool"] }, { actionId: "equip_tool" }, "1.6.15", { policyVersion: 1, deniedActions: ["equip_tool"], deniedFamilies: [] });
