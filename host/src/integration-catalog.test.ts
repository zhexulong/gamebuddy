...
function launcher(id = "arcade"): ConfigurableIntegrationLauncher {
  const action = defineTool({
    name: "arcade_execute",
    label: "Execute",
    description: "Fixture receipt-backed action.",
  ...
  return Object.freeze({
    ...
    module,
    launch: async () => {
      throw new Error("fixture_launcher_not_started");
    },
    prepare: async (config: unknown) => {
      if (config === null || typeof config !== "object" || Array.isArray(config))
        throw new Error("invalid_fixture_config");
      return Object.freeze({
        launchConfig: Object.freeze({ fixture: true }),
        identityScope: Object.freeze({ saveId: "fixture-save", worldId: "fixture-world" }),
      });
    },
  });
}
...
test("catalog selects only registered configurable launchers and preserves opaque config", async () => {
  ...
  await assert.rejects(
    () => catalog.select("stardew", {}, { configDirectory: "C:/profile" }),
...
test("catalog rejects duplicate, malformed, and fixture-free composition errors", () => {
  ...
  assert.throws(
    () => createIntegrationCatalog([{ integrationId: "broken" } as ConfigurableIntegrationLauncher]),
...
test("Host identity binds only validated adapter scope", () => {
  assert.deepEqual(
    bindIntegrationIdentity({ playerId: "player", companionId: "companion" }, { saveId: "save", worldId: "world" }),
    { playerId: "player", companionId: "companion", saveId: "save", worldId: "world" },
  );
  assert.throws(
    () => bindIntegrationIdentity({ playerId: "player", companionId: "companion" }, {} as never),
    /invalid_integration_identity_scope/,
  );
  assert.deepEqual(
    bindIntegrationIdentity(
      { playerId: "player", companionId: "companion", continuityId: "continuity" },
      { saveId: "save", worldId: "world" },
    ),
    { playerId: "player", companionId: "companion", continuityId: "continuity", saveId: "save", worldId: "world" },
  );
});

test("Stardew adapter alone parses Stardew operator config", () => {
  const parsed = parseStardewOperatorConfig({
    saveId: "save",
    worldId: "world",
    pipeName: "pipe",
