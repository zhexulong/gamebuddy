...
import { createContainedGameRuntime, type ContainedGameRuntimeAuthorization } from "./contained-game-runtime.internal.js";
...
const produce = (authorization: ContainedGameRuntimeAuthorization) => authorization(new Uint8Array([1]));
...
test("valid producer works, arm is once before launch, and contain is ordered", async () => {
  ...
  assert.deepEqual(await runtime.launchRole("server", produce), { role: "server", status: "succeeded" });
  assert.deepEqual(await runtime.launchRole("client", produce), { role: "client", status: "succeeded" });
...
test("authorization producer is called once and replay/forge/wrong role are rejected", async () => {
  ...
  const runtime = createContainedGameRuntime(fakeSession(log), binding);
  let calls = 0;
  await runtime.launchRole("role", (authorization) => { calls++; authorization(new Uint8Array()); });
  assert.equal(calls, 1);
  const replayRuntime = createContainedGameRuntime(fakeSession([]), binding);
  await assert.rejects(() => replayRuntime.launchRole("replay", (authorization) => {
    authorization(new Uint8Array());
    authorization(new Uint8Array());
  }), /replay/);
  await assert.rejects(() => runtime.launchRole("forged", (authorization) => (authorization as unknown as (x: unknown) => void)({}),), /forged/);
...
test("expired authorization and close prevent later launch; results are redacted", async () => {
  ...
  await assert.rejects(() => expired.launchRole("role", produce), /expired/);
  const runtime = createContainedGameRuntime(fakeSession(log), binding);
  await runtime.close();
  await assert.rejects(() => runtime.launchRole("role", produce), /closed/);
  assert.deepEqual(Object.keys(await createContainedGameRuntime(fakeSession([]), binding).launchRole("safe", produce)), ["role", "status"]);
});
...
test("authorization is invocation-bound and binding is snapshotted", async () => {
  ...
  let saved: ContainedGameRuntimeAuthorization | undefined;
  const runtime = createContainedGameRuntime(fakeSession(log), mutable);
  mutable.guardianEpoch = 99;
  await runtime.launchRole("first", (authorization) => { saved = authorization; authorization(new Uint8Array([7])); });
  await assert.rejects(() => runtime.launchRole("second", (authorization) => {
    void authorization;
...
test("failed launch is terminal and does not retry the native session call", async () => {
  ...
  assert.deepEqual(await runtime.launchRole("role", produce), { role: "role", status: "failed" });
  await assert.rejects(() => runtime.launchRole("role", produce), /already launched/);
...
test("containment is terminal and duplicate contain rejects before native call", async () => {
  const log: string[] = [];
  const runtime = createContainedGameRuntime(fakeSession(log), binding);
  await runtime.launchRole("role", produce);
...
test("runtime source creates authorization only inside launchRole and exports no mint factory", () => {
  const source = readFileSync(resolve(
    ...
    "../../../src/containment/runtime/contained-game-runtime.internal.ts",
  ), "utf8");
  const launchImplementationIndex = source.indexOf("launchRole(role, produceAuthorization) {");
...
test("arm failure is terminal and does not launch or retry", async () => {
  ...
  await assert.rejects(() => runtime.launchRole("role", produce), /arm failed/);
  await assert.rejects(() => runtime.launchRole("role", produce), /arm already failed/);
...
test("requested role is bound into the platform launch input", async () => {
  const log: string[] = [];
  const runtime = createContainedGameRuntime(fakeSession(log), binding);
  await runtime.launchRole("requested-role", produce);
