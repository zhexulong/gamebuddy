 ...
 */
export function createP3DialogueWebRequestHandler(options: DialogueWebOptions): P3DialogueWebRequestHandler {
  ...
  if (!isOpaqueHandle(bootstrapToken)) throw new Error("p3_bootstrap_token_invalid");
  ...
  const dispatch = async (request: IncomingMessage, response: ServerResponse, origin: string): Promise<void> => {
    ...
    try {
      if (request.method === "POST" && url.pathname === "/api/tavern/v1/bootstrap") {
        ...
        return await sendProjectedSnapshot(response, p3Facade, profile, session);
      }
