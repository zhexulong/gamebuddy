...
export type Snapshot = Readonly<{
  ...
  wateringCanFacts?: readonly Readonly<{ slot: number; qualifiedItemId: string; label: string; water: number; max: number }>[];
  ...
  woodFenceTargets?: readonly Readonly<{ targetId: string; location: string; slot: number; x: number; y: number; qualifiedItemId: "(O)322" }>[];
  /** Same-location native non-gate Fence result published only after placement. */
  woodFenceResultTargets?: readonly Readonly<{ targetId: string; location: string; slot: number; x: number; y: number; qualifiedItemId: "(O)322"; isFence: true; isGate: false; health: number; maxHealth: number }>[];
  /** Fresh valid Farm water tiles paired with one qualified (O)710 inventory slot. */
  crabPotTargets?: readonly Readonly<{ targetId: string; location: string; slot: number; x: number; y: number; qualifiedItemId: "(O)710" }>[];
  /** Same-location native Crab Pot result published only after placement. */
  crabPotResultTargets?: readonly Readonly<{ targetId: string; location: string; slot: number; x: number; y: number; qualifiedItemId: "(O)710"; ownerId: number; offsetX: number; offsetY: number; overlayTiles: readonly { x: number; y: number; count: number }[] }>[];
  /** Nearby native ResourceClump targets paired with a usable tool slot. */
  debrisTargets?: readonly Readonly<{ targetId: string; slot: number; x: number; y: number; parentSheetIndex: number; toolKind: string; requiredUpgradeLevel: number; health: number }>[];
  /** Nearby one-hit ordinary breakable stone sources for the bounded native Pickaxe path. */
  rockSourceTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; qualifiedItemId: string; health: number }>[];
  /** Adjacent live empty ground HoeDirt targets for one Basic Pickaxe clear. */
  clearHoeDirtTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; crop: false; ground: true }>[];
  /** Adjacent intact `(O)590` artifact spots for one native Basic Hoe use. */
  artifactSpotTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; qualifiedItemId: "(O)590" }>[];
  /** Same-location plain ground HoeDirt created by dig_artifact_spot, published only after that action. */
  artifactSpotResultTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; crop: false; ground: true }>[];
  ...
  treeChopSourceTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; treeType: string; growthStage: number; health: number; stump: false; moss: false; tapped: false }>[];
  /** Same-location native stump results after a terminal chop-tree-source strike. */
  treeChopResultTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; treeType: string; health: number; stump: true; moss: false; tapped: false }>[];
  ...
  machineTargets?: readonly Readonly<{ targetId: string; x: number; y: number; qualifiedItemId: string; readyForHarvest: boolean; minutesUntilReady: number; heldObjectQualifiedItemId?: string | null; lastInputQualifiedItemId?: string | null; loadInputSlot?: number | null; loadInputQualifiedItemId?: "(O)433" | null; loadInputStack?: 5 | null; collectOutputReady?: boolean | null }>[];
  /** Nearby ordinary tree sources observed for read-only native shake discovery. */
  treeShakeSourceTargets?: readonly Readonly<{ targetId: string; location: string; x: number; y: number; treeType: string; growthStage: number; health: number; moss: boolean; tapped: boolean }>[];
...
export type ExecutionRequest = Readonly<{
  ...
  action: "move_to_tile" | "equip_tool" | "travel" | "enter_exit" | "till_soil" | "pickup_forage" | "pickup_item" | "water_crop" | "refill_watering_can" | "harvest_crop" | "plant_seed" | "fertilize_tile" | "place_wood_fence" | "place_crab_pot" | "clear_debris" | "machine_inspect" | "machine_load" | "machine_collect_output" | "npc_relationship" | "pet_animal" | "collect_animal_product" | "feed_animal" | "use_item" | "tree_first_hit" | "chop_tree_source" | "break_rock_source" | "clear_hoedirt" | "dig_artifact_spot" | "inspect_self";
...
export function validateExecutionRequest(value: unknown, snapshot: Snapshot, nowMs = Date.now()): string | null {
  ...
  if (value.action !== "move_to_tile" && value.action !== "equip_tool" && value.action !== "travel" && value.action !== "enter_exit" && value.action !== "till_soil" && value.action !== "pickup_forage" && value.action !== "pickup_item" && value.action !== "water_crop" && value.action !== "refill_watering_can" && value.action !== "harvest_crop" && value.action !== "plant_seed" && value.action !== "fertilize_tile" && value.action !== "place_wood_fence" && value.action !== "place_crab_pot" && value.action !== "clear_debris" && value.action !== "machine_inspect" && value.action !== "machine_load" && value.action !== "machine_collect_output" && value.action !== "npc_relationship" && value.action !== "pet_animal" && value.action !== "collect_animal_product" && value.action !== "feed_animal" && value.action !== "use_item" && value.action !== "tree_first_hit" && value.action !== "chop_tree_source" && value.action !== "break_rock_source" && value.action !== "clear_hoedirt" && value.action !== "dig_artifact_spot" && value.action !== "inspect_self") return "unknown_action";
  ...
  if (value.action === "move_to_tile") {
    ...
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y) || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_refill_watering_can_target";
  ...
  } else if (value.action === "place_wood_fence" || value.action === "place_crab_pot") {
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedQualifiedItemId", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y)
      || value.args.expectedQualifiedItemId !== (value.action === "place_wood_fence" ? "(O)322" : "(O)710") || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return value.action === "place_wood_fence" ? "invalid_wood_fence_target" : "invalid_crab_pot_target";
    ...
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedQualifiedItemId", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y)
      || value.args.expectedQualifiedItemId !== "(O)433" || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_machine_load_target";
  } else if (value.action === "machine_collect_output") {
    if (!hasOnlyKeys(value.args, ["x", "y", "expectedTargetId"]) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y)
      || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_machine_collect_target";
    ...
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y) || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_break_rock_source_target";
  } else if (value.action === "dig_artifact_spot") {
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y) || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_dig_artifact_spot_target";
  } else if (value.action === "clear_hoedirt") {
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"]) || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y) || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_clear_hoedirt_target";
  } else if (value.action === "tree_first_hit") {
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"])
      || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y)
      || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_tree_first_hit_target";
  } else if (value.action === "chop_tree_source") {
    if (!hasOnlyKeys(value.args, ["slot", "x", "y", "expectedTargetId"])
      || !isToolSlot(value.args.slot) || !isTileCoordinate(value.args.x) || !isTileCoordinate(value.args.y)
      || typeof value.args.expectedTargetId !== "string" || !isOpaqueId(value.args.expectedTargetId)) return "invalid_chop_tree_source_target";
...
function diagnoseSnapshot(value: Record<string, unknown>): string {
  ...
  if (value.wateringCanFacts !== undefined && (!Array.isArray(value.wateringCanFacts) || value.wateringCanFacts.length > 36 || !value.wateringCanFacts.every(isWateringCanFact))) return "invalid_snapshot:wateringCanFacts";
  if (value.refillWateringCanTargets !== undefined && (!Array.isArray(value.refillWateringCanTargets) || value.refillWateringCanTargets.length > 8 || !value.refillWateringCanTargets.every(isRefillWateringCanTargetFact))) return "invalid_snapshot:refillWateringCanTargets";
  ...
  if (value.rockSourceTargets !== undefined && (!Array.isArray(value.rockSourceTargets) || value.rockSourceTargets.length > 8 || !value.rockSourceTargets.every(isRockSourceTargetFact))) return "invalid_snapshot:rockSourceTargets";
  if (value.clearHoeDirtTargets !== undefined && (!Array.isArray(value.clearHoeDirtTargets) || value.clearHoeDirtTargets.length > 8 || !value.clearHoeDirtTargets.every(isClearHoeDirtTargetFact))) return "invalid_snapshot:clearHoeDirtTargets";
  if (value.artifactSpotTargets !== undefined && (!Array.isArray(value.artifactSpotTargets) || value.artifactSpotTargets.length > 8 || !value.artifactSpotTargets.every(isArtifactSpotTargetFact))) return "invalid_snapshot:artifactSpotTargets";
  if (value.artifactSpotResultTargets !== undefined && (!Array.isArray(value.artifactSpotResultTargets) || value.artifactSpotResultTargets.length > 1 || !value.artifactSpotResultTargets.every(isArtifactSpotResultTargetFact))) return "invalid_snapshot:artifactSpotResultTargets";
  if (value.artifactSpotFarmSourceCount !== undefined && value.artifactSpotFarmSourceCount !== null && (typeof value.artifactSpotFarmSourceCount !== "number" || !Number.isSafeInteger(value.artifactSpotFarmSourceCount) || value.artifactSpotFarmSourceCount < 0)) return "invalid_snapshot:artifactSpotFarmSourceCount";
  ...
  if (value.treeChopSourceTargets !== undefined && (!Array.isArray(value.treeChopSourceTargets) || value.treeChopSourceTargets.length > 64 || !value.treeChopSourceTargets.every(isTreeChopSourceTargetFact))) return "invalid_snapshot:treeChopSourceTargets";
  if (value.treeChopResultTargets !== undefined && (!Array.isArray(value.treeChopResultTargets) || value.treeChopResultTargets.length > 64 || !value.treeChopResultTargets.every(isTreeChopResultTargetFact))) return "invalid_snapshot:treeChopResultTargets";
  if (value.treeShakeSourceTargets !== undefined && (!Array.isArray(value.treeShakeSourceTargets) || value.treeShakeSourceTargets.length > 64 || !value.treeShakeSourceTargets.every(isTreeShakeSourceTargetFact))) return "invalid_snapshot:treeShakeSourceTargets";
  ...
  if (value.inventoryItemFacts !== undefined && (!Array.isArray(value.inventoryItemFacts) || value.inventoryItemFacts.length > 36 || !value.inventoryItemFacts.every(isInventoryItemFact))) return "invalid_snapshot:inventoryItemFacts";
...
function validateSnapshot(value: Record<string, unknown>): string | null {
  return Number.isSafeInteger(value.revision) && typeof value.location === "string" && isRecord(value.tile)
    ...
    && (value.wateringCanFacts === undefined || (Array.isArray(value.wateringCanFacts) && value.wateringCanFacts.length <= 36 && value.wateringCanFacts.every(isWateringCanFact)))
    && (value.refillWateringCanTargets === undefined || (Array.isArray(value.refillWateringCanTargets) && value.refillWateringCanTargets.length <= 8 && value.refillWateringCanTargets.every(isRefillWateringCanTargetFact)))
    ...
    && (value.woodFenceTargets === undefined || (Array.isArray(value.woodFenceTargets) && value.woodFenceTargets.length <= 16 && value.woodFenceTargets.every(isWoodFenceTargetFact)))
    && (value.woodFenceResultTargets === undefined || (Array.isArray(value.woodFenceResultTargets) && value.woodFenceResultTargets.length <= 1 && value.woodFenceResultTargets.every(isWoodFenceResultTargetFact)))
    && (value.crabPotTargets === undefined || (Array.isArray(value.crabPotTargets) && value.crabPotTargets.length <= 16 && value.crabPotTargets.every(isCrabPotTargetFact)))
    && (value.crabPotResultTargets === undefined || (Array.isArray(value.crabPotResultTargets) && value.crabPotResultTargets.length <= 1 && value.crabPotResultTargets.every(isCrabPotResultTargetFact)))
    ...
    && (value.rockSourceTargets === undefined || (Array.isArray(value.rockSourceTargets) && value.rockSourceTargets.length <= 8 && value.rockSourceTargets.every(isRockSourceTargetFact)))
    && (value.clearHoeDirtTargets === undefined || (Array.isArray(value.clearHoeDirtTargets) && value.clearHoeDirtTargets.length <= 8 && value.clearHoeDirtTargets.every(isClearHoeDirtTargetFact)))
    && (value.artifactSpotTargets === undefined || (Array.isArray(value.artifactSpotTargets) && value.artifactSpotTargets.length <= 8 && value.artifactSpotTargets.every(isArtifactSpotTargetFact)))
    && (value.artifactSpotResultTargets === undefined || (Array.isArray(value.artifactSpotResultTargets) && value.artifactSpotResultTargets.length <= 1 && value.artifactSpotResultTargets.every(isArtifactSpotResultTargetFact)))
    && (value.artifactSpotFarmSourceCount === undefined || value.artifactSpotFarmSourceCount === null || (typeof value.artifactSpotFarmSourceCount === "number" && Number.isSafeInteger(value.artifactSpotFarmSourceCount) && value.artifactSpotFarmSourceCount >= 0))
    ...
    && (value.treeChopSourceTargets === undefined || (Array.isArray(value.treeChopSourceTargets) && value.treeChopSourceTargets.length <= 64 && value.treeChopSourceTargets.every(isTreeChopSourceTargetFact)))
    && (value.treeChopResultTargets === undefined || (Array.isArray(value.treeChopResultTargets) && value.treeChopResultTargets.length <= 64 && value.treeChopResultTargets.every(isTreeChopResultTargetFact)))
    && (value.treeShakeSourceTargets === undefined || (Array.isArray(value.treeShakeSourceTargets) && value.treeShakeSourceTargets.length <= 64 && value.treeShakeSourceTargets.every(isTreeShakeSourceTargetFact)))
    ...
    && (value.inventoryItemFacts === undefined || (Array.isArray(value.inventoryItemFacts) && value.inventoryItemFacts.length <= 36 && value.inventoryItemFacts.every(isInventoryItemFact)))
...
function validateExecutionRequestEnvelope(value: Record<string, unknown>): string | null {
  return isOpaqueId(value.requestId) && isOpaqueId(value.idempotencyKey)
    && (value.action === "move_to_tile" || value.action === "equip_tool" || value.action === "travel" || value.action === "enter_exit" || value.action === "till_soil" || value.action === "pickup_forage" || value.action === "pickup_item" || value.action === "water_crop" || value.action === "refill_watering_can" || value.action === "harvest_crop" || value.action === "plant_seed" || value.action === "fertilize_tile" || value.action === "place_wood_fence" || value.action === "place_crab_pot" || value.action === "clear_debris" || value.action === "machine_inspect" || value.action === "machine_load" || value.action === "machine_collect_output" || value.action === "npc_relationship" || value.action === "pet_animal" || value.action === "collect_animal_product" || value.action === "feed_animal" || value.action === "use_item" || value.action === "tree_first_hit" || value.action === "chop_tree_source" || value.action === "break_rock_source" || value.action === "clear_hoedirt" || value.action === "dig_artifact_spot" || value.action === "inspect_self")
...
function isWateringCanFact(value: unknown): boolean {
  return isRecord(value) && isToolSlot(value.slot) && typeof value.qualifiedItemId === "string" && value.qualifiedItemId.length > 0 && value.qualifiedItemId.length <= 128
    && typeof value.label === "string" && value.label.length > 0 && value.label.length <= 128
    && typeof value.water === "number" && Number.isSafeInteger(value.water) && value.water >= 0 && value.water <= 100
    && typeof value.max === "number" && Number.isSafeInteger(value.max) && value.max > 0 && value.max <= 100 && value.water <= value.max;
}
function isRefillWateringCanTargetFact(value: unknown): boolean { return isRecord(value) && isOpaqueId(value.targetId) && isTileCoordinate(value.x) && isTileCoordinate(value.y); }
...
function isWoodFenceTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId) && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 256
    && isToolSlot(value.slot) && isTileCoordinate(value.x) && isTileCoordinate(value.y) && value.qualifiedItemId === "(O)322";
}

function isWoodFenceResultTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId) && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 256
    && isToolSlot(value.slot) && isTileCoordinate(value.x) && isTileCoordinate(value.y) && value.qualifiedItemId === "(O)322"
    && value.isFence === true && value.isGate === false && isFiniteNumber(value.health) && isFiniteNumber(value.maxHealth)
    && value.health > 0 && value.maxHealth >= value.health;
}

function isCrabPotTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId) && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 256
    && isToolSlot(value.slot) && isTileCoordinate(value.x) && isTileCoordinate(value.y) && value.qualifiedItemId === "(O)710";
}
function isCrabPotResultTargetFact(value: unknown): boolean {
  return isCrabPotTargetFact(value) && isRecord(value) && typeof value.ownerId === "number" && Number.isSafeInteger(value.ownerId)
    && isFiniteNumber(value.offsetX) && isFiniteNumber(value.offsetY)
    && Array.isArray(value.overlayTiles) && value.overlayTiles.length <= 4 && value.overlayTiles.every((tile) => isRecord(tile)
      && isTileCoordinate(tile.x) && isTileCoordinate(tile.y) && typeof tile.count === "number" && Number.isSafeInteger(tile.count) && tile.count > 0);
}
...
function isDebrisTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId) && isToolSlot(value.slot) && isTileCoordinate(value.x) && isTileCoordinate(value.y)
    ...
    && typeof value.health === "number" && Number.isSafeInteger(value.health) && value.health > 0 && value.health <= 100;
}

function isArtifactSpotTargetFact(value: unknown): boolean {
  return isRecord(value) && hasOnlyKeys(value, ["targetId", "location", "x", "y", "qualifiedItemId"])
    && isOpaqueId(value.targetId) && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 128
    && isTileCoordinate(value.x) && isTileCoordinate(value.y) && value.qualifiedItemId === "(O)590";
}

function isClearHoeDirtTargetFact(value: unknown): boolean {
  return isRecord(value) && hasOnlyKeys(value, ["targetId", "location", "x", "y", "crop", "ground"])
    && isOpaqueId(value.targetId) && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 128
    && isTileCoordinate(value.x) && isTileCoordinate(value.y) && value.crop === false && value.ground === true;
}
...
function isRockSourceTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId)
    && typeof value.location === "string" && value.location.length > 0 && value.location.length <= 256
    && isTileCoordinate(value.x) && isTileCoordinate(value.y)
    && value.qualifiedItemId === "(O)2"
    && typeof value.health === "number" && Number.isSafeInteger(value.health) && value.health === 1;
}
...
function isMachineTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId) && isTileCoordinate(value.x) && isTileCoordinate(value.y)
    ...
    && (value.loadInputQualifiedItemId === undefined || value.loadInputQualifiedItemId === null || value.loadInputQualifiedItemId === "(O)433")
    && (value.loadInputStack === undefined || value.loadInputStack === null || value.loadInputStack === 5)
    && (value.collectOutputReady === undefined || value.collectOutputReady === null || typeof value.collectOutputReady === "boolean")
    && ((value.loadInputSlot === undefined && value.loadInputQualifiedItemId === undefined && value.loadInputStack === undefined) || (isToolSlot(value.loadInputSlot) && value.loadInputQualifiedItemId === "(O)433" && value.loadInputStack === 5));
}
...
function isTreeChopSourceTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId)
    ...
    && typeof value.growthStage === "number" && Number.isSafeInteger(value.growthStage) && value.growthStage >= 5 && value.growthStage <= 20
    && value.health === 1 && value.stump === false && value.moss === false && value.tapped === false;
}
...
function isTreeChopResultTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId)
    ...
    && isTileCoordinate(value.x) && isTileCoordinate(value.y)
    && typeof value.treeType === "string" && value.treeType.length > 0 && value.treeType.length <= 32
    && value.health === 5 && value.stump === true && value.moss === false && value.tapped === false;
}
...
function isTreeShakeSourceTargetFact(value: unknown): boolean {
  return isRecord(value) && isOpaqueId(value.targetId)
    ...
    && isFiniteNumber(value.health) && value.health >= -100 && value.health <= 100
    && typeof value.moss === "boolean" && typeof value.tapped === "boolean";
}
