...
test("presentation tools are materialized only for configured and mounted surfaces", () => {
  ...
  const tools = createCompanionPresentationTools({
    profile: profile({ voiceProfile: "companion.default", perUtteranceDirection: true }, true), sessionId: "session_01",
  ...
  const noSurface = createCompanionPresentationTools({ profile: profile({ voiceProfile: "companion.default", perUtteranceDirection: true }, true), sessionId: "session_02" });
...
test("text and speech stay independent and reject mechanism language", async () => {
  ...
  await speechTool.execute("tool_call_speech_01", { line: "我们先看看门口。", direction: "自然、简短，不打断对方。" }, new AbortController().signal, () => {}, {} as never);
  assert.deepEqual(text.map((item) => item.text), ["我在这边等你。"]);
  assert.deepEqual(speech.map((item) => ({ text: item.text, direction: item.direction, sourceEventId: item.sourceEventId })), [{ text: "我们先看看门口。", direction: "自然、简短，不打断对方。", sourceEventId: "tool_call_speech_01" }]);
  ...
  await assert.rejects(() => speechTool.execute("bad", { line: "我会调用 subagent", direction: "自然" }, new AbortController().signal, () => {}, {} as never), /invalid_player_expression/);
});

test("speech-only provider without direction does not expose a direction schema", async () => {
  const speech: VoiceExpression[] = [];
  const tools = createCompanionPresentationTools({ profile: profile({ voiceProfile: "plain", perUtteranceDirection: false }), sessionId: "session_01", speechPort: { enqueue(expression) { speech.push(expression); } } });
  const tool = tools[0]!;
  await tool.execute("speech", { line: "只说这句话。" }, new AbortController().signal, () => {}, {} as never);
  assert.equal(speech[0]?.direction, undefined);
  assert.rejects(() => tool.execute("speech", { line: "只说这句话。", direction: "多余" }, new AbortController().signal, () => {}, {} as never), /invalid/).catch(() => undefined);
});
