...
export function createReferencePipelineDialogueWebRequestHandler(
  ...
  if (!isOpaqueHandle(bootstrapToken)) throw new Error("reference_pipeline_bootstrap_token_invalid");
  ...
  const dispatch = async (request: IncomingMessage, response: ServerResponse, origin: string): Promise<void> => {
    ...
    try {
      if (request.method === "POST" && url.pathname === "/api/tavern/v1/bootstrap") {
        ...
        return await sendProjectedSnapshot(response, referenceStateFacade, profile, session);
      }
      ...
      if (request.method === "GET" && url.pathname === "/api/tavern/v1/draft") {
        if (url.search !== "" || (await hasRequestBody(request))) return sendProblem(response, 400, "invalid_request");
        if (!authenticate(request, browser, origin)) return sendProblem(response, 401, "unauthorized");
...
function sendJsonAfterFinish(response: ServerResponse, status: number, body: unknown): Promise<void> {
