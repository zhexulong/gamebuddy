...
function hasWaterCropCompletionEvidence(detail: string): boolean {
  ...
  return (
    ...
    before !== null &&
    after !== null &&
    // ExecutionManager emits only waterConsumed plus before/after quantities:
    // bottomless cans and watering-can enchantments legitimately report a
    // consumed watering operation without decrementing WaterLeft. Therefore
    // accept exactly the producer-observable relation, 0 or 1 used, rather
    // than inventing an unavailable branch discriminator.
    after <= before &&
    before - after <= 1 &&
