...
function hasStardewCompletionEvidence(actionId: string, receipt: IntegrationReceiptEvidence): boolean {
  ...
  const requirements: Readonly<Record<string, Readonly<{ reasonCode: string; keys: readonly string[] }>>> = {
    ...
    plant_seed: { reasonCode: "seed_planted", keys: ["target=", "item=", "crop=", "inventory_before=", "inventory_after="] },
    fertilize_tile: { reasonCode: "fertilizer_applied", keys: ["target=", "inventory_"] },
    place_wood_fence: { reasonCode: "wood_fence_placed", keys: ["target=", "item=(O)322", "is_fence=true", "is_gate=false", "health=", "max_health=", "inventory_before=", "inventory_after="] },
    place_crab_pot: { reasonCode: "crab_pot_placed", keys: ["target=", "item=(O)710", "is_crab_pot=true", "owner=", "offset_x=", "offset_y=", "overlay_tiles=", "inventory_before=", "inventory_after="] },
    ...
    break_rock_source: { reasonCode: "rock_source_broken", keys: ["target=", "tool=pickaxe", "slot=", "qualified_item_id=(O)2", "durability_before=1", "durability_after=removed", "removed=true"] },
    chop_tree_source: { reasonCode: "tree_source_chopped", keys: ["target=", "tool=axe", "slot=", "tree=", "health_before=1", "health_after=5", "stump_before=false", "stump_after=true", "source_transformed=true"] },
  };
  const requirement = requirements[actionId];
  if (requirement === undefined
    || receipt.state !== "succeeded"
    || receipt.reasonCode !== requirement.reasonCode
    || !requirement.keys.every((key) => detail.includes(key))) return false;
  ...
  return hasOpaqueEvidenceValue(evidence.target)
...
function hasTillSoilCompletionEvidence(detail: string): boolean {
  ...
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
  return hasBoundedNonemptyEvidenceValue(evidence.location)
...
function hasWaterCropCompletionEvidence(detail: string): boolean {
  const evidence = parseSemicolonEvidence(detail);
  if (evidence === null) return false;
  const expectedKeys = ["location", "target", "tile", "before_watered", "after_watered", "water_before", "water_after", "water_consumed"];
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
...
function hasDigArtifactSpotCompletionEvidence(detail: string): boolean {
  const evidence = parseSemicolonEvidence(detail);
  if (evidence === null) return false;
  const expectedKeys = ["location", "target", "result_target", "tile", "tool", "slot", "stamina_before", "stamina_after", "stamina_delta", "expected_stamina_cost", "qualified_item_id", "source_present_before", "source_present_after", "hoedirt_present_before", "hoedirt_present_after", "source_removed"];
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
  ...
  return hasBoundedNonemptyEvidenceValue(evidence.location)
    ...
    && hasTileEvidenceValue(evidence.tile)
    && evidence.tool === "hoe"
    && slot !== null && slot >= 0 && slot <= 36
    && staminaBefore !== null && staminaAfter !== null && staminaDelta !== null && expectedStaminaCost !== null
...
function hasRefillWateringCanCompletionEvidence(detail: string): boolean {
  ...
  return hasOpaqueIdEvidenceValue(evidence.target)
...
function hasBreakRockSourceCompletionEvidence(detail: string): boolean {
  const evidence = parseSemicolonEvidence(detail);
  if (evidence === null) return false;
  const expectedKeys = ["target", "tool", "slot", "qualified_item_id", "durability_before", "durability_after", "removed"];
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
  const slot = integerEvidenceValue(evidence.slot);
  return hasOpaqueIdEvidenceValue(evidence.target)
...
function hasClearHoeDirtCompletionEvidence(detail: string): boolean {
  const evidence = parseSemicolonEvidence(detail);
  if (evidence === null) return false;
  const expectedKeys = ["location", "target", "tile", "tool", "slot", "crop_before", "hoedirt_present_before", "hoedirt_present_after", "removed"];
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
  const slot = integerEvidenceValue(evidence.slot);
  return hasBoundedNonemptyEvidenceValue(evidence.location)
    && hasOpaqueIdEvidenceValue(evidence.target)
...
function hasWoodFenceCompletionEvidence(detail: string): boolean {
  ...
  const before = integerEvidenceValue(evidence.inventory_before);
  const after = integerEvidenceValue(evidence.inventory_after);
  return evidence.source === "(O)322"
    && hasBoundedNonemptyEvidenceValue(evidence.location)
    && x !== null && x <= 1000 && y !== null && y <= 1000
    && hasOpaqueIdEvidenceValue(evidence.target)
    && evidence.item === "(O)322"
    && slot !== null && slot <= 36
    ...
    && maxHealth !== null && maxHealth >= health
    && before === 1 && after === 0;
}
...
function hasChopTreeSourceCompletionEvidence(detail: string): boolean {
  const evidence = parseSemicolonEvidence(detail);
  if (evidence === null) return false;
  const expectedKeys = ["target", "tool", "slot", "tree", "health_before", "health_after", "stump_before", "stump_after", "source_transformed"];
  if (Object.keys(evidence).length !== expectedKeys.length || !expectedKeys.every((key) => key in evidence)) return false;
  const slot = integerEvidenceValue(evidence.slot);
  return hasOpaqueIdEvidenceValue(evidence.target)
    && evidence.tool === "axe"
