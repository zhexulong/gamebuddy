...
export function parseDialogueLaunchMode(args: readonly string[]): Readonly<{
  ...
  if (knownRootRecoveryCount > 1 || managementProfileCount > 1) throw new Error("dialogue_launch_mode_rejected");

  const positional = args.filter(
    (argument) => argument !== "--known-root-recovery" && argument !== "--tavern-management",
  );
  if (
    positional.length > 1 ||
    positional.some((argument) => argument.length === 0 || argument.startsWith("--"))
  )
    throw new Error("dialogue_launch_mode_rejected");
