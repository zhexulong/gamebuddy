...
function snapshot(revision = 3) {
  return { revision, location: "Farm", tile: { x: 10, y: 11 }, stamina: 270, health: 100, actionable: true, capabilities: ["move_to_tile", "inspect_self"], activeExecution: null } as const;
}
...
test("integration client exposes only Mod-originated state and receipts", () => {
  ...
  const request = { requestId: "request_01", idempotencyKey: "idempotency_01", action: "move_to_tile" as const, args: { x: 11, y: 12 }, expectedRevision: 3, deadlineMs: now + 10_000 };
