...
const baseSession = {
  schemaVersion: 1, integrationId: "stardew", integrationVersion: "0.1.0", gameVersion: "1.6.15", gameBuildNumber: 24356, smapiVersion: "4.5.2", multiplayerProtocol: "1.6.15",
  endpoint: "127.0.0.1:24642", saveId: "save_01", worldId: "world_01", hostPlayerId: "world_01", publishedAtUnixMs: 1_000,
  expiresAtUnixMs: 20_000, nonce: "nonce_01", state: "ready", cabins: [{ cabinId: "cabin_01", ownerFarmhandId: "", boundCompanionId: "", isBusy: false }], signature: "",
};

function signed<T extends { signature: string }>(value: T): T {
  const unsigned = { ...value } as Record<string, unknown>;
  delete unsigned.signature;
  return { ...value, signature: createHmac("sha256", token).update(JSON.stringify(unsigned), "utf8").digest("base64url") };
}
...
test("Stardew attachment flow requires explicit confirmation and emits only a bounded request", async () => {
  ...
  try {
    ...
    const request = JSON.parse(await readFile(join(directory, "stardew-attachment-request.json"), "utf8")) as Record<string, unknown>;
...
test("Stardew attachment flow rejects stale or unauthenticated advertisements", async () => {
  const directory = await mkdtemp(join(tmpdir(), "gamebuddy-stardew-"));
  try {
    await writeFile(join(directory, "stardew-session.json"), JSON.stringify({ ...baseSession, signature: "tampered-session-signature" }));
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", nowMs: () => 2_000 });
...
test("Stardew attachment flow waits through awaiting_save and rejects a manifest for another request", async () => {
  ...
  try {
    ...
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", nowMs: () => 2_000 });
    const response = signed({ schemaVersion: 1, requestId: "request_01", state: "awaiting_save", reasonCode: "binding_persist_pending", updatedAtUnixMs: 2_001, signature: "" });
    ...
    const finalResponse = signed({ schemaVersion: 1, requestId: "request_01", state: "rejected", reasonCode: "binding_readback_mismatch", updatedAtUnixMs: 2_100, signature: "" });
    ...
    const mismatch = signed({ schemaVersion: 1, requestId: "request_02", state: "ready", reasonCode: "manifest_issued", updatedAtUnixMs: 2_200, manifestPath: "stardew-farmhand-manifest.json", signature: "" });
    await writeFile(requestFile, JSON.stringify(mismatch));
    await assert.rejects(() => flow.waitForManifest("request_01", 200), /stardew_attachment_rejected_binding_readback_mismatch|stardew_attachment_timeout/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("Stardew attachment flow accepts signed 64-bit negative native IDs in unrelated cabins", async () => {
  const directory = await mkdtemp(join(tmpdir(), "gamebuddy-stardew-"));
  try {
    const session = signed({ ...baseSession, cabins: [
    ...
    await writeFile(join(directory, "stardew-session.json"), JSON.stringify(session));
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", expectedFarmhandId: "123456789", nowMs: () => 2_000 });
...
test("Stardew attachment flow rejects signed manifests with a different save/world scope", async () => {
  const directory = await mkdtemp(join(tmpdir(), "gamebuddy-stardew-"));
  try {
    const session = signed({ ...baseSession, cabins: [{ cabinId: "cabin_01", ownerFarmhandId: "123456789", boundCompanionId: "companion_01", isBusy: false }] });
    await writeFile(join(directory, "stardew-session.json"), JSON.stringify(session));
    const manifest = signed({
      schemaVersion: 1, requestId: "request_01", integrationId: "stardew", integrationVersion: "0.1.0", gameVersion: "1.6.15", gameBuildNumber: 24356, smapiVersion: "4.5.2", multiplayerProtocol: "1.6.15",
      endpoint: "127.0.0.1:24642", saveId: "other_save", worldId: session.worldId, companionId: "companion_01", farmhandId: "123456789", cabinId: "cabin_01", sessionNonce: session.nonce,
      issuedAtUnixMs: 2_000, expiresAtUnixMs: 19_000, signature: "",
    });
    await writeFile(join(directory, "stardew-farmhand-manifest.json"), JSON.stringify(manifest));
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", nowMs: () => 2_000 });
    await assert.rejects(() => flow.readIssuedManifest("stardew-farmhand-manifest.json", "request_01"), /stardew_manifest_session_mismatch/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("Stardew attachment flow rejects a tampered manifest before it can be consumed", async () => {
  ...
  try {
    ...
    const manifest = signed({
      schemaVersion: 1, requestId: "request_01", integrationId: "stardew", integrationVersion: "0.1.0", gameVersion: "1.6.15", gameBuildNumber: 24356, smapiVersion: "4.5.2", multiplayerProtocol: "1.6.15",
      endpoint: "127.0.0.1:24642", saveId: session.saveId, worldId: session.worldId, companionId: "companion_01", farmhandId: "123456789", cabinId: "cabin_01", sessionNonce: session.nonce,
      issuedAtUnixMs: 2_000, expiresAtUnixMs: 19_000, signature: "",
    });
    await writeFile(join(directory, "stardew-farmhand-manifest.json"), JSON.stringify({ ...manifest, farmhandId: "987654321" }));
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", nowMs: () => 2_000 });
    await assert.rejects(() => flow.readIssuedManifest("stardew-farmhand-manifest.json", "request_01"), /stardew_manifest_cabin_binding_mismatch/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("Stardew attachment flow rejects a manifest with a tampered signature", async () => {
  ...
  try {
    ...
    await writeFile(join(directory, "stardew-farmhand-manifest.json"), JSON.stringify({ ...manifest, signature: "tampered-manifest-signature" }));
    const flow = new StardewAttachmentFlow({ sessionDirectory: directory, sessionToken: token, companionId: "companion_01", cabinId: "cabin_01", nowMs: () => 2_000 });
    await assert.rejects(() => flow.readIssuedManifest("stardew-farmhand-manifest.json", "request_01"), /stardew_manifest_authentication_failed/);
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("Stardew attachment flow accepts the Host fixed manifest filename only after session binding", async () => {
  ...
  try {
    ...
    await assert.rejects(() => flow.readIssuedManifest(join(directory, "other.json"), "request_01"), /invalid_manifest_path/);
