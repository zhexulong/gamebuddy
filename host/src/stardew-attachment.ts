...
export class StardewAttachmentFlow {
  ...
  public constructor(options: StardewAttachmentOptions) {
    if (!isAbsoluteSafePath(options.sessionDirectory) || !isToken(options.sessionToken) || !isOpaque(options.companionId) || !isOpaque(options.cabinId)) {
      throw new Error("invalid_stardew_attachment_options");
    }
    if (options.expectedFarmhandId !== undefined && !isNativeId(options.expectedFarmhandId)) throw new Error("invalid_expected_farmhand_id");
    this.#options = options;
  }

  public async readLiveSession(): Promise<StardewSessionAdvertisement> {
    const value = JSON.parse(await readFile(resolve(this.#options.sessionDirectory, "stardew-session.json"), "utf8")) as unknown;
    ...
    if (session.integrationId !== STARDew_INTEGRATION_ID || session.schemaVersion !== STARDew_PROVISIONING_VERSION) throw new Error("stardew_protocol_mismatch");
    if (session.state !== "ready") throw new Error(`stardew_${session.state}`);
    return session;
  }

  public async confirmAndRequest(session: StardewSessionAdvertisement, confirmation: Readonly<{ confirmed: boolean; expectedFarmhandId?: string }> = { confirmed: false }): Promise<string> {
    if (!confirmation.confirmed) throw new Error("user_confirmation_required");
    const current = await this.readLiveSession();
    if (!verifySignature(session, this.#options.sessionToken)
    ...
    if (cabin.boundCompanionId !== "" && cabin.boundCompanionId !== this.#options.companionId) throw new Error("cabin_bound_to_other_companion");
    const expectedFarmhandId = confirmation.expectedFarmhandId ?? this.#options.expectedFarmhandId ?? cabin.ownerFarmhandId;
    ...
    await atomicWrite(resolve(this.#options.sessionDirectory, "stardew-attachment-request.json"), JSON.stringify(signed));
    return request.requestId;
  }

  public async waitForResponse(requestId: string, timeoutMs = 60_000): Promise<StardewAttachmentResponse> {
    if (!isOpaque(requestId) || !Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 120_000) throw new Error("invalid_attachment_timeout");
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      try {
        const value = JSON.parse(await readFile(resolve(this.#options.sessionDirectory, "stardew-attachment-response.json"), "utf8")) as unknown;
        const response = validateResponse(value);
        if (!verifySignature(response, this.#options.sessionToken)) throw new Error("stardew_response_authentication_failed");
  ...
  public async readIssuedManifest(manifestPath: string, requestId: string): Promise<StardewJoinManifest> {
    ...
    if (manifest.expiresAtUnixMs <= now || manifest.issuedAtUnixMs > now + 30_000) throw new Error("stardew_manifest_expired");
    if (manifest.companionId !== this.#options.companionId || manifest.cabinId !== this.#options.cabinId) throw new Error("stardew_manifest_identity_mismatch");
    const cabinMatches = current.cabins.filter(cabin => cabin.cabinId === manifest.cabinId);
    if (cabinMatches.length !== 1 || cabinMatches[0].ownerFarmhandId !== manifest.farmhandId || (cabinMatches[0].boundCompanionId !== "" && cabinMatches[0].boundCompanionId !== manifest.companionId)) throw new Error("stardew_manifest_cabin_binding_mismatch");
    if (manifest.sessionNonce !== current.nonce || manifest.saveId !== current.saveId || manifest.worldId !== current.worldId || manifest.endpoint !== current.endpoint || manifest.integrationVersion !== current.integrationVersion || manifest.gameVersion !== current.gameVersion || manifest.gameBuildNumber !== current.gameBuildNumber || manifest.smapiVersion !== current.smapiVersion || manifest.multiplayerProtocol !== current.multiplayerProtocol) throw new Error("stardew_manifest_session_mismatch");
    if (!verifySignature(manifest, this.#options.sessionToken)) throw new Error("stardew_manifest_authentication_failed");
    return manifest;
  }
  ...
  private resolveManifestPath(manifestPath: string): string {
    ...
    if (!isAbsoluteSafePath(manifestPath) || resolve(manifestPath) !== expected) throw new Error("invalid_manifest_path");
    return expected;
  }
...
function validateResponse(value: unknown): StardewAttachmentResponse {
  if (!isRecord(value)
    ...
    || (value.state === "ready" ? value.manifestPath !== "stardew-farmhand-manifest.json" : value.manifestPath !== undefined)
    || !isToken(value.signature))
...
function isToken(value: unknown): value is string { return typeof value === "string" && /^[\x21-\x7e]{16,256}$/.test(value); }
function isReason(value: unknown): value is string { return typeof value === "string" && /^[a-z0-9_:-]{1,128}$/.test(value); }
function isVersion(value: unknown): value is string { return typeof value === "string" && value.length >= 1 && value.length <= 64 && /^[A-Za-z0-9._+-]+$/.test(value); }
...
function isEndpoint(value: unknown): value is string {
  if (typeof value !== "string" || value.length < 3 || value.length > 255 || !/^[A-Za-z0-9.-]+:[0-9]{1,5}$/.test(value)) return false;
  const port = Number(value.slice(value.lastIndexOf(":") + 1));
  return port >= 1 && port <= 65535;
}
function isAbsoluteSafePath(value: string): boolean { return typeof value === "string" && value.length > 0 && isAbsolute(value) && !basename(value).includes(".."); }
