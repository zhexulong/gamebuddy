...
test("till_soil completion evidence requires exact location, coordinates, and soil transition", () => {
  ...
  for (const malformed of [
    ...
    assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("till_soil", { ...receipt, evidence: { detail: malformed } }), false);
  }
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("till_soil", { ...receipt, state: "accepted" }), false);
});

test("water_crop completion evidence requires exact opaque target, transition, and one water unit", () => {
  const valid = "location=Farm;target=crop_abcdef0123456789;tile=38,18;before_watered=false;after_watered=true;water_before=40;water_after=39;water_consumed=true";
  ...
  for (const malformed of [
    ...
    assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("water_crop", { ...receipt, evidence: { detail: malformed } }), false);
  }
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("water_crop", { ...receipt, state: "uncertain" }), false);
});

test("place_wood_fence completion evidence is exact and fail-closed", () => {
  const valid = "source=(O)322;location=Farm;x=10;y=12;target=wood_fence_deadbeef;item=(O)322;slot=4;source_empty_before=true;is_fence=true;is_gate=false;health=99;max_health=100;inventory_before=1;inventory_after=0";
...
test("dig_artifact_spot completion evidence is strict and source-only", () => {
  ...
  for (const malformed of [
    ...
    valid.replace("stamina_before=100;stamina_after=98;stamina_delta=-2;expected_stamina_cost=2", "stamina_before=270;stamina_after=268;stamina_delta=-2;expected_stamina_cost=999"),
  ...
  ]) assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("dig_artifact_spot", { ...receipt, evidence: { detail: malformed } }), false);
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("dig_artifact_spot", { ...receipt, evidence: { detail: valid.replace("stamina_before=100;stamina_after=98;stamina_delta=-2;expected_stamina_cost=2", "stamina_before=100;stamina_after=98.5;stamina_delta=-1.5;expected_stamina_cost=1.5") } }), true);
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("dig_artifact_spot", { ...receipt, state: "accepted" }), false);
});
