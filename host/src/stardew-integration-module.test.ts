...
test("every published Stardew action has an explicit fail-closed completion rule", () => {
  for (const action of PUBLISHED_STARDEW_ACTIONS)
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(action.actionId, {
        state: "succeeded",
...
test("equip completion rejects malicious substring detail and accepts only the exact Mod fixture", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "tool_selected", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("equip_tool", receipt), true);
  for (const malicious of [
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("equip_tool", {
        ...receipt,
...
test("plant, refill, break, and chop completion evidence reject opaque sentinels and out-of-range slots", () => {
  ...
  for (const [actionId, reasonCode, valid, expected, invalid] of cases) {
    const receipt = { state: "succeeded", reasonCode, evidence: { detail: valid } } as const;
    assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(actionId, receipt), true, actionId);
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(actionId, {
        ...receipt,
...
test("every accepted semantic evidence field rejects protocol scalar sentinels", () => {
  const cases = [
    ["equip_tool", "tool_selected", "slot=1;before=none;expected=Axe;after=Axe", ["before", "expected", "after"]],
    ["till_soil", "soil_tilled", "location=Farm;target=37,18;before=none;after=HoeDirt", ["location", "target"]],
    ["move_to_tile", "target_reached", "tile=37,18;target=37,18;arrival=exact;path=stardew_native", ["tile", "target"]],
    [
      "water_crop",
      "crop_watered",
  ...
  const scalars = ["none", "true", "false", "null", "undefined", "nan", "infinity"];
    ...
    for (const field of fields)
      for (const scalar of scalars)
        assert.equal(
          STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(actionId, {
            ...receipt,
            evidence: { detail: valid.replace(new RegExp(`(${field}=)[^;]+`), `$1${scalar}`) },
          }),
...
test("move_to_tile completion evidence accepts exact native arrival after :0.## tile serialization", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "target_reached", evidence: { detail: valid } } as const;
  for (const producerPermitted of [
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("move_to_tile", {
        ...receipt,
...
test("water_crop completion evidence accepts both Mod water relations and rejects inconsistent variants", () => {
  ...
  // ExecutionManager emits no bottomless/enchantment discriminator. Its
  // producer-defined waterConsumed=true branch can leave WaterLeft unchanged.
  const noDecrement = normal.replace("water_before=40;water_after=39", "water_before=40;water_after=40");
  const receipt = { state: "succeeded", reasonCode: "crop_watered", evidence: { detail: normal } } as const;
  ...
  for (const malformed of [
    ...
    normal.replace("water_before=40;water_after=39", "water_before=40;water_after=41"),
    normal.replace("water_before=40;water_after=39", "water_before=40;water_after=38"),
    normal.replace("water_before=40;water_after=39", "water_before=40.5;water_after=39.5"),
    normal.replace("water_before=40;water_after=39", "water_before=Infinity;water_after=39"),
...
test("travel and enter_exit completion evidence require the exact warped destination", () => {
  ...
  assert.equal(
    STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("enter_exit", {
      state: "succeeded",
  ...
  for (const [actionId, reasonCode, malformed] of [
    ["travel", "travel_completed", travelValid.replace("actual=BusStop:20,12", "actual=BusStop:20,13")],
    ["travel", "travel_completed", travelValid.replace("actual=BusStop:20,12", "actual=Town:20,12")],
    ["enter_exit", "enter_exit_completed", exitValid.replace("expected=SeedShop:4,9", "expected=SeedShop:4,09")],
    ["enter_exit", "enter_exit_completed", exitValid.replace(";actual=SeedShop:4,9", "")],
    ["travel", "travel_completed", travelValid.replace("expected=BusStop", "expected=none")],
    ["travel", "travel_completed", travelValid.replace("actual=BusStop:20,12", "actual=BusStop:1001,12")],
    ["travel", "travel_completed", `${travelValid};actual=BusStop:20,12`],
    ["travel", "travel_completed", travelValid.replace("actual=BusStop:20,12", "actual=BusStop:x,12")],
  ] as const) {
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(actionId, {
        state: "succeeded",
  ...
  assert.equal(
    STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("travel", {
      ...travelReceipt,
      reasonCode: "enter_exit_completed",
    }),
    false,
...
test("pickup_forage completion evidence requires the native removal and inventory gain", () => {
  const valid = "location=Forest;target=12,9;item=(O)16;removed=true;inventory_before=0;inventory_after=1";
  const receipt = { state: "succeeded", reasonCode: "forage_picked_up", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("pickup_forage", receipt), true);
  for (const malformed of [
    valid.replace("removed=true", "removed=false"),
    valid.replace("inventory_before=0;inventory_after=1", "inventory_before=1;inventory_after=1"),
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("pickup_forage", {
        ...receipt,
        evidence: { detail: malformed },
      }),
      false,
...
test("fertilize_tile completion evidence requires the exact applied fertilizer and decrement", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "fertilizer_applied", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("fertilize_tile", receipt), true);
  for (const malformed of [
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("fertilize_tile", {
        ...receipt,
        evidence: { detail: malformed },
      }),
      false,
...
test("place_crab_pot completion evidence requires the exact native placement facts", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "crab_pot_placed", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("place_crab_pot", receipt), true);
  for (const malformed of [
    valid.replace("overlay_tiles=11,12:1|10,13:2", "overlay_tiles=11,12:0"),
    valid.replace("overlay_tiles=11,12:1|10,13:2", "overlay_tiles=11,12:1|10,13"),
    valid.replace("overlay_tiles=11,12:1|10,13:2", "overlay_tiles=11:1|10,13:2"),
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("place_crab_pot", {
        ...receipt,
        evidence: { detail: malformed },
      }),
      false,
...
test("machine inspection, load, and collect completion evidence are exact Keg contracts", () => {
  ...
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("machine_inspect", inspectReady), true);
  for (const malformed of [
    inspect.evidence.detail.replace("minutes_until_ready=120", "minutes_until_ready=1.5"),
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("machine_inspect", {
        ...inspect,
        evidence: { detail: malformed },
      }),
      false,
  ...
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("machine_load", load), true);
  for (const malformed of [
    load.evidence.detail.replace("minutes_until_ready=120", "minutes_until_ready=119"),
    load.evidence.detail.replace("input_stack_after=removed", "input_stack_after=4"),
    load.evidence.detail.replace("input_stack_before=5", "input_stack_before=4"),
    load.evidence.detail.replace("last_input=(O)433", "last_input=none"),
    load.evidence.detail.replace("held=(O)395", "held=(O)395_extra"),
    load.evidence.detail.replace("native_check_action=true", "native_check_action=false"),
    load.evidence.detail.replace("input=(O)433", "input=(O)472"),
    `${load.evidence.detail};minutes_until_ready=120`,
  ])
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("machine_load", {
        ...load,
        evidence: { detail: malformed },
      }),
      false,
  ...
  for (const malformed of [
    ...
    collect.evidence.detail.replace("minutes_until_ready_before=0", "minutes_until_ready_before=0.0"),
    `${collect.evidence.detail};native_check_action=true`,
  ])
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("machine_collect_output", {
        ...collect,
        evidence: { detail: malformed },
      }),
      false,
...
test("use_item completion evidence requires the native consumption and animation completion", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "item_used", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("use_item", receipt), true);
  const stacked = valid.replace("stack_before=1;stack_after=0", "stack_before=5;stack_after=4");
  assert.equal(
    STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("use_item", {
      ...receipt,
  ...
  for (const malformed of [
    valid.replace("stack_before=1;stack_after=0", "stack_before=1;stack_after=1"),
    valid.replace("stack_before=1;stack_after=0", "stack_before=5;stack_after=5"),
...
test("feed_animal completion evidence requires the exact trough placement", () => {
  ...
  const receipt = { state: "succeeded", reasonCode: "hay_placed_in_trough", evidence: { detail: valid } } as const;
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("feed_animal", receipt), true);
  for (const malformed of [
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("feed_animal", {
        ...receipt,
        evidence: { detail: malformed },
      }),
      false,
...
test("harvest_crop completion evidence distinguishes regrowing and destroyed crops", () => {
  ...
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("harvest_crop", destroyed), true);
  const regrow = {
    ...destroyed,
    evidence: {
      detail:
  ...
  assert.equal(STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("harvest_crop", regrow), true);
  for (const malformed of [
    destroyed.evidence.detail.replace("native_path_return=true", "native_path_return=false"),
    destroyed.evidence.detail.replace("crop_present_after=false", "crop_present_after=true"),
    destroyed.evidence.detail.replace("phase_after=none", "phase_after=4"),
    destroyed.evidence.detail.replace("day_of_phase_after=none", "day_of_phase_after=2"),
    destroyed.evidence.detail.replace("inventory_after=1", "inventory_after=0"),
    destroyed.evidence.detail.replace("inventory_gained=true", "inventory_gained=false"),
    ...
    assert.equal(
      STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence("harvest_crop", {
        ...destroyed,
        evidence: { detail: malformed },
      }),
      false,
...
test("completion parsers never accept another action's succeeded evidence", () => {
  ...
  for (const [actionId, , detail] of cross) {
    ...
    for (const other of PUBLISHED_STARDEW_ACTIONS) {
      if (other.actionId === actionId) continue;
      assert.equal(
        STARDEW_INTEGRATION_MODULE.actionCatalog.hasCompletionEvidence(other.actionId, receipt),
        false,
