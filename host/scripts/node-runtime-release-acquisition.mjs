...
async function closure(root, prefix = "") { const entries = []; for (const item of await readdir(resolve(root, prefix), { withFileTypes: true })) { const path = prefix ? `${prefix}/${item.name}` : item.name; const absolute = resolve(root, path); const state = await lstat(absolute); if (state.isSymbolicLink() || (!state.isDirectory() && !state.isFile())) throw new Error("runtime_extraction_nonregular_entry"); if (state.isDirectory()) entries.push(...await closure(root, path)); else entries.push({ sourcePath: slash(path), sha256: sha256(await readFile(absolute)) }); } return entries.sort((left, right) => left.sourcePath.localeCompare(right.sourcePath)); }
