...
async function readLockedSdkVersion() {
  ...
  catch { throw new Error("windows_stardew_folder_picker_dotnet_sdk_lock_invalid"); }
  const version = parsed?.sdk?.version;
  if (typeof version !== "string" || !expectedSdkVersionPattern.test(version)) throw new Error("windows_stardew_folder_picker_dotnet_sdk_lock_invalid");
  return version;
}
...
async function verifyOutputRootChain() {
  ...
  for (;;) {
    ...
    if (state !== undefined) {
      if (!state.isDirectory() || state.isSymbolicLink()) throw new Error("windows_stardew_folder_picker_output_unsafe");
      await verifyPhysicalPath(current);
      break;
    }
    missing.push(current);
    const parent = dirname(current);
    if (parent === current) throw new Error("windows_stardew_folder_picker_output_unsafe");
    current = parent;
  }
...
async function verifyPhysicalPath(path) {
  ...
  if (normalized(path).toLowerCase() !== normalized(physical).toLowerCase()) {
    throw new Error("windows_stardew_folder_picker_output_unsafe");
  }
}
...
export function canonicalManifest(sha256) {
  if (!/^[a-f0-9]{64}$/.test(sha256)) throw new Error("windows_stardew_folder_picker_helper_hash_invalid");
...
function minimalDotnetEnvironment() {
  const temporaryRoot = resolve(repositoryRoot, ".tmp", "windows-stardew-folder-picker");
  return {
...
async function runBoundedDotnet(command, args) {
  return await new Promise((resolveRun, rejectRun) => {
    ...
    } catch (error) { finish(new Error("windows_stardew_folder_picker_dotnet_spawn_failed", { cause: error })); return; }
    const collect = (chunk) => {
    ...
    child.once("error", (error) => { clearTimeout(timer); finish(new Error("windows_stardew_folder_picker_dotnet_spawn_failed", { cause: error })); });
    child.once("close", (code, signal) => {
      clearTimeout(timer);
      if (timedOut) return finish(new Error("windows_stardew_folder_picker_dotnet_timeout"));
      if (outputBytes > outputLimitBytes) return finish(new Error("windows_stardew_folder_picker_dotnet_output_overflow"));
      if (code !== 0 || signal) return finish(new Error("windows_stardew_folder_picker_dotnet_failed"));
...
async function assertLockedSdk(dotnet) {
  ...
  if (actual !== expected) throw new Error("windows_stardew_folder_picker_dotnet_sdk_drift");
}

export async function buildWindowsStardewFolderPicker() {
  if (process.platform !== "win32") throw new Error("windows_stardew_folder_picker_build_requires_windows");
  ...
  if (!reservation.isDirectory() || reservation.isSymbolicLink()) throw new Error("windows_stardew_folder_picker_output_unsafe");
  await verifyPhysicalPath(outputRoot);
  ...
  if (!helperState?.isFile() || helperState.isSymbolicLink()) throw new Error("windows_stardew_folder_picker_helper_missing");
  await verifyPhysicalPath(helperPath);
  ...
  if (publishedEntries.length !== 1 || publishedEntries[0] !== helperFileName) throw new Error("windows_stardew_folder_picker_output_unsafe");
  ...
  if (preExistingManifest !== undefined) throw new Error("windows_stardew_folder_picker_output_unsafe");
  await writeFile(manifestPath, canonicalManifest(sha256), { encoding: "utf8", flag: "wx" });
  const manifestState = await lstat(manifestPath);
  if (!manifestState.isFile() || manifestState.isSymbolicLink()) throw new Error("windows_stardew_folder_picker_output_unsafe");
  await verifyPhysicalPath(manifestPath);
  const completedEntries = await readdir(outputRoot);
  if (completedEntries.length !== 2 || !completedEntries.includes(helperFileName) || !completedEntries.includes(manifestFileName)) throw new Error("windows_stardew_folder_picker_output_unsafe");
  const pair = await readVerifiedPair();
  if (pair === undefined) throw new Error("windows_stardew_folder_picker_output_unsafe");
  return pair;
}

if (process.argv[1] === scriptPath) {
  try {
    const result = await buildWindowsStardewFolderPicker();
    process.stdout.write(`${canonicalManifest(result.sha256)}`);
  } catch (error) {
    process.stderr.write(`${error instanceof Error ? error.message : "windows_stardew_folder_picker_build_failed"}\n`);
    process.exitCode = 1;
  }
}
