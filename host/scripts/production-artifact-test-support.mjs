...
function testDescriptor(source) { return { runtimePath: "runtime/node.exe", bootstrapPath: "desktop-host-entry.internal.js", nodeSha256: source.descriptor.nodeSha256 }; }
...
async function selectedTestArtifact({ hostRoot, outputRoot }) {
  ...
  const config = testConfig(await readArtifactConfig(hostRoot), { runtimePath: "runtime/node.exe", bootstrapPath: "desktop-host-entry.internal.js" });
