...
async function liveReport(invocation, result, registration, dependencies) {
  ...
  return report(invocation, "live", {
    outcome,
    ...(outcome === "passed" ? {} : { reasonCode: safeReason(reason, `live_${outcome}`) }),
  });
}

function actionRegistryFor(dependencies) {
  // This module-private seam is reserved for deterministic package tests. The
  // production registration factory remains the canonical owner registry.
  return dependencies?.__testOnlyActionRegistrations === undefined
    ? ACTION_REGISTRY
    : createActionRegistry(dependencies.__testOnlyActionRegistrations);
}
...
export async function runActionProject({ manifest, invocation, dependencies }) {
  ...
  const registry = actionRegistryFor(dependencies);
  ...
  if (invocation.command === "run-live") {
    ...
    return await liveReport(invocation, result, registration, dependencies);
  }
