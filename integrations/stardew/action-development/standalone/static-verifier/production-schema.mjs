 ...
 * GameBuddy.Stardew.dll / GameBuddy.Stardew.Core.dll /
 * FarmhandCapabilityPublicationProjection.Contract.dll closure with expected
...
const REPORT_CONTRACT_KEYS = new Set(["executed", "executable", "shell", "exitCode", "signal", "timeout", "args", "stderr"]);
...
function validateContractSection(report) {
  ...
  if (typeof contract.executed !== "boolean") fail("report_contract_executed");
  if (typeof contract.executable !== "string" || contract.executable.length === 0) fail("report_contract_executable");
  if (contract.shell !== false) fail("report_contract_shell");
  if (contract.exitCode !== null && !Number.isSafeInteger(contract.exitCode)) fail("report_contract_exit_code");
  if (contract.signal !== null && typeof contract.signal !== "string") fail("report_contract_signal");
  if (typeof contract.timeout !== "boolean") fail("report_contract_timeout");
  if (typeof contract.stderr !== "string") fail("report_contract_stderr");
  if (!Array.isArray(contract.args) || contract.args.some((arg) => typeof arg !== "string")) fail("report_contract_args");

  const contractCheck = report.checks[4];
  if (contractCheck.state === "not_run") {
    if (contract.executed !== false || contract.exitCode !== null || contract.signal !== null || contract.timeout !== false || contract.args.length !== 0 || contract.stderr !== "") {
      fail("report_contract_state");
    }
    return;
  }
  if (contract.args.length !== 7 || contract.args[1] !== "--expected-mod-sha256" || contract.args[3] !== "--expected-core-sha256") {
    fail("report_contract_state");
  }
  if (contractCheck.state === "passed") {
    if (contract.executed !== true || contract.exitCode !== 0 || contract.timeout !== false) fail("report_contract_state");
  } else if (contractCheck.reasonCode === FAILED_TARGET_PUBLICATION_CONTRACT_NONZERO) {
    if (contract.executed !== true || contract.exitCode === null || contract.exitCode === 0 || contract.timeout !== false) fail("report_contract_state");
  } else if (contractCheck.reasonCode === FAILED_TARGET_PUBLICATION_CONTRACT_OUTPUT) {
    if (contract.executed !== true || contract.exitCode !== 0 || contract.timeout !== false) fail("report_contract_state");
  } else if (contractCheck.reasonCode === FAILED_TARGET_PUBLICATION_CONTRACT_TIMEOUT) {
    if (contract.executed !== false || contract.exitCode !== null || contract.timeout !== true) fail("report_contract_state");
  } else {
    // spawn failure
    if (contract.executed !== false || contract.exitCode !== null || contract.timeout !== false) fail("report_contract_state");
  }
}
