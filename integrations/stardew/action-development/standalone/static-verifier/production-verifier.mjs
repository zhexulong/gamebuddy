...
const CONTRACT_SUCCESS_RECEIPT = "Farmhand capability publication identity/path/digest contract passed.";
...
function contractArguments(manifest) {
  ...
  const contract = byId.get("capability-publication-contract");
  return freeze([
 ...
 */
function runContractProcess(manifest, options) {
  const args = contractArguments(manifest);
...
function contractCheck(contract) {
  ...
  } else if (contract.executed && (contract.stderr.length > 0 || contract.stdout !== CONTRACT_SUCCESS_RECEIPT)) {
    state = "failed";
...
function composeReport(manifest, inputId, checks, sections, executable) {
  ...
  const report = {
    ...
    contract: deepFreeze({ ...sections.contract, executable }),
    checks: checksList,
    integration: freeze({
...
export async function verifyTargetPublication(manifest, options = {}) {
  ...
  const contract = await runContractProcess(manifest, resolved);
  const contractSection = freeze({
