...
} from "../standalone/standalone-extraction-manifest.mjs";

const MANIFEST_URL = new URL("../standalone/standalone-extraction-manifest.json", import.meta.url);
...
function entry(manifest, id) {
  return manifest.entries.find((candidate) => candidate.id === id);
}
...
test("rejects root tools, .ci, package.json, and Host paths", async () => {
  ...
  entry(rootTools, "devkit-workspace-link").path = "tools/run-stardew-action.mjs";
  assertRejected(rootTools, "root_tools_path_forbidden");

  const rootCi = structuredClone(manifest);
  entry(rootCi, "stardew-contract-exporter-project").path = ".ci/test-portfolio.json";
  assertRejected(rootCi, "root_ci_path_forbidden");

  const rootPackage = structuredClone(manifest);
  entry(rootPackage, "package-owned-node-pnpm-pins").path = "package.json";
  assertRejected(rootPackage, "root_package_json_forbidden");

  const rootHost = structuredClone(manifest);
  entry(rootHost, "stardew-core-source-closure").projectPath = "Host/src/stardew-adapter.csproj";
...
test("rejects workspace devkit declarations and non-packed artifact forms", async () => {
  ...
  entry(unpacked, "devkit-workspace-link").path = "inputs/devkit/package.json";
...
test("rejects escaping, absolute, and duplicate dependency paths", async () => {
  ...
  entry(parentEscape, "stardew-contract-exporter-project").path = "inputs/../ActionDevelopmentContractExport.csproj";
  assertRejected(parentEscape, "entry_path_escape");

  const absolute = structuredClone(manifest);
  entry(absolute, "stardew-contract-exporter-project").path = "/tmp/ActionDevelopmentContractExport.csproj";
  assertRejected(absolute, "entry_path_escape");

  const windowsAbsolute = structuredClone(manifest);
  entry(windowsAbsolute, "stardew-contract-exporter-project").path = "C:/ActionDevelopmentContractExport.csproj";
  assertRejected(windowsAbsolute, "entry_path_escape");

  const caseFoldedDuplicate = structuredClone(manifest);
  entry(caseFoldedDuplicate, "stardew-core-source-closure").projectPath = "inputs/stardew-contract-export/ActionDevelopmentContractExport.csproj";
...
test("rejects live/runtime inputs and any attempt to widen the root read policy", async () => {
  ...
  entry(sourceRoot, "stardew-contract-exporter-project").path = "inputs/../../integrations/stardew/tests/ActionDevelopmentContractExport.csproj";
