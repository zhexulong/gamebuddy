...
public static class FarmhandActionLifecycleWire
{
    public static string ToWireValue(this FarmhandActionLifecycle lifecycle) => lifecycle switch
    {
        FarmhandActionLifecycle.Published => "published",
        FarmhandActionLifecycle.Experimental => "experimental",
        _ => throw new ArgumentOutOfRangeException(nameof(lifecycle), lifecycle, "Unknown action lifecycle."),
    };
}

public enum FarmhandOperationKind
{
    Execution,
    ReadOnly,
}

public static class FarmhandOperationKindWire
{
    public static string ToWireValue(this FarmhandOperationKind kind) => kind switch
    {
        FarmhandOperationKind.Execution => "execution",
        FarmhandOperationKind.ReadOnly => "read_only",
        _ => throw new ArgumentOutOfRangeException(nameof(kind), kind, "Unknown farmhand operation kind."),
    };
}

/// <summary>
/// The only ordinary-Farmhand operation membership source. This catalog is
/// composed once at Mod startup; policy may expose or withdraw entries, but no
/// config, Host message, or bridge request can add an entry or choose a native
/// handler. Read-only entries deliberately have no handler group and never
/// enter the execution router.
/// </summary>
...
public static class FarmhandActionCatalog
{
    public static readonly IReadOnlyList<FarmhandActionRegistration> Registrations = Array.AsReadOnly(new[]
    {
        ...
        Registration("navigate_to_destination", "world_navigation", 1, FarmhandActionHandlerGroup.Movement),
    ...
    private static FarmhandActionRegistration Registration(
        ...
        new(actionId, familyId, identityVersion, lifecycle, FarmhandOperationKind.Execution, handlerGroup);

    private static FarmhandActionRegistration ReadOnlyRegistration(
        string actionId,
        string familyId,
        int identityVersion,
        FarmhandActionLifecycle lifecycle = FarmhandActionLifecycle.Published) =>
        new(actionId, familyId, identityVersion, lifecycle, FarmhandOperationKind.ReadOnly, null);
}
