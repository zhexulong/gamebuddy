...
public sealed class OpenBodyProgramJournalAuthority
{
    ...
    public BodyProgramControllerResult<BodyProgramTerminalResult> TryComplete(HostAdmissionGrant grant, BodyProgramTerminalResult result)
    {
        ...
        if (result.Outcome == BodyProgramNodeOutcome.Succeeded)
        {
            ...
            if (!ValidFactSet(descriptor!, grant, result.Facts)) return BodyProgramControllerResult.Failure<BodyProgramTerminalResult>(BodyProgramControllerResultCode.FactProvenanceMismatch);
        }
    ...
    private bool ValidFactSet(VerifiedBodyProgramNode descriptor, HostAdmissionGrant grant, IReadOnlyList<RuntimeFact>? facts)
    {
        if (facts is null || !this.catalog.TryGetAction(descriptor.ActionId, out BodyProgramActionDescriptor? action) || facts.Count != action!.OutputFacts.Count) return false;
        HashSet<string> names = new(StringComparer.Ordinal);
        foreach (RuntimeFact fact in facts)
        {
            if (fact is null || fact.ProgramId != grant.ProgramId || fact.NodeId != grant.NodeId || fact.NodeAttempt != grant.NodeAttempt
                || !names.Add(fact.FactName) || action.OutputFacts.SingleOrDefault(output => output.Name == fact.FactName) is not BodyProgramFactDescriptor output
