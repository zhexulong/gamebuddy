...
// Node link and identity checks remain defense in depth. On Windows the shared
// native policy is also invoked before every filesystem traversal and read.
...
async function createProductionArtifactManifestWithPolicy(artifactRoot, policy) {
  ...
  const manifest = { schemaVersion: 1, browserContract: BROWSER_CONTRACT, profileId: PROFILE_ID, entryHtml: "index.html", assets };
  await writeFile(resolve(root, ARTIFACT_MANIFEST_FILE), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  await verifyProductionArtifactManifestWithPolicy(root, policy);
  return manifest;
}

export async function verifyProductionArtifactManifest(artifactRoot, policy) {
