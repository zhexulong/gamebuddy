...
export function ReferenceApp() {
  ...
  const eventStream = view.kind === "ready" ? view.session.snapshot.eventStream : null;

  useEffect(() => {
    if (view.kind !== "ready" || eventStream === null) return;
    let disposed = false;
    let recovering = false;
    let work = Promise.resolve();
    let cursor = eventStream.cursor;
